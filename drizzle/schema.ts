...
export const users = mysqlTable("users", {
  ...
  personType: mysqlEnum("personType", ["full_user", "teammate"])
    .default("full_user")
    .notNull(),
...
export const ptoPolicies = mysqlTable(
  "pto_policies",
  {
    id: int("id").autoincrement().primaryKey(),
    ptoType: mysqlEnum("ptoType", [
      "vacation",
      "sick",
      "personal",
      "bereavement",
    ...
    annualAccrualDays: decimal("annualAccrualDays", {
      precision: 7,
      scale: 2,
    }).notNull(),
    carryoverCapDays: decimal("carryoverCapDays", { precision: 7, scale: 2 })
      .notNull()
      .default("0"),
  ...
  table => [
    uniqueIndex("pto_policies_type_effective_date_uq").on(
      table.ptoType,
      table.effectiveDate
    ),
    index("pto_policies_type_effective_date_idx").on(
      table.ptoType,
      table.effectiveDate
    ),
  ]
...
export const ptoSettings = mysqlTable("pto_settings", {
  id: int("id").autoincrement().primaryKey(),
  negativeBalanceAllowed: boolean("negativeBalanceAllowed")
    .notNull()
    .default(false),
  payoutAllowed: boolean("payoutAllowed").notNull().default(false),
  reportingLineSource: varchar("reportingLineSource", { length: 128 })
    .notNull()
    .default("users.reportsToId"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
export type PtoSettings = typeof ptoSettings.$inferSelect;

export const ptoRequests = mysqlTable(
  "pto_requests",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    requestedDays: decimal("requestedDays", {
      precision: 7,
      scale: 2,
    }).notNull(),
  ...
  table => [
    index("pto_requests_employee_status_idx").on(
      table.employeeId,
      table.status
    ),
    index("pto_requests_manager_status_created_idx").on(
      table.managerId,
      table.status,
      table.createdAt
    ),
    index("pto_requests_dates_status_idx").on(
      table.startDate,
      table.endDate,
      table.status
    ),
...
export const ptoBalanceAdjustments = mysqlTable(
  ...
  table => [
    ...
    index("pto_balance_adjustments_employee_type_date_idx").on(
      table.employeeId,
      table.ptoType,
      table.effectiveDate
    ),
...
export type InsertPtoBalanceAdjustment =
  typeof ptoBalanceAdjustments.$inferInsert;
...
export const ptoRequestEvents = mysqlTable(
  "pto_request_events",
  {
    id: int("id").autoincrement().primaryKey(),
    ptoRequestId: int("ptoRequestId")
      .notNull()
      .references(() => ptoRequests.id, { onDelete: "cascade" }),
    actorId: int("actorId")
      .notNull()
      .references(() => users.id),
  ...
  table => [
    index("pto_request_events_request_created_idx").on(
      table.ptoRequestId,
      table.createdAt
    ),
  ]
...
// Chat entry is independent from a user's channel memberships. This allows a
// teammate to start a private conversation without being added to a company
// channel solely to make Chat visible.
export const chatUserAccess = mysqlTable(
  "chat_user_access",
  {
    id: int("id").autoincrement().primaryKey(),
    userId: int("userId")
...
export const leadSources = mysqlTable("lead_sources", {
  ...
  requireAgreementForSubSources: boolean("requireAgreementForSubSources")
    .default(false)
    .notNull(),
...
export const partnerPortalMagicLinks = mysqlTable(
  "partner_portal_magic_links",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  table => [
    index("partner_portal_magic_links_email_requested_idx").on(
      table.email,
      table.requestedAt
    ),
  ]
);
export type PartnerPortalMagicLink =
  typeof partnerPortalMagicLinks.$inferSelect;
export type InsertPartnerPortalMagicLink =
  typeof partnerPortalMagicLinks.$inferInsert;
...
export const contacts = mysqlTable(
  "contacts",
  {
    ...
    leadSourceType: mysqlEnum("leadSourceType", [
      "referral",
      "paid_lead",
      "paid_partnership",
      "organic",
    ...
    doNotContactByUserId: int("doNotContactByUserId").references(
      () => users.id
    ),
    // Dead Connections list exclusions. A null expiry means the removal is permanent.
    deadConnectionsExclusionMode: mysqlEnum("deadConnectionsExclusionMode", [
      "permanent",
      "temporary",
    ]),
    ...
    deadConnectionsExcludedByUserId: int(
      "deadConnectionsExcludedByUserId"
    ).references(() => users.id),
    // Email deliverability tracking
    emailStatus: mysqlEnum("emailStatus", ["valid", "bounced", "unsubscribed"])
      .default("valid")
      .notNull(),
    ...
    smsMarketingOptOutReason: varchar("smsMarketingOptOutReason", {
      length: 255,
    }),
    // The shared Hot Leads outreach guard. This is set only after a user confirms
    // a send from Hot Leads and prevents the same contact from receiving another
    // Hot Leads text for 24 hours, even when they appear on multiple lists.
    lastHotLeadTextAt: timestamp("lastHotLeadTextAt"),
    lastHotLeadTextById: int("lastHotLeadTextById").references(() => users.id),
  ...
  table => ({
    ...
    cohortActiveCreatedAtIdx: index("contacts_archived_createdAt_idx").on(
      table.archivedAt,
      table.createdAt
    ),
    deadConnectionsExclusionIdx: index(
      "contacts_dead_connections_exclusion_idx"
    ).on(table.deadConnectionsExcludedAt, table.deadConnectionsExcludedUntil),
  })
);
...
// ─── Agent Connections ────────────────────────────────────────────────────────
export const agentConnections = mysqlTable(
  "agent_connections",
  {
    id: int("id").autoincrement().primaryKey(),
    agentId: int("agentId")
    ...
    appointmentSetByUserId: int("appointmentSetByUserId").references(
      () => users.id
    ),
    ...
    mergeArchivedById: int("mergeArchivedById").references(() => users.id, {
      onDelete: "set null",
    }),
  ...
  table => ({
    agentContactUniqueIdx: uniqueIndex(
      "agent_connections_agent_contact_uidx"
    ).on(table.agentId, table.contactId),
  })
);
...
export const propertyOwnership = mysqlTable("property_ownership", {
  ...
  ownerContactId: int("ownerContactId")
    .notNull()
    .references(() => contacts.id),
...
export const transactions = mysqlTable("transactions", {
  ...
  status: mysqlEnum("status", ["under_contract", "closed", "terminated"])
  ...
  grossCommissionIncome: decimal("grossCommissionIncome", {
    precision: 12,
    scale: 2,
  }),
  commissionRate: decimal("commissionRate", { precision: 5, scale: 4 }),
  commissionType: mysqlEnum("commissionType", ["percentage", "flat"])
    .default("percentage")
    .notNull(),
  ...
  buyerCommissionRate: decimal("buyerCommissionRate", {
    precision: 5,
    scale: 4,
  }),
  buyerCommissionType: mysqlEnum("buyerCommissionType", [
    "percentage",
    "flat",
  ]).default("percentage"),
  ...
  referralAgentId: int("referralAgentId").references(() => referralAgents.id, {
    onDelete: "set null",
  }),
...
export const reviewRequests = mysqlTable(
  ...
  table => ({
    transactionRecipientIdx: uniqueIndex(
      "review_requests_transaction_recipient_uidx"
    ).on(table.transactionId, table.recipientEmail),
...
export const reviews = mysqlTable(
  ...
  table => ({
    agentSubmittedIdx: index("reviews_agent_submitted_idx").on(
      table.agentId,
      table.submittedAt
    ),
...
export const isaOutcomeAttributions = mysqlTable(
  "isa_outcome_attributions",
  {
    id: int("id").autoincrement().primaryKey(),
    transactionId: int("transactionId")
      .notNull()
    ...
    contactId: int("contactId")
      .notNull()
      .references(() => contacts.id),
    appointmentConnectionId: int("appointmentConnectionId").references(
      () => agentConnections.id,
      { onDelete: "set null" }
    ),
    attributionBasis: mysqlEnum("attributionBasis", [
      "appointment_setter",
      "assigned_isa",
      "manual",
    ]).notNull(),
  ...
  table => ({
    transactionUniqueIdx: uniqueIndex("isa_outcome_transaction_uidx").on(
      table.transactionId
    ),
    isaStatusIdx: index("isa_outcome_isa_status_idx").on(
      table.isaId,
      table.status
    ),
    contactIdx: index("isa_outcome_contact_idx").on(table.contactId),
    appointmentIdx: index("isa_outcome_appointment_idx").on(
      table.appointmentConnectionId
    ),
  })
);

export type IsaOutcomeAttribution = typeof isaOutcomeAttributions.$inferSelect;
export type InsertIsaOutcomeAttribution =
  typeof isaOutcomeAttributions.$inferInsert;

// ─── Transaction Export History ───────────────────────────────────────────────
export const transactionExports = mysqlTable(
  "transaction_exports",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  table => ({
    exportedByIdx: index("transaction_exports_exportedBy_idx").on(
      table.exportedById
    ),
    createdAtIdx: index("transaction_exports_createdAt_idx").on(
      table.createdAt
    ),
  })
);
...
export const transactionPayoutItems = mysqlTable(
  "transaction_payout_items",
  {
    ...
    expMemoNumber: varchar("expMemoNumber", { length: 64 }),
    // For referral fees: who pays (savvy, agent, split, group_leader)
    referralFeePaidBy: mysqlEnum("referralFeePaidBy", [
      "savvy",
      "agent",
      "split",
      "group_leader",
...
export type InsertTransactionPayoutItem =
  typeof transactionPayoutItems.$inferInsert;
...
export const tasks = mysqlTable("tasks", {
  ...
  relatedTransactionId: int("relatedTransactionId").references(
    () => transactions.id
  ),
  relatedPropertyId: int("relatedPropertyId").references(() => properties.id),
  relatedAgentConnectionId: int("relatedAgentConnectionId").references(
    () => agentConnections.id
  ),
...
export const webinarMarketingTemplates = mysqlTable(
...
export type WebinarMarketingTemplate =
  typeof webinarMarketingTemplates.$inferSelect;
export type InsertWebinarMarketingTemplate =
  typeof webinarMarketingTemplates.$inferInsert;

export const webinarMarketingTemplateTasks = mysqlTable(
  "webinar_marketing_template_tasks",
  {
    id: int("id").autoincrement().primaryKey(),
    templateId: int("templateId")
      .notNull()
      .references(() => webinarMarketingTemplates.id, { onDelete: "cascade" }),
    title: varchar("title", { length: 512 }).notNull(),
    description: text("description"),
    assignedToId: int("assignedToId").references(() => users.id, {
      onDelete: "set null",
  ...
  table => [
    index("webinar_marketing_template_tasks_template_idx").on(
      table.templateId,
      table.sortOrder
    ),
  ]
);
export type WebinarMarketingTemplateTask =
  typeof webinarMarketingTemplateTasks.$inferSelect;
export type InsertWebinarMarketingTemplateTask =
  typeof webinarMarketingTemplateTasks.$inferInsert;

export const webinars = mysqlTable(
  "webinars",
  {
    id: int("id").autoincrement().primaryKey(),
    title: varchar("title", { length: 255 }).notNull(),
    ...
    timezone: varchar("timezone", { length: 64 })
      ...
      .notNull(),
    status: mysqlEnum("status", ["scheduled", "live", "ended", "cancelled"])
      .default("scheduled")
      .notNull(),
    registrationEnabled: boolean("registrationEnabled").default(true).notNull(),
    registrationApproval: mysqlEnum("registrationApproval", [
      "automatically",
      "manually",
      "no_registration",
    ])
      ...
      .notNull(),
    marketingTemplateId: int("marketingTemplateId").references(
      () => webinarMarketingTemplates.id,
      { onDelete: "set null" }
    ),
...
export const webinarTaskLinks = mysqlTable(
  "webinar_task_links",
  {
    ...
    templateTaskId: int("templateTaskId").references(
      () => webinarMarketingTemplateTasks.id,
      { onDelete: "set null" }
    ),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  },
...
export const webinarAttendees = mysqlTable(
  "webinar_attendees",
  {
    id: int("id").autoincrement().primaryKey(),
    webinarId: int("webinarId")
  ...
  table => [
    uniqueIndex("webinar_attendees_registrant_unique").on(
      table.webinarId,
      table.zoomRegistrantId
    ),
    index("webinar_attendees_webinar_status_idx").on(
      table.webinarId,
      table.status
    ),
    index("webinar_attendees_webinar_email_idx").on(
      table.webinarId,
      table.email
    ),
...
export const zoomWebhookEvents = mysqlTable(
  "zoom_webhook_events",
  {
    id: int("id").autoincrement().primaryKey(),
    eventKey: varchar("eventKey", { length: 128 }).notNull().unique(),
    webinarId: int("webinarId").references(() => webinars.id, {
      onDelete: "set null",
    }),
  ...
  table => [
    index("zoom_webhook_events_webinar_idx").on(
      table.webinarId,
      table.receivedAt
    ),
  ]
...
export const communications = mysqlTable(
  ...
  table => [
    index("communications_contact_pinned_idx").on(
...
export const listings = mysqlTable("listings", {
  ...
  listingStatus: mysqlEnum("listingStatus", [
    "active",
    "terminated",
    "expired",
    "under_contract",
...
export const smartPlans = mysqlTable("smart_plans", {
  ...
  triggerLeadSourceId: int("triggerLeadSourceId").references(
    () => leadSources.id
  ),
  ...
  triggerScope: mysqlEnum("triggerScope", [
    "new_only",
    "existing_and_new",
    "manual",
  ])
  ...
  defaultSendWindowEnabled: boolean("defaultSendWindowEnabled")
    .default(true)
    .notNull(),
  ...
  defaultSendTimezone: varchar("defaultSendTimezone", { length: 64 })
    .default("America/New_York")
    .notNull(),
  // Optional, plan-scoped property-address merge behavior for specialized intake flows.
  propertyAddressFromNotes: boolean("propertyAddressFromNotes")
    .default(false)
    .notNull(),
...
export const oneTimeSends = mysqlTable(
  "one_time_sends",
  {
    id: int("id").autoincrement().primaryKey(),
    name: varchar("name", { length: 255 }).notNull(),
...
export const oneTimeSendRecipients = mysqlTable(
  "one_time_send_recipients",
  {
    id: int("id").autoincrement().primaryKey(),
    sendId: int("sendId")
      .notNull()
      .references(() => oneTimeSends.id),
    contactId: int("contactId")
      .notNull()
      .references(() => contacts.id),
  ...
  table => [
    uniqueIndex("one_time_send_recipient_unique").on(
      table.sendId,
      table.contactId,
...
export type InsertOneTimeSendRecipient =
  typeof oneTimeSendRecipients.$inferInsert;
...
export const oneTimeSendMessageEvents = mysqlTable(
  "one_time_send_message_events",
  {
    id: int("id").autoincrement().primaryKey(),
    recipientId: int("recipientId")
      .notNull()
      .references(() => oneTimeSendRecipients.id, { onDelete: "cascade" }),
  ...
  table => [
    uniqueIndex("one_time_send_message_events_provider_event_unique").on(
      table.providerEventId
    ),
    index("one_time_send_message_events_recipient_type_idx").on(
      table.recipientId,
      table.eventType
    ),
  ]
);
export type OneTimeSendMessageEvent =
  typeof oneTimeSendMessageEvents.$inferSelect;
export type InsertOneTimeSendMessageEvent =
  typeof oneTimeSendMessageEvents.$inferInsert;

export const smartPlanSteps = mysqlTable("smart_plan_steps", {
  id: int("id").autoincrement().primaryKey(),
  planId: int("planId")
    .notNull()
    .references(() => smartPlans.id),
...
export const smartPlanEnrollments = mysqlTable(
  "smart_plan_enrollments",
  {
    ...
    bypassInitialSendWindow: boolean("bypassInitialSendWindow")
      .default(false)
      .notNull(),
...
export type InsertSmartPlanEnrollment =
  typeof smartPlanEnrollments.$inferInsert;

export const smartPlanExecutions = mysqlTable(
  "smart_plan_executions",
  {
    id: int("id").autoincrement().primaryKey(),
    enrollmentId: int("enrollmentId")
      .notNull()
      .references(() => smartPlanEnrollments.id),
    stepId: int("stepId")
      .notNull()
      .references(() => smartPlanSteps.id),
    channel: mysqlEnum("channel", ["email", "sms"]).notNull(),
  ...
  table => [
    ...
    index("smart_plan_executions_provider_message_idx").on(
      table.providerMessageId
    ),
    uniqueIndex("smart_plan_executions_reply_token_unique").on(
      table.replyToken
    ),
  ]
);
...
export const smartPlanMessageEvents = mysqlTable(
  "smart_plan_message_events",
  {
    id: int("id").autoincrement().primaryKey(),
    executionId: int("executionId")
      .notNull()
      .references(() => smartPlanExecutions.id, { onDelete: "cascade" }),
    provider: varchar("provider", { length: 64 }).notNull(),
    providerEventId: varchar("providerEventId", { length: 255 }),
    eventType: varchar("eventType", { length: 64 }).notNull(),
    occurredAt: timestamp("occurredAt").defaultNow().notNull(),
  ...
  table => [
    uniqueIndex("smart_plan_message_events_provider_event_unique").on(
      table.providerEventId
    ),
    index("smart_plan_message_events_execution_type_idx").on(
      table.executionId,
      table.eventType
    ),
  ]
...
export type InsertSmartPlanMessageEvent =
  typeof smartPlanMessageEvents.$inferInsert;
...
export type InsertTransactionDocument =
  typeof transactionDocuments.$inferInsert;
...
export const onboardingTemplates = mysqlTable("onboarding_templates", {
  ...
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  type: mysqlEnum("type", ["onboarding", "offboarding"])
    .default("onboarding")
    .notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
...
export type OnboardingTemplateTask =
  typeof onboardingTemplateTasks.$inferSelect;
...
export const onboardingInstanceTasks = mysqlTable(
...
export const leadershipFeedback = mysqlTable("leadership_feedback", {
  ...
  agentUserId: int("agentUserId")
    .notNull()
    .references(() => users.id),
  conductedByUserId: int("conductedByUserId")
    .notNull()
    .references(() => users.id),
...
export const commissionExceptions = mysqlTable("commission_exceptions", {
  ...
  brokerageSplitPct: decimal("brokerageSplitPct", {
    precision: 5,
    scale: 2,
  }).notNull(),
  teamLeaderSplitPct: decimal("teamLeaderSplitPct", { precision: 5, scale: 2 })
    .default("0")
    .notNull(),
  referralSplitPct: decimal("referralSplitPct", { precision: 5, scale: 2 })
    .default("0")
    .notNull(),
  // Status
...
export type InsertCommissionException =
  typeof commissionExceptions.$inferInsert;
...
export const userDocuments = mysqlTable("user_documents", {
  ...
  aiSummaryStatus: varchar("aiSummaryStatus", { length: 32 })
...
export type InsertRoleResponsibility =
  typeof rolesResponsibilities.$inferInsert;

export const rrSops = mysqlTable(
  "rr_sops",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  table => [
    index("rr_sops_responsibility_idx").on(
...
export const rrSopSteps = mysqlTable(
  "rr_sop_steps",
  {
    id: int("id").autoincrement().primaryKey(),
    sopId: int("sopId")
      .notNull()
      .references(() => rrSops.id, { onDelete: "cascade" }),
...
export const rrResources = mysqlTable(
  "rr_resources",
  {
    id: int("id").autoincrement().primaryKey(),
    responsibilityId: int("responsibilityId").references(
      () => rolesResponsibilities.id,
      { onDelete: "cascade" }
    ),
    ...
    url: text("url"),
    userDocumentId: int("userDocumentId").references(() => userDocuments.id, {
      onDelete: "set null",
    }),
    sortOrder: int("sortOrder").default(0).notNull(),
    createdById: int("createdById").references(() => users.id, {
  ...
  table => [
    index("rr_resources_responsibility_idx").on(
      table.responsibilityId,
      table.sortOrder
    ),
...
export const rrTaskLinks = mysqlTable(
  "rr_task_links",
  {
    id: int("id").autoincrement().primaryKey(),
    responsibilityId: int("responsibilityId")
  ...
  table => [
    index("rr_task_links_responsibility_idx").on(table.responsibilityId),
  ]
);
export type RrTaskLink = typeof rrTaskLinks.$inferSelect;
export type InsertRrTaskLink = typeof rrTaskLinks.$inferInsert;

export const rrScorecardMetrics = mysqlTable(
  "rr_scorecard_metrics",
  {
    id: int("id").autoincrement().primaryKey(),
    responsibilityId: int("responsibilityId")
      .notNull()
    ...
    displayFormat: mysqlEnum("displayFormat", [
      ...
      .default("number")
      .notNull(),
    rollupMethod: mysqlEnum("rollupMethod", [
      "sum",
      "average",
      "count",
      "unique_count",
  ...
  table => [
    index("rr_metrics_responsibility_idx").on(
      table.responsibilityId,
      table.status
    ),
...
export const rrMetricValues = mysqlTable(
  "rr_metric_values",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  table => [
    uniqueIndex("rr_metric_period_unique").on(
      table.metricId,
      table.periodStart,
      table.periodEnd
    ),
...
export const agentProfiles = mysqlTable("agent_profiles", {
  ...
  agentStatus: mysqlEnum("agentStatus", [
    "active",
    "paused",
    "recruiting",
    "offboarded",
  ...
  directoryProductionLevel: mysqlEnum("directoryProductionLevel", [
    "emerging",
    "growing",
    "established",
    "elite",
  ...
  transactionCoordinatorAssigned: varchar("transactionCoordinatorAssigned", {
    length: 255,
  }),
  ...
  anniversaryRecognitionOptIn: boolean("anniversaryRecognitionOptIn").default(
    true
  ),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
...
export const agentRenewals = mysqlTable(
  "agent_renewals",
  {
    id: int("id").autoincrement().primaryKey(),
    agentId: int("agentId")
  ...
  table => [
    index("agent_renewals_agent_status_date_idx").on(
      table.agentId,
      table.status,
      table.renewalDate
    ),
    index("agent_renewals_status_completed_idx").on(
      table.status,
      table.completedAt
    ),
  ]
...
export const adminProfiles = mysqlTable("admin_profiles", {
  ...
  userId: int("userId")
    ...
    .unique()
    .references(() => users.id, { onDelete: "cascade" }),
  adminStatus: mysqlEnum("adminStatus", [
    "active",
    "inactive",
    "on_leave",
    "offboarded",
...
// ─── Agent Goals ──────────────────────────────────────────────────────────────
export const agentGoals = mysqlTable(
  "agent_goals",
  {
    id: int("id").autoincrement().primaryKey(),
    agentId: int("agentId")
  ...
  table => ({
    ...
    agentYearMonthUnq: uniqueIndex("agent_goals_agent_year_month_unq").on(
      table.agentId,
      table.year,
      table.month
    ),
...
export const companyGoals = mysqlTable(
  "company_goals",
  {
    id: int("id").autoincrement().primaryKey(),
    title: varchar("title", { length: 255 }).notNull(),
...
export const automaticMarketingGraphics = mysqlTable(
  "automatic_marketing_graphics",
  {
    ...
    agentId: int("agentId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    graphicType: mysqlEnum("graphicType", [
      "under_contract",
      "just_closed",
      "just_listed",
    ]).notNull(),
  ...
  table => [
    index("automatic_marketing_graphics_agent_created_idx").on(
      table.agentId,
      table.createdAt
    ),
  ]
);
export type AutomaticMarketingGraphic =
  typeof automaticMarketingGraphics.$inferSelect;
export type InsertAutomaticMarketingGraphic =
  typeof automaticMarketingGraphics.$inferInsert;
...
export const techRequests = mysqlTable(
  ...
  {
    id: int("id").autoincrement().primaryKey(),
    trackingNumber: varchar("trackingNumber", { length: 32 })
      .notNull()
      .unique(),
...
export const customEmailNotifications = mysqlTable(
  "custom_email_notifications",
  {
    id: int("id").autoincrement().primaryKey(),
...
export type CustomEmailNotification =
  typeof customEmailNotifications.$inferSelect;
export type InsertCustomEmailNotification =
  typeof customEmailNotifications.$inferInsert;
...
export const pipelineEmailTemplates = mysqlTable(
  "pipeline_email_templates",
  {
    ...
    visibleToRoles: varchar("visibleToRoles", { length: 64 })
      .notNull()
      .default(""),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
...
export type InsertPipelineEmailTemplate =
  typeof pipelineEmailTemplates.$inferInsert;
...
export const pipelineEmailDailyQuotas = mysqlTable(
  ...
  table => [
    uniqueIndex("pipeline_email_daily_quota_user_day_unique").on(
      table.senderUserId,
      table.sendDate
    ),
  ]
);
export type PipelineEmailDailyQuota =
  typeof pipelineEmailDailyQuotas.$inferSelect;
...
export const pipelineEmailBatches = mysqlTable(
  "pipeline_email_batches",
  {
    ...
    senderUserId: int("senderUserId")
      .notNull()
      .references(() => users.id, { onDelete: "restrict" }),
    templateId: int("templateId").references(() => pipelineEmailTemplates.id, {
      onDelete: "set null",
    }),
    subject: varchar("subject", { length: 512 }).notNull(),
    ...
    deliveredCount: int("deliveredCount").notNull().default(0),
    failedCount: int("failedCount").notNull().default(0),
    status: mysqlEnum("status", ["sending", "completed", "partial", "failed"])
      .notNull()
      .default("sending"),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    completedAt: timestamp("completedAt"),
  ...
  table => [
    index("pipeline_email_batches_sender_created_idx").on(
      table.senderUserId,
      table.createdAt
    ),
  ]
...
export const pipelineEmailSends = mysqlTable(
  "pipeline_email_sends",
  {
    ...
    batchId: int("batchId")
      .notNull()
      .references(() => pipelineEmailBatches.id, { onDelete: "cascade" }),
    senderUserId: int("senderUserId")
      .notNull()
      .references(() => users.id, { onDelete: "restrict" }),
    contactId: int("contactId")
    ...
    recipientEmail: varchar("recipientEmail", { length: 320 }).notNull(),
    status: mysqlEnum("status", ["sending", "sent", "failed"])
      .notNull()
      .default("sending"),
  ...
  table => [
    ...
    index("pipeline_email_sends_contact_idx").on(
      table.contactId,
      table.createdAt
    ),
  ]
...
export const pmProjectCollaborators = mysqlTable(
  ...
  table => [
    uniqueIndex("pm_project_collaborators_project_user_unique").on(
      table.projectId,
      table.userId
    ),
  ]
...
export const pmTasks = mysqlTable(
  "pm_tasks",
  {
    id: int("id").autoincrement().primaryKey(),
    projectId: int("projectId")
...
export const pmPersonalTodos = mysqlTable(
  ...
  table => [
    index("pm_personal_todos_user_status_idx").on(
      table.userId,
      table.completed,
      table.dueDate
    ),
...
export const pmNoteMentions = mysqlTable("pm_note_mentions", {
  ...
  noteId: int("noteId")
    .notNull()
    .references(() => pmProjectNotes.id, { onDelete: "cascade" }),
  mentionedUserId: int("mentionedUserId")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
...
export const agentSupportAssignments = mysqlTable("agent_support_assignments", {
  id: int("id").autoincrement().primaryKey(),
  agentSupportUserId: int("agentSupportUserId")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  agentId: int("agentId")
    .notNull()
...
export type AgentSupportAssignment =
  typeof agentSupportAssignments.$inferSelect;
export type InsertAgentSupportAssignment =
  typeof agentSupportAssignments.$inferInsert;

// ─── Knowledge Base ───────────────────────────────────────────────────────────
export const kbCategories = mysqlTable("kb_categories", {
  id: int("id").autoincrement().primaryKey(),
  name: varchar("name", { length: 255 }).notNull(),
  type: mysqlEnum("type", ["sop", "reference", "training"])
    .notNull()
    .default("reference"),
  description: text("description"),
  sortOrder: int("sortOrder").notNull().default(0),
...
export const duplicateContactPairs = mysqlTable("duplicate_contact_pairs", {
  id: int("id").autoincrement().primaryKey(),
  contactAId: int("contactAId")
    .notNull()
    .references(() => contacts.id, { onDelete: "cascade" }),
  contactBId: int("contactBId")
    .notNull()
    .references(() => contacts.id, { onDelete: "cascade" }),
  ...
  // Admin review status
  status: mysqlEnum("status", ["pending", "merged", "dismissed"])
    .notNull()
    .default("pending"),
  // Which contact was kept after merge (null until merged)
  keptContactId: int("keptContactId").references(() => contacts.id, {
    onDelete: "set null",
  }),
  reviewedById: int("reviewedById").references(() => users.id, {
    onDelete: "set null",
...
export type InsertDuplicateContactPair =
  typeof duplicateContactPairs.$inferInsert;
...
export const contactMergeArchives = mysqlTable(
  "contact_merge_archives",
  {
    id: int("id").autoincrement().primaryKey(),
    mergePairId: int("mergePairId")
      .notNull()
      .references(() => duplicateContactPairs.id, { onDelete: "cascade" }),
    winnerContactId: int("winnerContactId")
      .notNull()
      .references(() => contacts.id),
    loserContactId: int("loserContactId")
      .notNull()
      .references(() => contacts.id),
    kind: varchar("kind", { length: 64 }).notNull(),
    sourceContactId: int("sourceContactId").references(() => contacts.id, {
      onDelete: "set null",
    }),
    ...
    restoredById: int("restoredById").references(() => users.id, {
      onDelete: "set null",
    }),
  },
  table => [
    index("contact_merge_archives_pair_idx").on(
      table.mergePairId,
      table.archivedAt
    ),
    index("contact_merge_archives_loser_idx").on(
      table.loserContactId,
      table.archivedAt
    ),
  ]
);
...
export const webhookEndpoints = mysqlTable("webhook_endpoints", {
  ...
  signatureHeader: varchar("signatureHeader", { length: 128 }).default(
    "x-savvy-signature"
  ),
  // Default lead source to assign when not specified in payload
  defaultLeadSourceId: int("defaultLeadSourceId").references(
    () => leadSources.id,
    { onDelete: "set null" }
  ),
  // Default agent to assign when not specified in payload
  defaultAgentId: int("defaultAgentId").references(() => users.id, {
    onDelete: "set null",
  }),
  isActive: boolean("isActive").default(true).notNull(),
  createdById: int("createdById").references(() => users.id, {
...
export const webhookLogs = mysqlTable("webhook_logs", {
  id: int("id").autoincrement().primaryKey(),
  endpointId: int("endpointId").references(() => webhookEndpoints.id, {
    onDelete: "set null",
  }),
...
export const scheduledReportRuns = mysqlTable(
  "scheduled_report_runs",
  {
    ...
    successfulRecipientCount: int("successfulRecipientCount")
      .notNull()
      .default(0),
    errorMessage: text("errorMessage"),
    startedAt: timestamp("startedAt").defaultNow().notNull(),
  ...
  table => [
    uniqueIndex("scheduled_report_runs_key_date_unique").on(
      table.reportKey,
      table.reportDate
    ),
...
export const dailyAgentReports = mysqlTable(
  ...
  table => [
    uniqueIndex("daily_agent_reports_agent_date_unique").on(
      table.agentId,
      table.reportDate
    ),
...
export const shortLinks = mysqlTable(
  "short_links",
  {
    ...
    status: mysqlEnum("status", ["active", "disabled", "archived"])
      .default("active")
      .notNull(),
...
export const shortLinkClicks = mysqlTable(
  ...
  table => [
    index("short_link_clicks_link_clicked_idx").on(
      table.shortLinkId,
      table.clickedAt
    ),
  ]
...
export const vendorLists = mysqlTable(
  ...
  table => [
    index("vendor_lists_published_updated_idx").on(
      table.isPublished,
      table.updatedAt
    ),
  ]
...
export const vendorCategories = mysqlTable(
  "vendor_categories",
  {
    id: int("id").autoincrement().primaryKey(),
    vendorListId: int("vendorListId")
      .notNull()
      .references(() => vendorLists.id, { onDelete: "cascade" }),
    name: varchar("name", { length: 120 }).notNull(),
    description: text("description"),
  ...
  table => [
    index("vendor_categories_list_sort_idx").on(
      table.vendorListId,
      table.sortOrder
    ),
  ]
...
export const vendors = mysqlTable(
  ...
  {
    id: int("id").autoincrement().primaryKey(),
    vendorCategoryId: int("vendorCategoryId")
      .notNull()
  ...
  table => [
    index("vendors_category_sort_idx").on(
      table.vendorCategoryId,
      table.sortOrder
    ),
  ]
);
...
export const vendorFeaturedSubscriptions = mysqlTable(
  "vendor_featured_subscriptions",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  table => [
    index("vendor_featured_subscriptions_vendor_status_idx").on(
      table.vendorId,
      table.billingStatus
    ),
    index("vendor_featured_subscriptions_agent_status_idx").on(
      table.agentId,
      table.billingStatus
    ),
    index("vendor_featured_subscriptions_customer_idx").on(
      table.stripeCustomerId
    ),
  ]
);
export type VendorFeaturedSubscription =
  typeof vendorFeaturedSubscriptions.$inferSelect;
export type InsertVendorFeaturedSubscription =
  typeof vendorFeaturedSubscriptions.$inferInsert;

export const vendorBillingPayments = mysqlTable(
  ...
  table => [
    ...
    index("vendor_billing_payments_subscription_paid_idx").on(
      table.vendorFeaturedSubscriptionId,
      table.paidAt
    ),
    index("vendor_billing_payments_status_paid_idx").on(
      table.paymentStatus,
      table.paidAt
    ),
  ]
...
export type InsertVendorBillingPayment =
  typeof vendorBillingPayments.$inferInsert;
...
export const vendorBillingWebhookEvents = mysqlTable(
  "vendor_billing_webhook_events",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  table => [
    ...
    index("vendor_billing_webhook_events_status_idx").on(
      table.status,
      table.createdAt
    ),
    index("vendor_billing_webhook_events_subscription_idx").on(
      table.billingSubscriptionId,
      table.createdAt
    ),
  ]
);
export type VendorBillingWebhookEvent =
  typeof vendorBillingWebhookEvents.$inferSelect;
export type InsertVendorBillingWebhookEvent =
  typeof vendorBillingWebhookEvents.$inferInsert;
...
export const landingPages = mysqlTable(
  "landing_pages",
  {
    ...
    status: mysqlEnum("status", [
      ...
      .default("draft")
      .notNull(),
    primaryConversionType: mysqlEnum("primaryConversionType", [
      "form",
      "calendly",
    ])
      .default("form")
    ...
    postSubmitType: mysqlEnum("postSubmitType", [
      "inline",
      "landing_page",
      "external",
    ])
...
export const landingPageSessions = mysqlTable(
  ...
  table => [
    uniqueIndex("landing_page_session_unique").on(
      table.landingPageId,
      table.sessionId
    ),
    index("landing_page_sessions_page_first_viewed_idx").on(
      table.landingPageId,
      table.firstViewedAt
    ),
  ]
...
export const landingPageSubmissions = mysqlTable(
  "landing_page_submissions",
  {
    ...
    appliedLeadSourceId: int("appliedLeadSourceId").references(
      () => leadSources.id
    ),
  ...
  table => [
    index("landing_page_submissions_page_created_idx").on(
      table.landingPageId,
      table.createdAt
    ),
    index("landing_page_submissions_contact_idx").on(
      table.contactId,
      table.createdAt
    ),
    index("landing_page_submissions_session_idx").on(
      table.sessionId,
      table.createdAt
    ),
  ]
...
export const landingPageEvents = mysqlTable(
  "landing_page_events",
  {
    id: int("id").autoincrement().primaryKey(),
    landingPageId: int("landingPageId")
    ...
    submissionId: int("submissionId").references(
      () => landingPageSubmissions.id,
      { onDelete: "set null" }
    ),
    contactId: int("contactId").references(() => contacts.id, {
      onDelete: "set null",
  ...
  table => [
    index("landing_page_events_page_type_occurred_idx").on(
      table.landingPageId,
      table.eventType,
      table.occurredAt
    ),
...
export const landingPageSmsConsents = mysqlTable(
  "landing_page_sms_consents",
  {
    id: int("id").autoincrement().primaryKey(),
    landingPageId: int("landingPageId")
      .notNull()
    ...
    submissionId: int("submissionId")
      .notNull()
      .references(() => landingPageSubmissions.id, { onDelete: "cascade" }),
    contactId: int("contactId")
      .notNull()
      .references(() => contacts.id),
  ...
  table => [
    index("landing_page_sms_consents_contact_idx").on(
      table.contactId,
      table.consentedAt
    ),
  ]
...
export const landingPageRevisions = mysqlTable(
  ...
  table => [
    index("landing_page_revisions_page_created_idx").on(
      table.landingPageId,
      table.createdAt
    ),
    uniqueIndex("landing_page_revisions_page_revision_unique").on(
      table.landingPageId,
...
export const landingPageRedirects = mysqlTable(
  "landing_page_redirects",
  {
    ...
    status: mysqlEnum("status", ["active", "disabled", "archived"])
      .default("active")
      .notNull(),
    redirectType: mysqlEnum("redirectType", ["permanent", "temporary"])
      .default("permanent")
      .notNull(),
    preserveQueryParams: boolean("preserveQueryParams").default(true).notNull(),
    clickCount: int("clickCount").default(0).notNull(),
  ...
  table => [
    index("landing_page_redirects_status_updated_idx").on(
      table.status,
      table.updatedAt
    ),
  ]
...
export const dashboardSettings = mysqlTable(
  ...
  table => ({
    goalYearUnq: uniqueIndex("dashboard_settings_goal_year_unq").on(
      table.goalYear
    ),
  })
);
...
export const dashboardAlertReviews = mysqlTable(
  "dashboard_alert_reviews",
  {
    id: int("id").autoincrement().primaryKey(),
    userId: int("userId")
      .notNull()
  ...
  table => ({
    userAlertUnq: uniqueIndex("dashboard_alert_reviews_user_alert_unq").on(
      table.userId,
      table.alertKey
    ),
    userStatusIdx: index("dashboard_alert_reviews_user_status_idx").on(
      table.userId,
      table.status,
      table.snoozedUntil
    ),
...
export type InsertDashboardAlertReview =
  typeof dashboardAlertReviews.$inferInsert;
...
export const analyticsInsightCaches = mysqlTable(
  "analytics_insight_caches",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    ownerUserId: int("ownerUserId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    viewerRole: mysqlEnum("viewerRole", [
      "admin",
      "agent",
      "isa",
      "agent_support",
    ...
    filters: json("filters").$type<Record<string, unknown>>().notNull(),
    insightPayload: json("insightPayload")
      .$type<Record<string, unknown>>()
      .notNull(),
    facts: json("facts").$type<Record<string, unknown>>().notNull(),
    status: mysqlEnum("status", ["refreshing", "ready", "failed"])
      .notNull()
      .default("refreshing"),
    refreshReason: mysqlEnum("refreshReason", [
      "automatic",
  ...
  table => ({
    expiresAtIdx: index("analytics_insight_caches_expiresAt_idx").on(
      table.expiresAt
    ),
...
export type InsertAnalyticsInsightCache =
  typeof analyticsInsightCaches.$inferInsert;
...
export const usCounties = mysqlTable("us_counties", {
  ...
  stateCode: varchar("stateCode", { length: 2 })
    .notNull()
    .references(() => usStates.code, { onDelete: "cascade" }),
...
export const marketCounties = mysqlTable("market_counties", {
  ...
  countyId: int("countyId")
    .notNull()
    .references(() => usCounties.id, { onDelete: "cascade" }),
});
...
export const customReports = mysqlTable(
  ...
  table => [index("custom_reports_created_by_idx").on(table.createdById)]
);
...
export const adminPermissions = mysqlTable("admin_permissions", {
  ...
  canManageReferralAgents: boolean("canManageReferralAgents")
    .default(true)
    .notNull(),
  canEditReferralSplits: boolean("canEditReferralSplits")
    .default(true)
    .notNull(),
  canViewReferralFinancials: boolean("canViewReferralFinancials")
    .default(true)
    .notNull(),
  canUpdateReferralPayments: boolean("canUpdateReferralPayments")
    .default(true)
    .notNull(),
  canManageReferralAgreements: boolean("canManageReferralAgreements")
    .default(true)
    .notNull(),
  canEditHistoricalReferrals: boolean("canEditHistoricalReferrals")
    .default(true)
    .notNull(),
  // Pulse
  ...
  canViewPulseSettings: boolean("canViewPulseSettings")
    .default(false)
    .notNull(),
  // Operations
  ...
  canViewCoachFeedback: boolean("canViewCoachFeedback")
    .default(false)
    .notNull(),
  canViewLeadershipDashboard: boolean("canViewLeadershipDashboard")
    .default(true)
    .notNull(),
  ...
  canViewMarketingAdmin: boolean("canViewMarketingAdmin")
    .default(true)
    .notNull(),
  // Admin navigation: directory and vendor management are separately visible.
  canViewAgentDirectory: boolean("canViewAgentDirectory")
    .default(true)
    .notNull(),
  ...
  canCreateLandingPages: boolean("canCreateLandingPages")
    .default(false)
    .notNull(),
  canEditLandingPages: boolean("canEditLandingPages").default(false).notNull(),
  canPublishLandingPages: boolean("canPublishLandingPages")
    .default(false)
    .notNull(),
  ...
  canViewMarketingTextInbox: boolean("canViewMarketingTextInbox")
    .default(true)
    .notNull(),
  // Passwords
  ...
  canViewSuperPermissions: boolean("canViewSuperPermissions")
    .default(false)
    .notNull(),
...
export const eventObligations = mysqlTable(
  ...
  table => [
    ...
    index("event_obligations_sponsor_idx").on(table.sponsorId),
    index("event_obligations_ask_idx").on(table.sponsorAskId),
  ]
);
...
export const resendInboxThreads = mysqlTable(
  "resend_inbox_threads",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    speedToLeadExcludedAt: timestamp("speedToLeadExcludedAt"),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  },
  table => [
    index("resend_inbox_threads_active_idx").on(
      table.archivedAt,
      table.lastIncomingAt
    ),
    index("resend_inbox_threads_participant_idx").on(
...
export const resendInboxMessages = mysqlTable(
  "resend_inbox_messages",
  {
    id: int("id").autoincrement().primaryKey(),
    threadId: int("threadId")
      .notNull()
      .references(() => resendInboxThreads.id, { onDelete: "cascade" }),
    ...
    >(),
    sentById: int("sentById").references(() => users.id, {
      onDelete: "set null",
    }),
  ...
  table => [
    index("resend_inbox_messages_thread_idx").on(
      table.threadId,
      table.receivedAt
    ),
...
export const resendInboxThreadReads = mysqlTable(
  "resend_inbox_thread_reads",
  {
    id: int("id").autoincrement().primaryKey(),
    threadId: int("threadId")
      .notNull()
  ...
  table => [
    uniqueIndex("resend_inbox_thread_reads_user_thread_unq").on(
      table.threadId,
      table.userId
    ),
    index("resend_inbox_thread_reads_user_idx").on(
      table.userId,
      table.markedUnread
    ),
  ]
...
export const marketingTextInboxThreads = mysqlTable(
  "marketing_text_inbox_threads",
  {
    id: int("id").autoincrement().primaryKey(),
    contactId: int("contactId")
      .notNull()
...
export type MarketingTextInboxThread =
  typeof marketingTextInboxThreads.$inferSelect;
...
export const agentIntroductionFollowUps = mysqlTable(
  "agent_introduction_follow_ups",
  {
    id: int("id").autoincrement().primaryKey(),
    contactId: int("contactId")
      .notNull()
  ...
  table => [
    index("agent_intro_follow_ups_status_due_idx").on(
      table.status,
      table.dueAt
    ),
    index("agent_intro_follow_ups_contact_idx").on(
      table.contactId,
      table.createdAt
    ),
    index("agent_intro_follow_ups_connection_idx").on(
      table.connectionId,
      table.createdAt
    ),
  ]
);
export type AgentIntroductionFollowUp =
  typeof agentIntroductionFollowUps.$inferSelect;
...
export const emailBehaviors = mysqlTable(
  ...
  table => [
    ...
    uniqueIndex("email_behaviors_source_external_unique").on(
      table.source,
      table.externalId
    ),
  ]
...
export const emailBehaviorsUnmatched = mysqlTable(
  ...
  table => [
    ...
    uniqueIndex("email_behaviors_unmatched_source_external_unique").on(
      table.source,
      table.externalId
    ),
  ]
);
export type EmailBehaviorUnmatched =
  typeof emailBehaviorsUnmatched.$inferSelect;
export type InsertEmailBehaviorUnmatched =
  typeof emailBehaviorsUnmatched.$inferInsert;
...
export const emailBehaviorsSyncState = mysqlTable(
  "email_behaviors_sync_state",
  {
    id: int("id").autoincrement().primaryKey(),
...
export type EmailBehaviorsSyncState =
  typeof emailBehaviorsSyncState.$inferSelect;
...
export const listingDocuments = mysqlTable("listing_documents", {
  ...
  uploadedBy: int("uploadedBy")
    .notNull()
    .references(() => users.id),
  label: mysqlEnum("label_listing_doc", [
    "appraisal",
    "listing_agreement",
    "inspection",
    "disclosure",
...
export const aircallIsaAssignments = mysqlTable(
  ...
  table => [
    uniqueIndex("aircall_isa_assignments_savvy_user_unique").on(
      table.savvyUserId
    ),
    uniqueIndex("aircall_isa_assignments_aircall_user_unique").on(
      table.aircallUserId
    ),
    uniqueIndex("aircall_isa_assignments_aircall_number_unique").on(
      table.aircallNumberId
    ),
  ]
);
export type AircallIsaAssignment = typeof aircallIsaAssignments.$inferSelect;
export type InsertAircallIsaAssignment =
  typeof aircallIsaAssignments.$inferInsert;
...
export const aircallMessages = mysqlTable(
  ...
  table => [
    uniqueIndex("aircall_messages_aircall_message_unique").on(
      table.aircallMessageId
    ),
    index("aircall_messages_contact_sent_idx").on(
      table.contactId,
      table.sentAt
    ),
    index("aircall_messages_isa_sent_idx").on(table.savvyUserId, table.sentAt),
    index("aircall_messages_number_sent_idx").on(
      table.aircallNumberId,
      table.sentAt
    ),
    index("aircall_messages_group_conversation_idx").on(
      table.groupConversationId
    ),
    index("aircall_messages_number_inbound_read_idx").on(
      table.aircallNumberId,
      table.direction,
      table.readAt
    ),
...
export const aircallCalls = mysqlTable(
  "aircall_calls",
  {
    ...
    recordingRecoveryAttempts: int("recordingRecoveryAttempts")
      .notNull()
      .default(0),
    recordingRecoveryLastAttemptAt: timestamp("recordingRecoveryLastAttemptAt"),
    recordingRecoveryLastError: varchar("recordingRecoveryLastError", {
      length: 512,
    }),
    ...
    transcriptionRecoveryAttempts: int("transcriptionRecoveryAttempts")
      .notNull()
      .default(0),
    transcriptionRecoveryLastAttemptAt: timestamp(
      "transcriptionRecoveryLastAttemptAt"
    ),
    transcriptionRecoveryNextAttemptAt: timestamp(
      "transcriptionRecoveryNextAttemptAt"
    ),
    transcriptionRecoveryLastError: varchar("transcriptionRecoveryLastError", {
      length: 512,
    }),
    // Timestamps
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  ...
  table => [
    ...
    index("aircall_calls_recording_recovery_idx").on(
      table.recordingRecoveryAttempts,
      table.recordingRecoveryLastAttemptAt
    ),
    index("aircall_calls_transcription_recovery_idx").on(
      table.transcriptionRecoveryNextAttemptAt,
      table.startedAt
    ),
  ]
);
...
export type InsertAircallUnmatchedCall =
  typeof aircallUnmatchedCalls.$inferInsert;
...
export const aircallWebhookEvents = mysqlTable(
  ...
  table => [
    ...
    index("aircall_webhook_events_status_next_attempt_idx").on(
      table.status,
      table.nextAttemptAt
    ),
    index("aircall_webhook_events_call_event_idx").on(
      table.aircallCallId,
      table.eventType
    ),
  ]
...
export type AircallIntegrationState =
  typeof aircallIntegrationState.$inferSelect;
...
export const coachingProfiles = mysqlTable("coaching_profiles", {
  ...
  performanceStatus: mysqlEnum("performanceStatus", [
    "Launch",
    "Red",
    "Yellow",
    "Green",
  ...
  marketProtectionStatus: mysqlEnum("marketProtectionStatus", [
    "Protected",
    "Conditional",
    "Open for Additional Coverage",
    "Recruiting Active",
    "Exit Pending",
    "Unassigned",
    "Leadership Review",
  ]).default("Protected"),
  retentionRiskStatus: mysqlEnum("retentionRiskStatus", [
    "Low",
    "Watch",
    "Elevated",
    "Critical",
  ]).default("Low"),
  currentPrimaryDiagnosis: mysqlEnum("currentPrimaryDiagnosis", [
    "Commitment",
    "Capability",
    "Cadence",
    "Capacity",
  ]),
  secondaryDiagnosis: mysqlEnum("secondaryDiagnosis", [
    "Commitment",
    "Capability",
    "Cadence",
    "Capacity",
  ...
  coachingSetupRequired: boolean("coachingSetupRequired")
    .default(true)
    .notNull(),
  launchStartDate: timestamp("launchStartDate"),
  launchHealthStatus: mysqlEnum("launchHealthStatus", [
    "On Track",
    "At Risk",
    "Critical",
  ]).default("On Track"),
...
export const coachingSessions = mysqlTable(
  "coaching_sessions",
  {
    ...
    // AI processing
    aiSummary: text("aiSummary"),
    aiProcessingStatus: mysqlEnum("aiProcessingStatus", [
      "None",
      "Pending",
      "Processing",
      "Completed",
    ...
    aiNextCoachSuggestion: int("aiNextCoachSuggestion").references(
      () => users.id
    ),
    // Diagnosis
    primaryDiagnosis: mysqlEnum("primaryDiagnosis", [
      "Commitment",
      "Capability",
      "Cadence",
      "Capacity",
  ...
  table => [
    ...
    index("coaching_sessions_coach_idx").on(
...
export const coachingFeedbackInvitations = mysqlTable(
  "coaching_feedback_invitations",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  table => [
    index("coaching_feedback_invitation_due_idx").on(
      table.sentAt,
      table.submittedAt
    ),
  ]
);
export type CoachingFeedbackInvitation =
  typeof coachingFeedbackInvitations.$inferSelect;
export type InsertCoachingFeedbackInvitation =
  typeof coachingFeedbackInvitations.$inferInsert;
...
export const coachingFeedbackResponses = mysqlTable(
  "coaching_feedback_responses",
  {
    id: int("id").autoincrement().primaryKey(),
    coachId: int("coachId")
      .notNull()
  ...
  table => [
    index("coaching_feedback_response_coach_week_idx").on(
      table.coachId,
      table.sessionWeekStart
    ),
...
export type CoachingFeedbackResponse =
  typeof coachingFeedbackResponses.$inferSelect;
export type InsertCoachingFeedbackResponse =
  typeof coachingFeedbackResponses.$inferInsert;
...
export const coachingFeedbackSettings = mysqlTable(
  "coaching_feedback_settings",
  {
    id: int("id").autoincrement().primaryKey(),
...
export type CoachingFeedbackSettings =
  typeof coachingFeedbackSettings.$inferSelect;

// Coaching commitments — action items from sessions.
export const coachingCommitments = mysqlTable(
  "coaching_commitments",
  {
    ...
    coachVerificationStatus: mysqlEnum("coachVerificationStatus", [
      "Pending",
      "Verified",
      "Rejected",
    ]).default("Pending"),
    visibilityLabel: mysqlEnum("visibilityLabel", [
      "Agent Visible",
      "Internal",
      "Leadership",
...
export const performanceResets = mysqlTable(
  "performance_resets",
  {
    id: int("id").autoincrement().primaryKey(),
    agentId: int("agentId")
...
export const performanceResetRequirements = mysqlTable(
  "performance_reset_requirements",
  {
    id: int("id").autoincrement().primaryKey(),
    resetId: int("resetId")
      .notNull()
      .references(() => performanceResets.id, { onDelete: "cascade" }),
    description: text("description").notNull(),
    status: mysqlEnum("status", ["Pending", "Met", "Missed"])
      .default("Pending")
      .notNull(),
    sortOrder: int("sortOrder").default(0).notNull(),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
...
export type PerformanceResetRequirement =
  typeof performanceResetRequirements.$inferSelect;
export type InsertPerformanceResetRequirement =
  typeof performanceResetRequirements.$inferInsert;

// Scheduled check-in milestones for a reset plan.
export const performanceResetCheckpoints = mysqlTable(
  "performance_reset_checkpoints",
  {
    id: int("id").autoincrement().primaryKey(),
    resetId: int("resetId")
      .notNull()
    ...
    status: mysqlEnum("status", ["Pending", "Completed", "Missed"])
      .default("Pending")
      .notNull(),
    notes: text("notes"),
...
export type PerformanceResetCheckpoint =
  typeof performanceResetCheckpoints.$inferSelect;
export type InsertPerformanceResetCheckpoint =
  typeof performanceResetCheckpoints.$inferInsert;

// Capacity escalations — operational issues blocking production.
export const capacityEscalations = mysqlTable(
  "capacity_escalations",
  {
    ...
    relatedSessionId: int("relatedSessionId").references(
      () => coachingSessions.id
    ),
...
export type CoachOutRecommendation =
  typeof coachOutRecommendations.$inferSelect;
export type InsertCoachOutRecommendation =
  typeof coachOutRecommendations.$inferInsert;

// Personality and work-style assessments (DISC, Predictive Index, etc.).
export const coachingAssessments = mysqlTable(
  ...
  table => [index("coaching_assessments_agent_idx").on(table.agentId)]
);
...
export const coachingHistorySnapshots = mysqlTable(
  "coaching_history_snapshots",
  {
    id: int("id").autoincrement().primaryKey(),
    agentId: int("agentId")
      .notNull()
    ...
    trailing90DayVolume: decimal("trailing90DayVolume", {
      precision: 15,
      scale: 2,
    }),
    underContractUnits: int("underContractUnits"),
    underContractVolume: decimal("underContractVolume", {
      precision: 15,
      scale: 2,
    }),
  ...
  table => [
    index("coaching_history_snapshots_agent_date_idx").on(
      table.agentId,
      table.snapshotDate
    ),
  ]
...
export type CoachingHistorySnapshot =
  typeof coachingHistorySnapshots.$inferSelect;
export type InsertCoachingHistorySnapshot =
  typeof coachingHistorySnapshots.$inferInsert;
...
// ─── Pro-formas ──────────────────────────────────────────────────────────────
export const proformas = mysqlTable(
  "proformas",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    status: mysqlEnum("status", ["draft", "final", "archived"]).default(
      "draft"
    ),
...
export const contactRelationships = mysqlTable(
  "contact_relationships",
  {
    ...
    sourcePairId: int("sourcePairId").references(
      () => duplicateContactPairs.id,
      { onDelete: "set null" }
    ),
  ...
  table => ({
    ...
    relatedIdx: index("idx_contact_relationships_related").on(
      table.relatedContactId
    ),
  })
);
export type ContactRelationship = typeof contactRelationships.$inferSelect;
export type InsertContactRelationship =
...
export const passwordListShares = mysqlTable(
  "password_list_shares",
  {
    id: int("id").autoincrement().primaryKey(),
    listId: int("listId")
      .notNull()
      .references(() => passwordLists.id, { onDelete: "cascade" }),
    userId: int("userId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
  ...
  table => ({
    ...
    listUserUnique: uniqueIndex("password_list_shares_list_user_unique").on(
      table.listId,
      table.userId
    ),
  })
...
export const passwordEntries = mysqlTable(
  "password_entries",
  {
    id: int("id").autoincrement().primaryKey(),
    listId: int("listId")
...
export const referralStatusOptions = mysqlTable("referral_status_options", {
  ...
  category: mysqlEnum("category", ["active", "closed", "lost", "on_hold"])
    .default("active")
    .notNull(),
  sortOrder: int("sortOrder").default(0).notNull(),
  isActive: boolean("isActive").default(true).notNull(),
...
export const referralAgents = mysqlTable(
  "referral_agents",
  {
    id: int("id").autoincrement().primaryKey(),
    name: varchar("name", { length: 255 }).notNull(),
    ...
    defaultSavvyReferralPct: decimal("defaultSavvyReferralPct", {
      precision: 5,
      scale: 2,
    }).default("25.00"),
    licenseNumber: varchar("licenseNumber", { length: 128 }),
    licenseState: varchar("licenseState", { length: 64 }),
    relationshipOwnerId: int("relationshipOwnerId").references(() => users.id, {
      onDelete: "set null",
    }),
    notes: text("notes"),
    isActive: boolean("isActive").default(true).notNull(),
...
export const referralAgentCoverage = mysqlTable(
  "referral_agent_coverage",
  {
    id: int("id").autoincrement().primaryKey(),
    referralAgentId: int("referralAgentId")
      .notNull()
      .references(() => referralAgents.id, { onDelete: "cascade" }),
    state: varchar("state", { length: 64 }),
...
export const referralAgreements = mysqlTable(
  "referral_agreements",
  {
    id: int("id").autoincrement().primaryKey(),
    referralAgentId: int("referralAgentId")
  ...
  table => [
    index("referral_agreements_agent_idx").on(
      table.referralAgentId,
      table.status
    ),
...
export const referrals = mysqlTable(
  "referrals",
  {
    ...
    agreementId: int("agreementId").references(() => referralAgreements.id, {
      onDelete: "set null",
    }),
    ...
    referralType: mysqlEnum("referralType", [
      "buyer",
      "seller",
      "buyer_seller",
      "other",
    ]).notNull(),
    statusKey: varchar("statusKey", { length: 96 })
      .notNull()
      .default("referral_sent"),
    statusCategory: mysqlEnum("statusCategory", [
      "active",
      "closed",
      "lost",
      "on_hold",
    ...
    savvyReferralPct: decimal("savvyReferralPct", {
      precision: 5,
      scale: 2,
    }).notNull(),
...
export const referralEvents = mysqlTable(
  ...
  table => [
    index("referral_events_referral_date_idx").on(
      table.referralId,
      table.occurredAt
    ),
  ]
...
export const referralTransactionLinks = mysqlTable(
  "referral_transaction_links",
  {
    id: int("id").autoincrement().primaryKey(),
    referralId: int("referralId")
      .notNull()
  ...
  table => [
    uniqueIndex("referral_transaction_link_unique").on(
      table.referralId,
      table.transactionId
    ),
...
export type ReferralTransactionLink =
  typeof referralTransactionLinks.$inferSelect;

export const referralListingLinks = mysqlTable(
  "referral_listing_links",
  {
    id: int("id").autoincrement().primaryKey(),
    referralId: int("referralId")
      .notNull()
  ...
  table => [
    uniqueIndex("referral_listing_link_unique").on(
      table.referralId,
      table.listingId
    ),
...
export const referralPayments = mysqlTable(
  "referral_payments",
  {
    id: int("id").autoincrement().primaryKey(),
    referralId: int("referralId")
    ...
    referralFeeOwed: decimal("referralFeeOwed", { precision: 12, scale: 2 })
      .notNull()
      .default("0.00"),
    outsideAgentPortion: decimal("outsideAgentPortion", {
      precision: 12,
      scale: 2,
    }),
    ...
    notes: text("notes"),
    markedPaidById: int("markedPaidById").references(() => users.id, {
      onDelete: "set null",
    }),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  ...
  table => [
    index("referral_payments_referral_idx").on(
      table.referralId,
      table.paymentStatus
    ),
    index("referral_payments_transaction_idx").on(table.transactionId),
    index("referral_payments_status_due_idx").on(
      table.paymentStatus,
      table.dueAt
    ),
  ]
...
export const referralDocuments = mysqlTable(
  ...
  {
    id: int("id").autoincrement().primaryKey(),
    referralAgentId: int("referralAgentId").references(
      () => referralAgents.id,
      { onDelete: "cascade" }
    ),
    referralId: int("referralId").references(() => referrals.id, {
    ...
    paymentId: int("paymentId").references(() => referralPayments.id, {
      onDelete: "cascade",
    }),
    name: varchar("name", { length: 512 }).notNull(),
    fileKey: varchar("fileKey", { length: 1024 }).notNull(),
...
export const referralReassignments = mysqlTable(
  "referral_reassignments",
  {
    id: int("id").autoincrement().primaryKey(),
    priorReferralId: int("priorReferralId")
      .notNull()
      .references(() => referrals.id, { onDelete: "cascade" }),
    newReferralId: int("newReferralId")
      .notNull()
      .references(() => referrals.id, { onDelete: "cascade" }),
    previousReferralAgentId: int("previousReferralAgentId")
      .notNull()
      .references(() => referralAgents.id),
    newReferralAgentId: int("newReferralAgentId")
      .notNull()
      .references(() => referralAgents.id),
    reason: text("reason").notNull(),
    reassignedById: int("reassignedById").references(() => users.id, {
      onDelete: "set null",
    }),
...
export const pulseProfiles = mysqlTable("pulse_profiles", {
  userId: int("userId")
    .primaryKey()
    .references(() => users.id, { onDelete: "cascade" }),
  platformRole: mysqlEnum("platformRole", ["super_admin", "admin", "member"])
    .default("member")
    .notNull(),
  timezone: varchar("timezone", { length: 64 })
    .default("America/New_York")
...
export const pulseMeetings = mysqlTable(
  "pulse_meetings",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    name: varchar("name", { length: 255 }).notNull(),
    ...
    facilitatorId: int("facilitatorId").references(() => users.id, {
      onDelete: "set null",
    }),
    ...
    timezone: varchar("timezone", { length: 64 })
      .default("America/New_York")
      .notNull(),
    reminderDay: mysqlEnum("reminderDay", [
      "monday",
      "tuesday",
      "wednesday",
      "thursday",
    ...
    segueResetDay: mysqlEnum("segueResetDay", [
      "monday",
      "tuesday",
      "wednesday",
      "thursday",
    ...
    headlinesResetDay: mysqlEnum("headlinesResetDay", [
      "monday",
      "tuesday",
      "wednesday",
      "thursday",
    ...
    notificationConfig:
      json("notificationConfig").$type<
        Record<string, { email?: boolean; inApp?: boolean; enabled?: boolean }>
      >(),
    sectionsEnabled: json("sectionsEnabled")
      .$type<Record<string, boolean>>()
      .notNull(),
    sectionOrder: json("sectionOrder").$type<string[]>().notNull(),
    sectionDurations: json("sectionDurations")
    ...
    scorecardDeadlineDay: mysqlEnum("scorecardDeadlineDay", [
      "monday",
      "tuesday",
      "wednesday",
      "thursday",
...
export const pulseMeetingMembers = mysqlTable(
  "pulse_meeting_members",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
    ...
    personId: int("personId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    meetingRole: mysqlEnum("meetingRole", ["owner", "administrator", "member"])
      .default("member")
      .notNull(),
  ...
  table => [
    uniqueIndex("pulse_meeting_member_unique").on(
      table.meetingId,
      table.personId
    ),
    index("pulse_membership_person_visible_idx").on(
      table.personId,
      table.removedAt,
      table.deletedAt
    ),
    index("pulse_membership_meeting_visible_idx").on(
      table.meetingId,
      table.removedAt,
      table.deletedAt
    ),
...
export const pulseMeetingSessions = mysqlTable(
  "pulse_meeting_sessions",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
      .references(() => pulseMeetings.id, { onDelete: "cascade" }),
    scheduledFor: timestamp("scheduledFor").notNull(),
    status: mysqlEnum("status", ["running", "paused", "closed"])
      .default("running")
      .notNull(),
    activeStep: varchar("activeStep", { length: 64 })
      .notNull()
      .default("segue"),
  ...
  table => [
    index("pulse_session_meeting_status_idx").on(
      table.meetingId,
      table.status,
      table.startedAt
    ),
    index("pulse_session_meeting_date_idx").on(
      table.meetingId,
      table.scheduledFor
    ),
  ]
...
export const pulseSessionRatings = mysqlTable(
  "pulse_session_ratings",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
...
export const pulseSessionReports = mysqlTable(
  "pulse_session_reports",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    sessionId: varchar("sessionId", { length: 36 })
      .notNull()
  ...
  table => [
    ...
    index("pulse_session_report_meeting_idx").on(
      table.meetingId,
      table.createdAt
    ),
  ]
...
export const pulseWorkItems = mysqlTable(
  "pulse_work_items",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    ...
    resolvedInSessionId: varchar("resolvedInSessionId", {
      length: 36,
    }).references(() => pulseMeetingSessions.id, { onDelete: "set null" }),
    ownerPersonId: int("ownerPersonId").references(() => users.id),
    isPersonal: boolean("isPersonal").generatedAlwaysAs(
      sql`case when ${sql.identifier("meetingId")} is null then true else false end`
    ),
    ...
    savvyosMetricId: int("savvyosMetricId").references(
      () => rrScorecardMetrics.id,
      { onDelete: "set null" }
    ),
    sortOrder: int("sortOrder").default(0).notNull(),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  ...
  table => [
    ...
    index("pulse_work_items_source_session_idx").on(
      table.sourceSessionId,
      table.deletedAt
    ),
    index("pulse_work_items_resolved_session_idx").on(
      table.resolvedInSessionId,
      table.deletedAt
    ),
    index("pulse_work_items_owner_idx").on(
      table.ownerPersonId,
      table.deletedAt,
      table.sortOrder
    ),
...
export const pulseRockRaciAssignments = mysqlTable(
  ...
  table => [
    uniqueIndex("pulse_rock_raci_unique").on(
      table.workItemId,
      table.personId,
      table.role
    ),
...
export type PulseRockRaciAssignment =
  typeof pulseRockRaciAssignments.$inferSelect;

export const pulseWorkItemMoves = mysqlTable(
  "pulse_work_item_moves",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    workItemId: varchar("workItemId", { length: 36 })
      .notNull()
      .references(() => pulseWorkItems.id, { onDelete: "cascade" }),
    fromMeetingId: varchar("fromMeetingId", { length: 36 }).references(
      () => pulseMeetings.id
    ),
    toMeetingId: varchar("toMeetingId", { length: 36 }).references(
      () => pulseMeetings.id
    ),
    movedById: int("movedById")
      .notNull()
      .references(() => users.id),
    reason: text("reason"),
...
export const pulseRockMilestones = mysqlTable(
  "pulse_rock_milestones",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    workItemId: varchar("workItemId", { length: 36 })
      .notNull()
  ...
  table => [
    index("pulse_rock_milestones_item_idx").on(
      table.workItemId,
      table.deletedAt,
      table.sortOrder
    ),
...
export const pulseIssueResultingTodos = mysqlTable(
  "pulse_issue_resulting_todos",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    issueWorkItemId: varchar("issueWorkItemId", { length: 36 })
      .notNull()
      .references(() => pulseWorkItems.id, { onDelete: "cascade" }),
    todoWorkItemId: varchar("todoWorkItemId", { length: 36 })
      .notNull()
      .references(() => pulseWorkItems.id, { onDelete: "cascade" }),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  },
  table => [
    uniqueIndex("pulse_issue_resulting_todos_unique").on(
      table.issueWorkItemId,
      table.todoWorkItemId
    ),
...
export const pulseWorkItemComments = mysqlTable(
  ...
  table => [
    index("pulse_work_item_comments_item_idx").on(
      table.workItemId,
      table.deletedAt,
      table.createdAt
    ),
...
export const pulseWorkItemCommentMentions = mysqlTable(
  "pulse_work_item_comment_mentions",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    commentId: varchar("commentId", { length: 36 })
      .notNull()
      .references(() => pulseWorkItemComments.id, { onDelete: "cascade" }),
    mentionedPersonId: int("mentionedPersonId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  },
  table => [
    uniqueIndex("pulse_comment_mention_unique").on(
      table.commentId,
      table.mentionedPersonId
    ),
    index("pulse_comment_mention_person_idx").on(
      table.mentionedPersonId,
      table.createdAt
    ),
  ]
);
export type PulseWorkItemCommentMention =
  typeof pulseWorkItemCommentMentions.$inferSelect;
...
export const pulseWorkItemNotifications = mysqlTable(
  "pulse_work_item_notifications",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    recipientId: int("recipientId")
      .notNull()
    ...
    workItemId: varchar("workItemId", { length: 36 })
      .notNull()
      .references(() => pulseWorkItems.id, { onDelete: "cascade" }),
    commentId: varchar("commentId", { length: 36 }).references(
      () => pulseWorkItemComments.id,
      { onDelete: "cascade" }
    ),
  ...
  table => [
    index("pulse_notification_recipient_action_idx").on(
      table.recipientId,
      table.actionedAt,
      table.createdAt
    ),
    index("pulse_notification_item_idx").on(
      table.workItemId,
      table.notificationType
    ),
  ]
);
export type PulseWorkItemNotification =
  typeof pulseWorkItemNotifications.$inferSelect;

export const pulseWorkItemStatusNotes = mysqlTable(
  "pulse_work_item_status_notes",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    workItemId: varchar("workItemId", { length: 36 })
      .notNull()
  ...
  table => [
    index("pulse_work_item_status_notes_item_idx").on(
      table.workItemId,
      table.createdAt
    ),
  ]
);
export type PulseWorkItemStatusNote =
  typeof pulseWorkItemStatusNotes.$inferSelect;
...
export const pulseActivityLog = mysqlTable(
  "pulse_activity_log",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
  ...
  table => [
    index("pulse_activity_entity_idx").on(
      table.entityType,
      table.entityId,
      table.createdAt
    ),
...
export const pulseGlossary = mysqlTable(
  "pulse_glossary",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
  ...
  table => [uniqueIndex("pulse_glossary_term_unique").on(table.term)]
);
...
export const pulseMeetingScorecardMetrics = mysqlTable(
  "meeting_scorecard_metrics",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
  ...
  table => [
    uniqueIndex("meeting_scorecard_metric_unique").on(
      table.meetingId,
      table.savvyosMetricId
    ),
    index("meeting_scorecard_metric_meeting_idx").on(
      table.meetingId,
      table.sortOrder
    ),
  ]
);
export type PulseMeetingScorecardMetric =
  typeof pulseMeetingScorecardMetrics.$inferSelect;

// Prompt 8: Pulse places SavvyOS company goals in a meeting but owns neither
// goal definitions nor values.
export const pulseMeetingGoals = mysqlTable(
  "meeting_goals",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
      .references(() => pulseMeetings.id, { onDelete: "cascade" }),
    savvyosGoalId: int("savvyosGoalId").references(() => companyGoals.id, {
      onDelete: "set null",
    }),
    sortOrder: int("sortOrder").default(0).notNull(),
  },
...
export const pulseMeetingRocks = mysqlTable(
  "meeting_rocks",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
...
export const pulseMeetingTodos = mysqlTable(
  "pulse_meeting_todos",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
  ...
  table => [
    uniqueIndex("pulse_meeting_todo_unique").on(
      table.meetingId,
      table.workItemId
    ),
    index("pulse_meeting_todo_meeting_idx").on(
      table.meetingId,
      table.sortOrder
    ),
  ]
...
export const aiObservations = mysqlTable(
  "ai_observations",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    savvyosMetricId: int("savvyosMetricId")
      .notNull()
      .references(() => rrScorecardMetrics.id, { onDelete: "cascade" }),
    observation: text("observation").notNull(),
    triggerRule: varchar("triggerRule", { length: 128 }).notNull(),
    generatedAt: timestamp("generatedAt").defaultNow().notNull(),
    raisedAsIssueId: varchar("raisedAsIssueId", { length: 36 }).references(
      () => pulseWorkItems.id,
      { onDelete: "set null" }
    ),
    dismissedById: int("dismissedById").references(() => users.id, {
      onDelete: "set null",
    }),
    dismissedAt: timestamp("dismissedAt"),
  ...
  table => [
    index("ai_observation_metric_generated_idx").on(
      table.savvyosMetricId,
      table.generatedAt
    ),
    index("ai_observation_open_idx").on(
      table.raisedAsIssueId,
      table.dismissedAt
    ),
  ]
);
...
export const pulseMeetingUpdates = mysqlTable(
  "pulse_meeting_updates",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
    ...
    sessionId: varchar("sessionId", { length: 36 }).references(
      () => pulseMeetingSessions.id,
      { onDelete: "set null" }
    ),
    weekOf: date("weekOf"),
  ...
  table => [
    index("pulse_meeting_updates_meeting_idx").on(
      table.meetingId,
      table.updateType,
      table.deletedAt,
      table.createdAt
    ),
    index("pulse_meeting_updates_session_idx").on(
      table.sessionId,
      table.deletedAt
    ),
  ]
...
export const pulsePersonalInputs = mysqlTable(
  "pulse_personal_inputs",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    personId: int("personId")
      .notNull()
  ...
  table => [
    uniqueIndex("pulse_personal_input_week_unique").on(
      table.personId,
      table.meetingId,
      table.inputKey,
      table.weekOf
    ),
    index("pulse_personal_input_person_idx").on(
      table.personId,
      table.weekOf,
      table.deletedAt
    ),
...
export const pulseWeeklySubmissions = mysqlTable(
  "pulse_weekly_submissions",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
    ...
    weekOf: date("weekOf").notNull(),
    submittedAt: timestamp("submittedAt").defaultNow().notNull(),
    confirmationSummary: json("confirmationSummary").$type<
  ...
  table => [
    uniqueIndex("pulse_weekly_submission_unique").on(
      table.meetingId,
      table.personId,
      table.weekOf
    ),
    index("pulse_weekly_submission_person_idx").on(
      table.personId,
      table.weekOf,
      table.withdrawnAt
    ),
...
export const pulsePermissions = mysqlTable(
  "pulse_permissions",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    personId: int("personId")
  ...
  table => [
    ...
    index("pulse_permission_capability_idx").on(
      table.capability,
      table.allowed
    ),
  ]
...
export const pulseCascadeDestinations = mysqlTable(
  "pulse_cascade_destinations",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    cascadingMessageId: varchar("cascadingMessageId", { length: 36 })
      .notNull()
      .references(() => pulseCascadingMessages.id, { onDelete: "cascade" }),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
      .references(() => pulseMeetings.id),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  },
  table => [
    uniqueIndex("pulse_cascade_destination_unique").on(
      table.cascadingMessageId,
      table.meetingId
    ),
  ]
);
export type PulseCascadeDestination =
  typeof pulseCascadeDestinations.$inferSelect;

export const pulseCascadeRecipients = mysqlTable(
  "pulse_cascade_recipients",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    cascadingMessageId: varchar("cascadingMessageId", { length: 36 })
      .notNull()
    ...
    personId: int("personId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    viaMeetingId: varchar("viaMeetingId", { length: 36 })
      .notNull()
      .references(() => pulseMeetings.id),
  ...
  table => [
    uniqueIndex("pulse_cascade_recipient_unique").on(
      table.cascadingMessageId,
      table.personId,
      table.viaMeetingId
    ),
    index("pulse_cascade_recipient_person_idx").on(
      table.personId,
...
export const pulseNotifications = mysqlTable(
  "pulse_notifications",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    personId: int("personId")
  ...
  table => [
    index("pulse_notifications_person_action_idx").on(
      table.personId,
...
export const pulseNotificationPreferences = mysqlTable(
  "pulse_notification_preferences",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    personId: int("personId")
      .notNull()
  ...
  table => [
    uniqueIndex("pulse_notification_preference_person_template_unique").on(
      table.personId,
      table.templateKey
    ),
...
export type PulseNotificationPreference =
  typeof pulseNotificationPreferences.$inferSelect;

export const pulseCascadingMessages = mysqlTable(
  "pulse_cascading_messages",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    fromMeetingId: varchar("fromMeetingId", { length: 36 })
      .notNull()
      .references(() => pulseMeetings.id),
    toMeetingId: varchar("toMeetingId", { length: 36 })
      .notNull()
      .references(() => pulseMeetings.id),
    sessionId: varchar("sessionId", { length: 36 }).references(
      () => pulseMeetingSessions.id,
    ...
    deliveryStatus: mysqlEnum("deliveryStatus", ["draft", "published"])
      .default("published")
      .notNull(),
    publishedAt: timestamp("publishedAt"),
    body: text("body").notNull(),
    createdById: int("createdById")
      ...
      .references(() => users.id),
    acknowledgedAt: timestamp("acknowledgedAt"),
    acknowledgedById: int("acknowledgedById").references(() => users.id, {
      onDelete: "set null",
    }),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  ...
  table => [
    index("pulse_cascading_destination_idx").on(
      table.toMeetingId,
      table.deletedAt,
      table.createdAt
    ),
    index("pulse_cascading_session_status_idx").on(
      table.sessionId,
...
export const pulseMeetingsArchive = mysqlTable(
  "pulse_meetings_archive",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
...
export const pulseMeetingRuns = mysqlTable(
  "pulse_meeting_runs",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    meetingId: varchar("meetingId", { length: 36 })
      .notNull()
      .references(() => pulseMeetings.id, { onDelete: "cascade" }),
    status: mysqlEnum("status", ["running", "paused", "concluded"])
      .default("running")
      .notNull(),
  ...
  table => [
    index("pulse_meeting_run_active_idx").on(
      table.meetingId,
      table.status,
      table.startedAt
    ),
...
export const PULSE_MEETING_PRESETS: Record<
  "level_10" | "one_on_one" | "other",
  PulseSectionKey[]
> = {
  level_10: [
    "segue",
