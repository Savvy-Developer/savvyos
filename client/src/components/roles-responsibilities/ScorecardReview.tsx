...
function PeriodPerformanceBadge({ performance }: { performance?: any }) {
  ...
  const best = performance.kind === "best" || performance.kind === "tied_best";
  return <Badge className={best ? "bg-emerald-100 text-emerald-800 hover:bg-emerald-100" : "bg-rose-100 text-rose-800 hover:bg-rose-100"} title={`Compared with ${performance.comparedPeriods} reported periods`}>{performance.label}</Badge>;
}
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
      ...
      <div className="space-y-5 text-sm">
        ...
        <section><p className="font-medium">Result history</p><div className="mt-2 overflow-x-auto rounded-md border"><table className="w-full min-w-[680px] text-left text-sm"><thead className="bg-muted/50 text-muted-foreground"><tr><th className="px-3 py-2 font-medium">Period / event</th><th className="px-3 py-2 font-medium">Result</th><th className="px-3 py-2 font-medium">Target at the time</th><th className="px-3 py-2 font-medium">Status</th><th className="px-3 py-2 font-medium">Period-to-date</th><th className="px-3 py-2 font-medium">Note</th></tr></thead><tbody>{(metric.valueHistory ?? []).length === 0 ? <tr><td className="px-3 py-4 text-muted-foreground" colSpan={6}>No historical results yet.</td></tr> : metric.valueHistory.map((value: any) => <tr key={value.id} className="border-t"><td className="px-3 py-2">{value.eventLabel || value.eventDate || `${value.periodStart}–${value.periodEnd}`}</td><td className="px-3 py-2 font-medium">{value.resultState === "no_eligible_activity" ? noEligibleActivityText(metric) : formatValue(metric, value.actual)}</td><td className="px-3 py-2">{value.target.comparisonRule === "within_range" ? `${formatValue(metric, value.target.targetMinimum)}–${formatValue(metric, value.target.targetMaximum)}` : value.target.targetValue == null ? "Unset" : formatValue(metric, value.target.targetValue)}</td><td className="px-3 py-2"><StatusBadge status={value.grade.status} /></td><td className="px-3 py-2"><PeriodPerformanceBadge performance={value.periodToDatePerformance} /></td><td className="max-w-xs px-3 py-2 text-muted-foreground">{value.note ?? "—"}</td></tr>)}</tbody></table></div></section>
