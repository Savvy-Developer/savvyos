    ...
    <DialogContent className="flex max-h-[90vh] w-[min(calc(100vw-2rem),40rem)] flex-col gap-3 overflow-hidden p-5 sm:max-w-[40rem]">
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
        <section className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1 sm:col-span-2"><Label>Name *</Label><Input value={form.name} onChange={(event) => update({ name: event.target.value })} placeholder="Eligible booking conversion rate" /></div>
          <div className="space-y-1 sm:col-span-2"><Label>Definition</Label><Textarea className="min-h-[72px]" value={form.definition} onChange={(event) => update({ definition: event.target.value })} placeholder="Optional: what counts, what does not, and which people or records are included." /></div>
          <div className="space-y-1 sm:col-span-2"><Label>How it&apos;s calculated *</Label>{automatic ? <div className="rounded-md border bg-muted/40 px-3 py-2 text-sm leading-6 text-muted-foreground">{autoDescription}</div> : <Textarea className="min-h-[72px]" value={form.calculationDescription} onChange={(event) => update({ calculationDescription: event.target.value })} placeholder="Describe exactly how this number is calculated." />}</div>
          <div className="space-y-1"><Label>Owner *</Label><Select value={form.ownerId} onValueChange={(value) => update({ ownerId: value })}><SelectTrigger><SelectValue placeholder="Choose a person" /></SelectTrigger><SelectContent>{metricOwners.map((owner) => <SelectItem key={owner.id} value={String(owner.id)}>{owner.name ?? owner.email}{owner.title ? ` · ${owner.title}` : ""}</SelectItem>)}</SelectContent></Select></div>
          <div className="space-y-1"><Label>R&amp;R *</Label>{responsibilities?.length ? <Select value={form.responsibilityId} onValueChange={(value) => update({ responsibilityId: value })}><SelectTrigger><SelectValue placeholder="Choose an R&R" /></SelectTrigger><SelectContent>{responsibilities.map((responsibility) => <SelectItem key={responsibility.id} value={String(responsibility.id)}>{responsibility.title}</SelectItem>)}</SelectContent></Select> : <Input value={metric?.responsibility?.title ?? "This R&R"} disabled />}</div>
          <div className="space-y-1"><Label>Unit</Label><Select value={form.unit} onValueChange={(value) => { const totals = defaultTotalsMode(value); update({ unit: value, displayFormat: displayFormatForUnit(value), totalsMode: totals, ...applyTotals(totals, value) }); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{selectItems([["count", "Count"], ["dollars", "Dollars"], ["percentage", "Percentage"], ["hours", "Hours"], ["days", "Days"], ["score", "Score"]])}</SelectContent></Select></div>
          {!targetIsInformational && !targetIsRange && <div className="space-y-1"><Label>Target</Label><Input type="number" step="any" value={form.targetValue} onChange={(event) => update({ targetValue: event.target.value })} placeholder="Leave empty if not set" /></div>}
          <div className="space-y-1"><Label>Good is</Label><Select value={form.comparisonRule === "at_most" ? "at_most" : "at_least"} onValueChange={(value) => update({ comparisonRule: value, performanceDirection: value === "at_most" ? "lower" : "higher" })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{selectItems([["at_least", "Higher"], ["at_most", "Lower"]])}</SelectContent></Select></div>
          <div className="space-y-1"><Label>Warning at</Label><Input type="number" step="any" value={form.warningThreshold} onChange={(event) => update({ warningThreshold: event.target.value })} placeholder="Optional" /></div>
          <div className="space-y-1"><Label>Totals over time</Label><Select value={form.totalsMode ?? (form.isCumulative ? "cumulative" : "average")} onValueChange={(value) => update({ totalsMode: value, ...applyTotals(value, form.unit) })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{selectItems([["cumulative", "Cumulative"], ["average", "Average"]])}</SelectContent></Select><p className="text-xs text-muted-foreground">Resets annually.</p></div>
          {targetChanged && <div className="space-y-1 sm:col-span-2"><Label>Why did this change?</Label><Input value={form.targetChangeNote} onChange={(event) => update({ targetChangeNote: event.target.value })} placeholder="Required context is kept with the target history" /></div>}
          <div className="space-y-1 sm:col-span-2"><Label>Show in</Label>{l10Meetings.length ? <MultiSelect options={l10Meetings.map((meeting) => ({ value: meeting.id, label: meeting.name }))} value={form.l10MeetingIds} onValueChange={(value) => update({ l10MeetingIds: value })} placeholder="Choose L10s" searchPlaceholder="Search L10s…" emptyText="No L10s found." /> : <p className="text-sm text-muted-foreground">Create an L10 in Pulse Settings before assigning a measurable to weekly review.</p>}</div>
        </section>
