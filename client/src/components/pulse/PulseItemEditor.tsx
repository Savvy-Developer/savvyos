...
import { priorityBadgeClass, PulseIssueTimeframeBadge, PulseStatusBadge, statusBadgeClass } from "@/components/pulse/PulseWorkItemBadges";
...
function ItemEditorFields({ open, onOpenChange, workItemId, defaultType = "todo", defaultDestinationId, defaultAssigneeId, defaultDueDate: initialDueDate, defaultPriorityLevel, parentWorkItemId, sourceSessionId, inline = false, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    setStatus(item?.status ?? "not_started"); setIssueTimeframe(item?.issueTimeframe ?? "short_term"); setPendingAttachments([]);
  ...
  const submit = () => {
    ...
    if (type === "todo" && !dueDate) return toast.error("Choose a due date so this To-Do can be tracked.");
    if (!assigneeId) return toast.error("Choose an assignee.");
    save.mutate({ workItemId: workItemId ?? undefined, type, title: itemTitle.trim(), description: description || null, assigneeId, dueDate: type === "issue" ? defaultDueDate() : dueDate, priorityLevel: priorityLevel as any, status: status as any, issueTimeframe: type === "issue" ? issueTimeframe as any : null, statusNote: null, destinationId, sourceSessionId: sourceSessionId && destinationId === defaultDestinationId ? sourceSessionId : null, parentWorkItemId: parentWorkItemId ?? undefined, attachments: pendingAttachments });
  };
...
function PulseQuickWorkControls({ item, onChanged }: { item: any; onChanged: () => void }) {
  ...
  const noun = item.type === "issue" ? "Issue" : "To-Do";
  ...
  const quickUpdate = trpc.pulse.workItems.updateQuickWorkItemFields.useMutation({
  ...
  return <div ref={statusAnchor} className="order-last flex shrink-0 basis-full flex-wrap items-center gap-1 pt-1 pl-6 xl:order-none xl:basis-auto xl:pt-0 xl:pl-0" onClick={event => event.stopPropagation()}>
    ...
    <Dialog open={Boolean(nextStatus)} onOpenChange={open => { if (!open) { setNextStatus(null); setStatusNote(""); setBlockerPersonId(""); } }}><DialogContent><DialogHeader><DialogTitle>Save status update</DialogTitle><DialogDescription>{nextStatus === "completed" ? (item.type === "issue" ? "Before resolving this Issue, record the solve." : "Before completing this To-Do, define what done looks like.") : "A written update is required whenever status changes."}</DialogDescription></DialogHeader>{nextStatus === "blocked" && people.length ? <div className="space-y-2"><Label htmlFor={`pulse-quick-blocker-${item.id}`}>Who can unblock this {noun}?</Label><Select value={blockerPersonId} onValueChange={setBlockerPersonId}><SelectTrigger id={`pulse-quick-blocker-${item.id}`}><SelectValue placeholder="Choose the blocker" /></SelectTrigger><SelectContent>{people.map((person: any) => <SelectItem key={person.id} value={String(person.id)}>{formatName(person)}</SelectItem>)}</SelectContent></Select><p className="text-xs text-muted-foreground">This person receives a notification with the item and your update.</p></div> : null}<div className="space-y-2"><Label htmlFor={`pulse-quick-status-note-${item.id}`}>{nextStatus === "completed" ? (item.type === "issue" ? "What was the solve?" : "Definition of done") : "Describe this status update"}</Label><Textarea id={`pulse-quick-status-note-${item.id}`} autoFocus value={statusNote} onChange={event => setStatusNote(event.target.value)} className="min-h-24" placeholder={nextStatus === "completed" ? (item.type === "issue" ? "Describe the decision, resolution, or solve…" : "Describe the completed outcome, deliverable, or definition of done…") : "What changed, what is blocked, or what is moving forward?"} /></div><DialogFooter><Button variant="outline" onClick={() => { setNextStatus(null); setStatusNote(""); }}>Cancel</Button><Button disabled={!statusNote.trim() || (nextStatus === "blocked" && people.length > 0 && !blockerPersonId) || setStatus.isPending} onClick={() => nextStatus && setStatus.mutate({ workItemId: item.id, status: nextStatus as any, statusNote: statusNote.trim(), blockerPersonId: nextStatus === "blocked" && blockerPersonId ? Number(blockerPersonId) : null })}>Save update</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}

export function PulseInlineItemRow({ item, onChanged, defaultDestinationId, sourceSessionId, canReopen = true, showDestination = false }: { item: any; onChanged: () => void; defaultDestinationId?: string | null; sourceSessionId?: string | null; canReopen?: boolean; showDestination?: boolean }) {
  ...
  const [openSubTodosRequest, setOpenSubTodosRequest] = useState(0);
  // Only To-Dos can be parents. Issues remain standalone work and never expose sub-To-Do actions.
  const parentEligible = Boolean(item?.id) && item.type === "todo" && !item.parentWorkItemId;
  ...
  const openComments = () => { setExpanded(true); window.setTimeout(() => document.getElementById(`pulse-work-item-${item.id}-comments`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 80); };
  return <div className="overflow-hidden rounded-md border border-border bg-card"><div className="flex w-full flex-wrap items-center gap-1.5 px-2.5 py-1.5 text-left transition-colors hover:bg-muted/45"><button type="button" aria-label={completed ? "Completed. Open context to change status." : completeLabel} title={completed ? "Completed" : "Complete or resolve — add the required note"} onClick={requestCompletion} className={"flex h-5 w-5 shrink-0 items-center justify-center rounded-full border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 " + (completed ? "border-emerald-500 bg-emerald-500 text-white" : "border-muted-foreground hover:border-emerald-500 hover:bg-emerald-50 hover:text-emerald-700")}><span className="sr-only">{completed ? "Completed" : completeLabel}</span>{completed ? <Check className="h-3.5 w-3.5" /> : <Circle className="h-3.5 w-3.5" />}</button><button type="button" aria-expanded={expanded} onClick={() => setExpanded(value => !value)} className="flex min-w-[10rem] flex-1 items-center gap-2 text-left sm:min-w-[12rem]"><span className="min-w-0 flex-1">{item.type === "rock" ? <><span className={completed ? "block truncate text-sm font-medium text-muted-foreground line-through" : "block truncate text-sm font-medium"}>{item.title}</span><span className="block truncate text-xs text-muted-foreground">{item.assigneeName ?? "Unassigned"} · due {dateLabel(item.dueDate)}</span><span className="mt-0.5 flex flex-wrap items-center gap-1"><PulseStatusBadge value={item.status} compact /></span></> : <><span className="flex min-w-0 items-center gap-1"><span className={completed ? "min-w-0 flex-1 truncate text-sm font-medium text-muted-foreground line-through" : "min-w-0 flex-1 truncate text-sm font-medium"}>{item.title}</span>{item.type === "issue" ? <PulseIssueTimeframeBadge value={item.issueTimeframe} compact /> : null}</span>{showDestination && item.meetingName ? <span className="block truncate text-xs text-muted-foreground">{item.meetingName}</span> : null}</>}</span><span className="flex shrink-0 items-center gap-1">{indicator(Number(item.attachmentCount ?? 0), "documents", <Paperclip className="h-3.5 w-3.5" />)}</span><ChevronDown className={"h-4 w-4 shrink-0 text-muted-foreground transition-transform " + (expanded ? "rotate-180" : "")} /></button>{Number(item.linkedSubTodoCount ?? 0) > 0 ? <Button type="button" variant="ghost" size="sm" className="h-7 shrink-0 gap-1 px-1.5 text-xs text-primary" onClick={() => { setExpanded(true); setOpenSubTodosRequest(request => request + 1); }} title={`Show ${Number(item.linkedSubTodoCount)} sub-To-Do${Number(item.linkedSubTodoCount) === 1 ? "" : "s"}`} aria-label={`Show ${Number(item.linkedSubTodoCount)} sub-To-Do${Number(item.linkedSubTodoCount) === 1 ? "" : "s"}`}><CornerDownRight className="h-4 w-4" strokeWidth={2.75} /><span>{Number(item.linkedSubTodoCount)}</span></Button> : null}<Button type="button" variant="ghost" size="sm" className="h-7 shrink-0 gap-1 px-1.5 text-xs" onClick={openComments} title="Open comments" aria-label="Open comments"><MessageCircle className="h-3.5 w-3.5" />{Number(item.commentCount ?? 0) > 0 ? <span>{Number(item.commentCount)}</span> : null}</Button>{item.type === "todo" || item.type === "issue" ? <PulseQuickWorkControls item={item} onChanged={onChanged} /> : null}</div>{expanded ? <PulseItemContextPanel workItemId={item.id} completionRequest={completionRequest} openSubTodosRequest={openSubTodosRequest} onEdit={() => setEditing(true)} onAddSubTodo={parentEligible ? () => setAddingSubTodo(true) : undefined} canReopen={canReopen} onChanged={onChanged} onDeleted={() => setExpanded(false)} sourceSessionId={sourceSessionId} renderSubTodo={todo => <PulseInlineItemRow key={todo.id} item={{ ...todo, type: "todo", meetingId: todo.meetingId ?? item.meetingId, meetingName: todo.meetingName ?? item.meetingName, commentCount: todo.commentCount ?? 0, attachmentCount: todo.attachmentCount ?? 0, linkedSubTodoCount: todo.linkedSubTodoCount ?? 0 }} onChanged={onChanged} defaultDestinationId={defaultDestinationId ?? item.meetingId ?? null} sourceSessionId={sourceSessionId} canReopen={canReopen} />} /> : null}<PulseItemEditor open={editing} onOpenChange={setEditing} workItemId={item.id} defaultType={item.type === "issue" ? "issue" : "todo"} defaultDestinationId={defaultDestinationId ?? item.meetingId ?? null} sourceSessionId={sourceSessionId} title={"Edit " + item.title} onSaved={() => { onChanged(); setEditing(false); }} /><PulseItemEditor open={addingSubTodo} onOpenChange={setAddingSubTodo} defaultType="todo" defaultDestinationId={item.meetingId ?? defaultDestinationId ?? null} defaultAssigneeId={item.assigneeId ?? null} defaultDueDate={item.dueDate ?? null} defaultPriorityLevel={item.priorityLevel ?? "medium"} parentWorkItemId={item.id} sourceSessionId={sourceSessionId} title={"Add sub-To-Do under " + item.title} onSaved={() => { onChanged(); setAddingSubTodo(false); setExpanded(true); }} /></div>;
}
