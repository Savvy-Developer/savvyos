...
export function PulseInlineItemRow({ item, onChanged, defaultDestinationId, sourceSessionId }: { item: any; onChanged: () => void; defaultDestinationId?: string | null; sourceSessionId?: string | null }) {
  ...
  return <div className="overflow-hidden rounded-md border border-border bg-card"><div className="flex w-full items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-muted/45"><button type="button" aria-expanded={expanded} onClick={() => setExpanded(value => !value)} className="flex min-w-0 flex-1 items-center gap-2 text-left"><span className={"flex h-5 w-5 shrink-0 items-center justify-center rounded-full border " + (completed ? "border-emerald-500 bg-emerald-500 text-white" : "border-muted-foreground")}>{completed ? <Check className="h-3.5 w-3.5" /> : <Circle className="h-3.5 w-3.5" />}</span><span className="min-w-0 flex-1"><span className={completed ? "block truncate text-sm font-medium text-muted-foreground line-through" : "block truncate text-sm font-medium"}>{item.title}</span><span className="block truncate text-xs text-muted-foreground">{item.assigneeName ?? "Unassigned"} · {item.priorityLevel ?? "medium"} · due {dateLabel(item.dueDate)}{timeframe}{destination}</span></span><span className="flex shrink-0 items-center gap-1">{indicator(Number(item.commentCount ?? 0), "comments", <MessageCircle className="h-3.5 w-3.5" />)}{indicator(Number(item.attachmentCount ?? 0), "documents", <Paperclip className="h-3.5 w-3.5" />)}{indicator(Number(item.linkedSubTodoCount ?? 0), "sub-To-Dos", <ListChecks className="h-3.5 w-3.5 text-primary" />)}</span><ChevronDown className={"h-4 w-4 shrink-0 text-muted-foreground transition-transform " + (expanded ? "rotate-180" : "")} /></button>{parentEligible ? <Button type="button" variant="outline" size="sm" className="h-7 shrink-0 px-2 text-xs" onClick={() => setAddingSubTodo(true)} title="Add a sub-To-Do under this parent"><ListPlus className="mr-1 h-3.5 w-3.5" />Add sub-To-Do</Button> : null}</div>{expanded ? <PulseItemContextPanel workItemId={item.id} title={item.title} onEdit={() => setEditing(true)} onAddSubTodo={parentEligible ? () => setAddingSubTodo(true) : undefined} onChanged={onChanged} sourceSessionId={sourceSessionId} /> : null}<PulseItemEditor open={editing} onOpenChange={setEditing} workItemId={item.id} defaultType={item.type === "issue" ? "issue" : "todo"} defaultDestinationId={defaultDestinationId ?? item.meetingId ?? null} sourceSessionId={sourceSessionId} title={"Edit " + item.title} onSaved={() => { onChanged(); setEditing(false); }} /><PulseItemEditor open={addingSubTodo} onOpenChange={setAddingSubTodo} defaultType="todo" defaultDestinationId={item.meetingId ?? null} defaultAssigneeId={item.assigneeId ?? null} defaultDueDate={item.dueDate ?? null} defaultPriorityLevel={item.priorityLevel ?? "medium"} parentWorkItemId={item.id} sourceSessionId={sourceSessionId} title={"Add sub-To-Do under " + item.title} onSaved={() => { onChanged(); setAddingSubTodo(false); setExpanded(true); }} /></div>;
}
