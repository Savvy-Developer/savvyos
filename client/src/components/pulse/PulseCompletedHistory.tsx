...
export function PulseCompletedHistory({ contextId, title = "Completed & Resolved", description, initialType = "all", onlyMine = false, sourceSessionId, canReopen = true, onChanged, compact = false }: Props) {
  ...
  return <Card className="border-emerald-200/80 bg-emerald-50/20">
    ...
    {isOpen ? <CardContent className={resolvedCompact ? "space-y-1.5 border-t border-emerald-200/70 py-1.5" : compact ? "space-y-2 border-t border-emerald-200/70 py-2" : "space-y-3 border-t border-emerald-200/70 pt-3"}>
      ...
      {history.isLoading ? <div className="space-y-2"><Skeleton className="h-12 w-full" /><Skeleton className="h-12 w-full" /></div> : history.error ? <p className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">History could not be loaded: {history.error.message}</p> : items.length ? <div className={resolvedCompact ? "space-y-0.5" : compact ? "space-y-1" : "space-y-2"}>{items.map((item: any) => <div key={item.id}><PulseInlineItemRow item={item} defaultDestinationId={item.meetingId ?? null} sourceSessionId={sourceSessionId} canReopen={canReopen} onChanged={changed} /><p className={resolvedCompact ? "px-2 pt-px text-[11px] text-muted-foreground" : "px-2 pt-0.5 text-xs text-muted-foreground"}>{item.type === "issue" ? "Resolved" : "Completed"} {dateTime(item.completedAt)}{item.solvedNote ? ` · ${item.solvedNote}` : ""}</p></div>)}</div> : <div className={resolvedCompact ? "rounded-md border border-dashed p-2 text-center text-sm text-muted-foreground" : compact ? "rounded-md border border-dashed p-3 text-center text-sm text-muted-foreground" : "rounded-md border border-dashed p-5 text-center text-sm text-muted-foreground"}><CalendarDays className="mx-auto mb-2 h-5 w-5 text-emerald-700" />No {type === "issue" ? "resolved Issues" : type === "todo" ? "completed To-Dos" : "completed or resolved work"} matches these filters.</div>}
    </CardContent> : <CardContent className={resolvedCompact ? "border-t border-emerald-200/70 py-1 text-[11px] text-muted-foreground" : compact ? "border-t border-emerald-200/70 py-1.5 text-xs text-muted-foreground" : "border-t border-emerald-200/70 py-2.5 text-xs text-muted-foreground"}>{currentLabel.charAt(0).toUpperCase() + currentLabel.slice(1)} are retained here in the same permanent record and stay collapsed until recalled.</CardContent>}
  </Card>;
}
