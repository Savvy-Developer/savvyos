...
export function PulseWeeklyPreparation() {
  ...
  return <section id="weekly-preparation" className="pulse-section scroll-mt-6"><div><p className="text-sm font-medium text-primary">Week of {weekLabel(prep.data.weekOf)}</p><h2 className="mt-1 text-xl font-semibold tracking-tight">Weekly preparation</h2><p className="mt-1 text-sm text-muted-foreground">Prepare directly in My EOS. Every value, Segue, and Headline is saved to the L10 named on its card.</p></div><div className="space-y-3">{prep.data.meetings.map((meeting: any) => { const fields = prep.data.fields.filter((field: any) => field.meetingId === meeting.id); const required = fields.filter((field: any) => field.required); const missing = required.filter((field: any) => field.value === null || String(field.value).trim() === ""); return <Card key={meeting.id} className={`pulse-card-compact ${meeting.submitted ? "border-emerald-200" : ""}`}><CardHeader className="pb-2"><CardTitle className="text-lg">{meeting.name}</CardTitle><CardDescription>{meeting.submitted ? "Confirmed for this week. Reopen it to make a change." : missing.length ? `${missing.length} required preparation item${missing.length === 1 ? "" : "s"} remains.` : "Required preparation is ready to confirm."}</CardDescription></CardHeader><CardContent>{fields.map((field: any) => <PreparationField key={field.key} field={field} onSaved={() => void utils.pulse.personal.inputs.invalidate()}/>) }<div className="mt-3 flex justify-end border-t border-border pt-3">{meeting.submitted ? <Button variant="outline" disabled={withdraw.isPending} onClick={() => withdraw.mutate({ meetingId: meeting.id })}>Reopen preparation</Button> : <Button disabled={Boolean(missing.length) || submit.isPending} onClick={() => submit.mutate({ meetingId: meeting.id })}><Send className="mr-2 h-4 w-4"/>{submit.isPending ? "Confirming…" : "Confirm preparation"}</Button>}</div></CardContent></Card>; })}</div></section>;
}
