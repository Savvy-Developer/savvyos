    ...
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto">
        ...
        {!isReviewing ? <div className="space-y-5 py-2">
          ...
          <div className="flex justify-end pt-1"><Button type="button" variant="outline" size="sm" onClick={() => setTestSendOpen(true)} disabled={!body.trim() || (channel === "email" && !subject.trim())}><Send className="mr-1.5 h-4 w-4" /> Test send</Button></div>
          ...
          {channel === "sms" && <div className="flex gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900"><AlertTriangle className="h-4 w-4 shrink-0" /><p>Text sends use the dedicated Aircall marketing number. Contacts without a phone number, without recorded SMS marketing consent, or marked Do Not Contact or SMS opt-out are excluded before delivery.</p></div>}
          ...
          {preview.isLoading ? <div className="py-10 text-center text-sm text-muted-foreground">Checking the current audience...</div> : preview.error ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">{preview.error.message}</div> : preview.data ? <><div className="grid gap-3 sm:grid-cols-4"><div className="rounded-lg border p-4"><p className="text-xs text-muted-foreground">Matching contacts</p><p className="mt-1 text-2xl font-semibold">{preview.data.matchingCount.toLocaleString()}</p></div><div className="rounded-lg border p-4"><p className="text-xs text-muted-foreground">Eligible contacts</p><p className="mt-1 text-2xl font-semibold">{preview.data.eligibleContactCount.toLocaleString()}</p></div><div className="rounded-lg border border-emerald-200 bg-emerald-50/40 p-4"><p className="text-xs text-emerald-700">Messages to send</p><p className="mt-1 text-2xl font-semibold text-emerald-800">{preview.data.recipientCount.toLocaleString()}</p></div><div className="rounded-lg border p-4"><p className="text-xs text-muted-foreground">Excluded contacts</p><p className="mt-1 text-2xl font-semibold">{preview.data.excludedCount.toLocaleString()}</p></div></div>{preview.data.excludedCount > 0 && <div className="rounded-lg border bg-muted/30 p-4"><p className="text-sm font-medium">Why contacts are excluded</p><p className="mt-1 text-xs text-muted-foreground">Contacts are counted once using the first applicable reason.</p><div className="mt-3 grid gap-2 text-sm sm:grid-cols-2">{preview.data.exclusionReasons.noEmailAddress > 0 && <p><strong>{preview.data.exclusionReasons.noEmailAddress.toLocaleString()}</strong> without an email address</p>}{preview.data.exclusionReasons.noPhoneAddress > 0 && <p><strong>{preview.data.exclusionReasons.noPhoneAddress.toLocaleString()}</strong> without a phone number</p>}{preview.data.exclusionReasons.bounced > 0 && <p><strong>{preview.data.exclusionReasons.bounced.toLocaleString()}</strong> with a hard-bounced email</p>}{preview.data.exclusionReasons.unsubscribed > 0 && <p><strong>{preview.data.exclusionReasons.unsubscribed.toLocaleString()}</strong> unsubscribed from email</p>}{preview.data.exclusionReasons.doNotContact > 0 && <p><strong>{preview.data.exclusionReasons.doNotContact.toLocaleString()}</strong> marked Do Not Contact</p>}{(preview.data.exclusionReasons as any).smsNoConsent > 0 && <p><strong>{(preview.data.exclusionReasons as any).smsNoConsent.toLocaleString()}</strong> without recorded SMS marketing consent</p>}{(preview.data.exclusionReasons as any).smsOptedOut > 0 && <p><strong>{(preview.data.exclusionReasons as any).smsOptedOut.toLocaleString()}</strong> opted out of marketing SMS</p>}{preview.data.exclusionReasons.emailNotVerified > 0 && <p><strong>{preview.data.exclusionReasons.emailNotVerified.toLocaleString()}</strong> with an unverified email status</p>}</div></div>}</> : null}
