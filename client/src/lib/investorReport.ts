...
const fmt = (value: unknown): string => numeric(value).toLocaleString("en-US", {
  style: "currency",
  currency: "USD",
...
export function buildReportHTML(data: ReportData, headshotB64: string, photoB64: string, coBrandLogoB64: string): string {
  ...
  const residualBuildingRecoveryYears = numeric(calc.residualBuildingRecoveryYears) || 39;
  ...
  const comparablePage = reportSections.comps && comps.length ? `<div class="pdf-page"><div class="hdr">${brandLockup}<span class="hdr-sub">Comparable Properties</span></div><div class="page-intro"><h2>Revenue Comparable Properties</h2><p>Comparable-property inputs used to inform the revenue analysis.</p></div><table class="comp-table"><thead><tr><th>Comparable</th><th>Location</th><th>Beds</th><th>ADR</th><th>Occupancy</th><th>Est. Annual Revenue</th><th>Guest Signal</th><th>Listing</th></tr></thead><tbody>${comps.map((comp: any) => {
  ...
  const financingCard = `<div class="card financing-summary"><h4>Financing Summary</h4><div class="row"><span class="lbl">Loan type</span><span class="val">${escapeHtml(loanLabel(form.loanType))}</span></div><div class="row"><span class="lbl">Loan amount</span><span class="val">${fmt(calc.loanAmount)}</span></div><div class="row"><span class="lbl">Interest rate</span><span class="val">${escapeHtml(form.interestRate || 7)}%</span></div><div class="row"><span class="lbl">Loan term</span><span class="val">${escapeHtml(form.loanTermYears || 30)} years</span></div><div class="row"><span class="lbl">Monthly debt payment</span><span class="val">${fmt(calc.monthlyMortgage)}</span></div><div class="row brd"><span class="lbl bold">Annual debt service</span><span class="val big">${fmt(calc.annualDebtService)}</span></div></div>`;
  ...
  const taxBenefitsPage = reportSections.taxBenefits ? `<div class="pdf-page"><div class="hdr">${brandLockup}<span class="hdr-sub">Cost Segregation & Tax Benefits</span></div><div class="page-intro"><h2>Modeled Tax-Benefit Detail</h2><p>Illustrative deductions and tax savings based on the proforma inputs. Confirm eligibility, classification, and tax treatment with a CPA.</p></div><div class="cols"><div class="card"><h4>Basis & Study Inputs</h4><div class="row"><span class="lbl">Purchase price</span><span class="val">${fmt(purchasePrice)}</span></div><div class="row"><span class="lbl">Land allocation</span><span class="val">${escapeHtml(form.landAllocationPct || 0)}%</span></div><div class="row"><span class="lbl">Building basis after land</span><span class="val">${fmt(calc.buildingBasis)}</span></div><div class="row"><span class="lbl">Cost segregation study</span><span class="val">${calc.costSegEnabled ? "Yes" : "No"}</span></div><div class="row"><span class="lbl">Shorter-life property allocation (100% bonus)</span><span class="val">${calc.costSegEnabled ? `${escapeHtml(form.acceleratedDepreciationPct || 0)}%` : "0%"}</span></div><div class="row"><span class="lbl">Cost-seg shorter-life basis (100% bonus)</span><span class="val">${fmt(calc.acceleratedAmt)}</span></div><div class="row brd"><span class="lbl bold">Residual building basis</span><span class="val big">${fmt(calc.remainingBuildingBasis)}</span></div></div><div class="card"><h4>Year 1 Deduction Components</h4><div class="row"><span class="lbl">Cost-seg shorter-life property (100% bonus)</span><span class="val">${fmt(calc.acceleratedAmt)}</span></div><div class="row"><span class="lbl">Furnishing deduction (modeled eligible)</span><span class="val">${fmt(calc.furnishingDeduction)}</span></div><div class="row"><span class="lbl">Bonus-eligible improvements (100% bonus)</span><span class="val">${fmt(calc.bonusEligibleImprovements)}</span></div><div class="row"><span class="lbl">Full-year residual building depreciation (${residualBuildingRecoveryYears} yrs)</span><span class="val">${fmt(calc.straightLineDepreciation)}</span></div><div class="row"><span class="lbl">Scheduled Year 1 mortgage interest</span><span class="val">${fmt(calc.year1MortgageInterest)}</span></div><div class="row brd"><span class="lbl bold">Total Year 1 deduction</span><span class="val big">${fmt(calc.totalFirstYearDeduction)}</span></div></div></div><div class="cols"><div class="card emphasis-card"><h4>Year 1 Estimated Tax Benefit</h4><div class="row"><span class="lbl">Marginal tax rate</span><span class="val">${escapeHtml(form.marginalTaxRate || 0)}%</span></div><div class="row"><span class="lbl">Estimated tax savings</span><span class="val">${fmt(calc.taxSavings)}</span></div><div class="row"><span class="lbl">Cost-seg study cost</span><span class="val">-${fmt(calc.costSegCost)}</span></div><div class="row brd"><span class="lbl bold">Net tax benefit, Year 1</span><span class="val big teal">${fmt(calc.netTaxBenefit)}</span></div></div><div class="card"><h4>Estimated Year 2 Benefit</h4><div class="row"><span class="lbl">Full-year residual building depreciation</span><span class="val">${fmt(calc.straightLineDepreciation)}</span></div><div class="row"><span class="lbl">Scheduled Year 2 mortgage interest</span><span class="val">${fmt(calc.year2MortgageInterest)}</span></div><div class="row"><span class="lbl">Estimated Year 2 deduction</span><span class="val">${fmt(calc.ongoingAnnualDeduction)}</span></div><div class="row brd"><span class="lbl bold">Estimated Year 2 tax savings</span><span class="val big teal">${fmt(calc.ongoingAnnualTaxBenefit)}</span></div></div></div><div class="note-panel"><h3>Important modeling notes</h3><p>The renovation budget of ${fmt(calc.renovation)} is not automatically treated as bonus-depreciable. The model treats qualifying shorter-life cost-segregated property and the separately entered ${fmt(calc.bonusEligibleImprovements)} of bonus-eligible improvements as 100% Year 1 bonus deductions. The residual building figure is a full-year ${residualBuildingRecoveryYears}-year transient-STR estimate after that bonus-depreciated basis is removed, so it is the only building depreciation carried into Year 2+; first and disposition years require the IRS mid-month convention. Mortgage interest follows the loan’s amortization schedule and declines over time.</p></div><div class="disc">Tax calculations are illustrative and not tax advice. Actual results depend on placed-in-service dates, the property’s transient-lodging classification and operating facts, passive-activity and material-participation rules, basis adjustments, filing position, and professional tax advice.</div></div>` : "";

  const refiPercent = (value: unknown): string => value === Infinity ? "∞" : pct(value);
  const refiDscr = (value: unknown): string => value === Infinity ? "∞" : `${numeric(value).toFixed(2)}x`;
  const cashLeftInDeal = Math.max(0, numeric(refi.cashLeftInDeal));
  const preRefiEffectiveCash = Math.max(0, numeric(calc.totalCashNeeded) - numeric(calc.netTaxBenefit));
  const postRefiEffectiveCash = Math.max(0, cashLeftInDeal - numeric(calc.netTaxBenefit));
  const refiComparisonTable = calc.isCashoutRefi ? `
    <div class="stitle">Pre-Refi vs. Post-Refi Returns</div>
    <table class="refi-returns-table">
      <thead>
        <tr>
          <th rowspan="2">Metric</th>
          <th colspan="3">Pre-Refi (Original)</th>
          <th colspan="3">Post-Refi (New Loan)</th>
        </tr>
        <tr>
          <th>Cons.</th><th>Base</th><th>Strong</th>
          <th>Cons.</th><th>Base</th><th>Strong</th>
        </tr>
      </thead>
      <tbody>
        <tr><td>Monthly Mortgage</td><td>${fmt(calc.monthlyMortgage)}</td><td>${fmt(calc.monthlyMortgage)}</td><td>${fmt(calc.monthlyMortgage)}</td><td>${fmt(refi.refiMonthlyMortgage)}</td><td>${fmt(refi.refiMonthlyMortgage)}</td><td>${fmt(refi.refiMonthlyMortgage)}</td></tr>
        <tr><td>Annual Debt Service</td><td>${fmt(calc.annualDebtService)}</td><td>${fmt(calc.annualDebtService)}</td><td>${fmt(calc.annualDebtService)}</td><td>${fmt(refi.refiAnnualDebtService)}</td><td>${fmt(refi.refiAnnualDebtService)}</td><td>${fmt(refi.refiAnnualDebtService)}</td></tr>
        <tr class="hlr"><td>Annual Cash Flow</td><td>${fmt(s1.cashFlow)}</td><td>${fmt(s2.cashFlow)}</td><td>${fmt(s3.cashFlow)}</td><td>${fmt(refi.s1?.postRefiCashFlow)}</td><td>${fmt(refi.s2?.postRefiCashFlow)}</td><td>${fmt(refi.s3?.postRefiCashFlow)}</td></tr>
        <tr><td>Monthly Cash Flow</td><td>${fmt(s1.monthlyCashFlow)}</td><td>${fmt(s2.monthlyCashFlow)}</td><td>${fmt(s3.monthlyCashFlow)}</td><td>${fmt(refi.s1?.postRefiMonthlyCF)}</td><td>${fmt(refi.s2?.postRefiMonthlyCF)}</td><td>${fmt(refi.s3?.postRefiMonthlyCF)}</td></tr>
        <tr><td>Cash in Deal</td><td>${fmt(calc.totalCashNeeded)}</td><td>${fmt(calc.totalCashNeeded)}</td><td>${fmt(calc.totalCashNeeded)}</td><td>${fmt(cashLeftInDeal)}</td><td>${fmt(cashLeftInDeal)}</td><td>${fmt(cashLeftInDeal)}</td></tr>
        <tr class="tax-row"><td>Effective Cash (after Yr 1 tax benefit)</td><td>${fmt(preRefiEffectiveCash)}</td><td>${fmt(preRefiEffectiveCash)}</td><td>${fmt(preRefiEffectiveCash)}</td><td>${fmt(postRefiEffectiveCash)}</td><td>${fmt(postRefiEffectiveCash)}</td><td>${fmt(postRefiEffectiveCash)}</td></tr>
        <tr class="hlr"><td>Cash-on-Cash Return</td><td>${pct(s1.cashOnCash)}</td><td>${pct(s2.cashOnCash)}</td><td>${pct(s3.cashOnCash)}</td><td>${refi.s1?.infiniteReturn ? "∞" : refiPercent(refi.s1?.postRefiCoC)}</td><td>${refi.s2?.infiniteReturn ? "∞" : refiPercent(refi.s2?.postRefiCoC)}</td><td>${refi.s3?.infiniteReturn ? "∞" : refiPercent(refi.s3?.postRefiCoC)}</td></tr>
        <tr><td>DSCR</td><td>${refiDscr(s1.dscr)}</td><td>${refiDscr(s2.dscr)}</td><td>${refiDscr(s3.dscr)}</td><td>${refiDscr(refi.s1?.postRefiDSCR)}</td><td>${refiDscr(refi.s2?.postRefiDSCR)}</td><td>${refiDscr(refi.s3?.postRefiDSCR)}</td></tr>
        <tr class="tax-row"><td>CoC w/ Tax Benefits (Yr 1)</td><td>${pct(taxReturns.s1?.year1CoCWithTax)}</td><td>${pct(taxReturns.s2?.year1CoCWithTax)}</td><td>${pct(taxReturns.s3?.year1CoCWithTax)}</td><td>${refiPercent(refi.s1?.postRefiYear1CoCWithTax)}</td><td>${refiPercent(refi.s2?.postRefiYear1CoCWithTax)}</td><td>${refiPercent(refi.s3?.postRefiYear1CoCWithTax)}</td></tr>
        <tr class="tax-row"><td>CoC w/ Tax Benefits (Yr 2+)</td><td>${pct(taxReturns.s1?.ongoingCoCWithTax)}</td><td>${pct(taxReturns.s2?.ongoingCoCWithTax)}</td><td>${pct(taxReturns.s3?.ongoingCoCWithTax)}</td><td>${refiPercent(refi.s1?.postRefiOngoingCoCWithTax)}</td><td>${refiPercent(refi.s2?.postRefiOngoingCoCWithTax)}</td><td>${refiPercent(refi.s3?.postRefiOngoingCoCWithTax)}</td></tr>
      </tbody>
    </table>` : "";

  const valueAddPage = reportSections.valueAdd && (calc.isValueAdd || calc.isCashoutRefi) ? `<div class="pdf-page"><div class="hdr">${brandLockup}<span class="hdr-sub">Value-Add & Refinance Analysis</span></div>${calc.isValueAdd ? `<div class="stitle">Equity Creation Through Value-Add</div><div class="cols"><div class="card"><h4>Investment Basis</h4><div class="row"><span class="lbl">Purchase price</span><span class="val">${fmt(purchasePrice)}</span></div><div class="row"><span class="lbl">Renovation budget</span><span class="val">${fmt(calc.renovation)}</span></div><div class="row brd"><span class="lbl bold">All-in cost</span><span class="val big">${fmt(purchasePrice + numeric(calc.renovation))}</span></div></div><div class="card emphasis-card"><h4>After Repair Value</h4><div class="row"><span class="lbl">ARV</span><span class="val big teal">${fmt(calc.arv)}</span></div><div class="row"><span class="lbl">Forced equity</span><span class="val">${fmt(calc.forcedEquity)}</span></div><div class="row brd"><span class="lbl bold">Net equity created</span><span class="val big green">${fmt(calc.equityCreatedByReno)}</span></div></div></div>` : ""}${calc.isCashoutRefi ? `<div class="stitle">Cash-Out Refinance</div><div class="cols"><div class="card"><h4>Refinance Terms</h4><div class="row"><span class="lbl">Appraised value</span><span class="val">${fmt(refi.refiAppraised)}</span></div><div class="row"><span class="lbl">LTV</span><span class="val">${escapeHtml(form.refiLTV || 75)}%</span></div><div class="row"><span class="lbl">New loan amount</span><span class="val">${fmt(refi.refiNewLoanAmount)}</span></div><div class="row brd"><span class="lbl bold">Cash out</span><span class="val big green">${fmt(refi.refiCashOut)}</span></div></div><div class="card"><h4>Refinance Timing & Liquidity</h4><div class="row"><span class="lbl">New monthly payment</span><span class="val">${fmt(refi.refiMonthlyMortgage)}</span></div><div class="row"><span class="lbl">Seasoning period</span><span class="val">${numeric(refi.seasoningMonths)} months</span></div><div class="row"><span class="lbl">Cash left in deal</span><span class="val">${fmt(cashLeftInDeal)}</span></div><div class="row brd"><span class="lbl bold">Cash-out proceeds</span><span class="val big green">${fmt(refi.refiCashOut)}</span></div></div></div>${refiComparisonTable}` : ""}</div>` : "";
  ...
  ${reportSections.scenarioComparison ? `<div class="pdf-page">
