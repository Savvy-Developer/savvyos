...
function buildReportHTML(data: ReportData, headshotB64: string, photoB64: string, coBrandLogoB64: string): string {
  ...
  <div class="pdf-page">
    ...
    <div class="cols"><div class="card"><h4>Acquisition & Cash to Close</h4><div class="row"><span class="lbl">Purchase price</span><span class="val">${fmt(purchasePrice)}</span></div><div class="row"><span class="lbl">Down payment (${escapeHtml(form.downPaymentPct || 20)}%)</span><span class="val">${fmt(calc.downPayment)}</span></div><div class="row"><span class="lbl">Closing costs</span><span class="val">${fmt(calc.closingCosts)}</span></div><div class="row"><span class="lbl">Furnishing</span><span class="val">${fmt(calc.furnishing)}</span></div><div class="row"><span class="lbl">Renovation</span><span class="val">${fmt(calc.renovation)}</span></div><div class="row"><span class="lbl">Startup & inspection</span><span class="val">${fmt(numeric(calc.startup) + numeric(calc.inspection))}</span></div>${numeric(calc.sellerCredit) ? `<div class="row"><span class="lbl teal">Seller credit</span><span class="val teal">-${fmt(calc.sellerCredit)}</span></div>` : ""}<div class="row brd"><span class="lbl bold">Total cash needed</span><span class="val big">${fmt(calc.totalCashNeeded)}</span></div></div><div class="card"><h4>Financing & Operating Assumptions</h4><div class="row"><span class="lbl">Loan type</span><span class="val">${escapeHtml(loanLabel(form.loanType))}</span></div><div class="row"><span class="lbl">Loan amount</span><span class="val">${fmt(calc.loanAmount)}</span></div><div class="row"><span class="lbl">Interest rate</span><span class="val">${escapeHtml(form.interestRate || 7)}%</span></div><div class="row"><span class="lbl">Loan term</span><span class="val">${escapeHtml(form.loanTermYears || 30)} years</span></div><div class="row"><span class="lbl">Annual debt service</span><span class="val">${fmt(calc.annualDebtService)}</span></div><div class="row"><span class="lbl">Channel mix</span><span class="val">Airbnb ${escapeHtml(airbnbPct)}% / Vrbo ${escapeHtml(vrboPct)}% / Direct ${escapeHtml(directPct)}%</span></div><div class="row brd"><span class="lbl bold">Blended platform fee</span><span class="val big">${pct(calc.blendedFeeRate)}</span></div></div></div>
  </div>
