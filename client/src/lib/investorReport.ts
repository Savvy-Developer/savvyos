...
function buildReportHTML(data: ReportData, headshotB64: string, photoB64: string, coBrandLogoB64: string): string {
  ...
  const maxRevenue = Math.max(numeric(s1.grossRevenue), numeric(s2.grossRevenue), numeric(s3.grossRevenue), 1);
  const barData = [
    { label: "Conservative", value: numeric(s1.grossRevenue), color: "#94a3b8", x: 145 },
    { label: "Base Case", value: numeric(s2.grossRevenue), color: "#0891b2", x: 360 },
    { label: "Strong", value: numeric(s3.grossRevenue), color: "#059669", x: 575 },
  ];
  const barChart = `<svg class="revenue-chart" viewBox="0 0 720 215" role="img" aria-label="Gross annual revenue by scenario"><line x1="72" y1="168" x2="662" y2="168" stroke="#cbd5e1" stroke-width="2"/>${barData.map((item) => {
    const height = Math.max(16, (item.value / maxRevenue) * 118);
    const y = 168 - height;
    return `<text x="${item.x}" y="${y - 11}" text-anchor="middle" font-size="16" font-weight="700" fill="#172033">${fmtCompact(item.value)}</text><rect x="${item.x - 48}" y="${y}" width="96" height="${height}" rx="8" fill="${item.color}"/><text x="${item.x}" y="192" text-anchor="middle" font-size="14" font-weight="600" fill="#475569">${item.label}</text>`;
  }).join("")}<text x="72" y="28" font-size="13" fill="#64748b">Annual gross revenue</text></svg>`;
  ...
  return `<style>
    ...
    .mgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px;}.mc{display:flex;min-height:70px;flex-direction:column;align-items:center;justify-content:center;background:#f0fdfa;border:1px solid #99f6e4;border-radius:8px;padding:10px 8px;text-align:center;}.mc.hi{background:#0891b2;border-color:#0891b2;color:#fff;}.mc.hi .ml{color:#e0f2fe;}.ml{font-size:10px;font-weight:800;line-height:1.2;text-transform:uppercase;letter-spacing:.2px;color:#475569;margin-bottom:5px;}.mv{font-size:19px;line-height:1;font-weight:850;text-align:center;}
    ...
    .cols{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:12px;}.card{border:1px solid #dbe4ec;border-radius:9px;padding:13px;background:#fff;}.emphasis-card{background:#f0fdfa;}.card h4{text-align:center;font-size:14px;font-weight:850;color:#0e7490;margin:0 0 8px;}.row{display:flex;justify-content:space-between;gap:12px;font-size:11.5px;line-height:1.3;padding:4px 0;border-bottom:1px solid #eef2f6;}.row:last-child{border-bottom:none;}.lbl{color:#526174;}.val{font-weight:750;color:#172033;text-align:right;}.brd{border-top:2px solid #0891b2;margin-top:4px;padding-top:7px;}.bold{font-weight:850;}.big{font-size:14px;}.teal{color:#087d99;}.green{color:#047857;}.acquisition-summary{margin-bottom:12px;}.acquisition-grid{display:grid;grid-template-columns:1fr 1fr;column-gap:20px;}.acquisition-grid .row{font-size:11px;}.financing-summary{margin-bottom:12px;}.channel-mix{display:flex;justify-content:space-between;gap:10px;margin-top:8px;padding:7px 0;border-top:1px solid #cbd5e1;color:#0e7490;font-size:11px;line-height:1.35;}.channel-mix strong{text-align:right;color:#172033;}
  ...
  <div class="pdf-page">
    ...
    <div class="mgrid"><div class="mc"><div class="ml">Total Cash Needed</div><div class="mv">${fmt(calc.totalCashNeeded)}</div></div><div class="mc"><div class="ml">Monthly Mortgage</div><div class="mv">${fmt(calc.monthlyMortgage)}</div></div><div class="mc hi"><div class="ml">Base Case Cash Flow</div><div class="mv">${fmt(s2.cashFlow)}</div></div><div class="mc hi"><div class="ml">Cash-on-Cash Return</div><div class="mv">${pct(s2.cashOnCash)}</div></div><div class="mc"><div class="ml">Cap Rate</div><div class="mv">${pct(s2.capRate)}</div></div><div class="mc"><div class="ml">DSCR</div><div class="mv">${numeric(s2.dscr).toFixed(2)}x</div></div><div class="mc"><div class="ml">Break-Even Occupancy</div><div class="mv">${pct(s2.breakEvenOcc)}</div></div><div class="mc"><div class="ml">Net Tax Benefit, Year 1</div><div class="mv">${fmt(calc.netTaxBenefit)}</div></div></div>
  ...
  <div class="pdf-page">
    ...
    <div class="card financing-summary"><h4>Financing Summary</h4><div class="row"><span class="lbl">Loan type</span><span class="val">${escapeHtml(loanLabel(form.loanType))}</span></div><div class="row"><span class="lbl">Loan amount</span><span class="val">${fmt(calc.loanAmount)}</span></div><div class="row"><span class="lbl">Interest rate</span><span class="val">${escapeHtml(form.interestRate || 7)}%</span></div><div class="row"><span class="lbl">Loan term</span><span class="val">${escapeHtml(form.loanTermYears || 30)} years</span></div><div class="row"><span class="lbl">Annual debt service</span><span class="val">${fmt(calc.annualDebtService)}</span></div><div class="row brd"><span class="lbl bold">Blended platform fee</span><span class="val big">${pct(calc.blendedFeeRate)}</span></div></div>
  </div>
  ...
  <div class="pdf-page">
    ...
    ${calc.costSegEnabled ? `<div class="stitle">Tax Benefits</div><div class="cols"><div class="card"><h4>Year 1 — Cost Segregation</h4><div class="row"><span class="lbl">Total Year 1 deduction</span><span class="val">${fmt(calc.totalFirstYearDeduction)}</span></div><div class="row"><span class="lbl">Tax savings at ${escapeHtml(form.marginalTaxRate || 35)}%</span><span class="val">${fmt(calc.taxSavings)}</span></div><div class="row"><span class="lbl">Study cost</span><span class="val">-${fmt(calc.costSegCost)}</span></div><div class="row brd"><span class="lbl bold">Net tax benefit</span><span class="val big teal">${fmt(calc.netTaxBenefit)}</span></div></div><div class="card"><h4>Ongoing Annual Benefit</h4><div class="row"><span class="lbl">Straight-line depreciation</span><span class="val">${fmt(calc.straightLineDepreciation)}/yr</span></div><div class="row"><span class="lbl">Mortgage interest deduction</span><span class="val">${fmt(calc.year1MortgageInterest)}/yr</span></div><div class="row brd"><span class="lbl bold">Annual tax savings</span><span class="val big teal">${fmt(calc.ongoingAnnualTaxBenefit)}/yr</span></div></div></div>` : ""}
  </div>
