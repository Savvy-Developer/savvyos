...
export default function PublicMarketMatchQuizPage() {
  ...
  function toggleSelection(marketId: number) { setSelectedMarkets(current => { if (current.includes(marketId)) return current.filter(id => id !== marketId); if (current.length >= maxConnections) { toast.error(`Choose up to ${maxConnections} markets for introductions.`); return current; } return [...current, marketId]; }); }
