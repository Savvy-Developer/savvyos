...
export default function EmailTestPage() {
  ...
  const sendOne = trpc.emailTest.sendOne.useMutation({
    onSuccess: (data, vars) => {
      ...
      if (data.skipped) {
        setResults(prev => ({ ...prev, [vars.emailType]: "skipped: market AI update email sent within 48 hours" }));
        toast.info(`"${label}" was skipped because ${vars.recipientEmail} received a market AI update email within the last 48 hours.`);
        setSendingOne(null);
        return;
      }
