...
export default function CoachingAgentPage() {
  ...
  useEffect(() => {
    if (!Number.isInteger(agentId) || openedAgentIdRef.current === agentId)
      return;
  ...
  const { data, isLoading, error, refetch } = trpc.coaching.getProfile.useQuery(
    { agentId },
  ...
  const createSession = trpc.coaching.createSession.useMutation({
    onSuccess: (result: any) => {
      ...
      toast.success(
        warnings.length
          ? "Session created. Review the scheduling notice."
          : "Session created with calendar details."
      );
  ...
  if (!Number.isInteger(agentId) || agentId <= 0) {
    return (
      <div className="p-6 text-center text-muted-foreground">
        ...
        <p>This coaching profile link is invalid.</p>
        <Button
          variant="ghost"
          size="sm"
          className="mt-4"
  ...
  if (error) {
    return (
      <div className="p-6 text-center text-muted-foreground">
        <AlertTriangle className="h-8 w-8 mx-auto mb-2 text-amber-500" />
        <p>Unable to load this agent’s coaching workspace.</p>
  ...
  const terminationsToShow = showAllTerminations
    ? terminations
    : terminations.slice(0, 5);
  ...
  return (
    <div className="p-4 sm:p-6 max-w-screen-2xl mx-auto space-y-5">
      ...
      <Card className="border-l-4 border-l-primary">
        <CardContent className="p-5">
              ...
              <div>
                <h1 className="text-lg sm:text-xl font-bold flex items-center gap-2 flex-wrap">
                  ...
                  {profile?.performanceStatus && (
                    <Badge
                      className={`text-xs border ${PERF_STATUS_COLORS[profile.performanceStatus] ?? ""}`}
                      variant="outline"
                    >
                      {profile.performanceStatus}
                    </Badge>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    ...
                    <span
                      className={
                        RISK_COLORS[profile?.retentionRiskStatus] ?? ""
                      }
                    >
                      {profile?.retentionRiskStatus ?? "—"}
                    </span>
                  </span>
                  {profile?.performanceStatus === "Launch" && (
                    <span>
          ...
          <div className="mt-4 flex flex-wrap gap-4 rounded-lg bg-muted/40 p-3 text-xs">
            <div>
              <span className="text-muted-foreground">Last coached by:</span>{" "}
              <strong>{sessionStats?.lastCoachName ?? "—"}</strong> on{" "}
              <strong>
                {sessionStats?.lastSessionDate
                  ? safeFormat(sessionStats.lastSessionDate, "MMM d, yyyy")
                  : "—"}
              </strong>
            </div>
            <div>
              <span className="text-muted-foreground">Next session with:</span>{" "}
              <strong>{nextCoach?.name ?? "—"}</strong> on{" "}
              <strong>
                {profile?.nextSessionDate ? (
                  safeFormatET(profile.nextSessionDate, {
                    month: "short",
                    day: "numeric",
                    hour: "numeric",
                    minute: "2-digit",
            ...
            <div>
              ...
              <strong>{sessionStats?.totalSessions ?? 0}</strong>
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-9 gap-2">
            <StatBox
              label="SavvyOS Activity"
              value={`${activityScore}/100`}
              sub={`${adoption?.contactActivitiesWeek ?? 0} contact activities this week`}
              accent={activityScoreAccent}
            />
            <StatBox
              label="Closed Units (90d)"
              value={closedUnits}
              accent="text-emerald-700"
            />
            <StatBox
              label="Closed Volume"
              value={closedVolume > 0 ? fmtMoney(closedVolume) : "$0"}
            />
            <StatBox label="UC Units" value={ucUnits} accent="text-blue-700" />
            <StatBox
              label="UC Volume"
              value={ucVolume > 0 ? fmtMoney(ucVolume) : "$0"}
            />
            <StatBox label="Leads" value={totalLeads} />
            <StatBox
              label="Avg Lead Age"
              value={`${avgLeadAge}d`}
              accent={avgLeadAge > 30 ? "text-red-600" : ""}
            />
            <StatBox
              label="Overdue Tasks"
              value={overdueTasks}
              accent={overdueTasks > 5 ? "text-red-600" : ""}
            />
            <StatBox
              label="Term Rate"
              value={`${terminationRate}%`}
              accent={terminationRate > 20 ? "text-red-600" : ""}
            />
          </div>
            ...
            <div className="mt-3 rounded-lg bg-red-50 border border-red-200 p-3 flex items-center gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px] text-red-700">
                  This agent does not have goals entered in SavvyOS.
                </p>
              </div>
              <Button
                size="sm"
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div
          className="w-full overflow-x-auto rounded-t-lg border-b bg-muted/20"
          ...
          <TabsList className="flex h-auto min-w-max items-stretch justify-start gap-0 bg-transparent p-0">
            {[
              ...
              >
                ...
                {tab.label}
                {(tab as any).count > 0 && (
                  <span className="ml-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-primary/10 px-1 text-[9px] font-bold text-primary">
                    {(tab as any).count}
                  </span>
                )}
                {(tab as any).active && (
              ...
              <CardContent>
                <Table>
                  <TableBody>
                    ...
                    <TableRow>
                      <TableCell className="text-xs font-medium">
                        Closed Volume (Trailing 90d)
                      </TableCell>
                      <TableCell className="text-xs text-right">
                        {fmtMoney(closedVolume)}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="text-xs font-medium">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  {(() => {
                    const closed =
                      prodStats?.buyerClosedCount +
                        prodStats?.sellerClosedCount +
                        prodStats?.dualClosedCount || 0;
                  ...
                  <div className="mt-3 pt-3 border-t">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                      <StatBox
                        label="Buyer Closed"
                        value={prodStats?.buyerClosedCount ?? 0}
                        accent="text-blue-700"
                      />
                      <StatBox
                        label="Seller Closed"
                        value={prodStats?.sellerClosedCount ?? 0}
                        accent="text-purple-700"
                      />
                      <StatBox
                        label="Dual Closed"
                        value={prodStats?.dualClosedCount ?? 0}
              ...
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-3">
                  <StatBox
                    ...
                    value={prodStats?.activeListings ?? 0}
                    accent="text-emerald-700"
                  />
                  <StatBox
                    label="Under Contract"
                    value={prodStats?.ucListings ?? 0}
                    accent="text-blue-700"
                  />
                  <StatBox
                    label="Sold (Closed)"
                    value={prodStats?.soldListings ?? 0}
                  />
                  <StatBox
                    label="Terminated"
                    value={prodStats?.terminatedListings ?? 0}
                    accent={
                      prodStats?.terminatedListings > 0 ? "text-red-600" : ""
                    }
                  />
                  <StatBox
                    label="Expired"
                    value={prodStats?.expiredListings ?? 0}
                    accent={
                      prodStats?.expiredListings > 0 ? "text-amber-600" : ""
                    }
                  />
                  <StatBox
                    label="Total Listings"
                    value={prodStats?.totalListings ?? 0}
                  />
                </div>
                {(prodStats?.terminatedListings > 0 ||
                  prodStats?.expiredListings > 0) && (
                  <div className="rounded-md bg-amber-50 border border-amber-200 p-2 text-[10px] text-amber-800">
                    <strong>
                      {(prodStats?.terminatedListings ?? 0) +
                        (prodStats?.expiredListings ?? 0)}
                    </strong>{" "}
                    listing(s) did not sell (terminated or expired)
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                <Table>
                  <TableBody>
                    ...
                    <TableRow>
                      ...
                      <TableCell className="text-xs text-right">
                        {prodStats?.openTasks ?? 0}
                      </TableCell>
                    </TableRow>
                    {pipelineData?.pipelineByStatus &&
                      Object.entries(pipelineData.pipelineByStatus).map(
              ...
              <CardContent>
                {goalsData?.annualGoal ? (
                  <Table>
                    <TableBody>
                      <TableRow>
                        ...
                        <TableCell className="text-xs text-right">
                          {goalsData.annualGoal.targetUnits ?? "—"} units /{" "}
                          {fmtMoney(
                            Number(goalsData.annualGoal.targetVolume ?? 0)
                          )}
                        </TableCell>
                      </TableRow>
                      <TableRow>
                  ...
                  <div className="text-center py-6 text-muted-foreground">
                    <Target className="h-6 w-6 mx-auto mb-1 opacity-40" />
                    <p className="text-xs">No goals configured</p>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="p-0 overflow-x-auto">
                {(recentSessions ?? []).length === 0 ? (
                  <div className="text-center py-6 text-muted-foreground">
                    <CalendarDays className="h-6 w-6 mx-auto mb-1 opacity-40" />
                    <p className="text-xs">No coaching sessions recorded</p>
                  </div>
                ) : (
                  <Table>
                    <TableHeader>
                      ...
                      {(recentSessions ?? []).slice(0, 5).map((s: any) => (
                        ...
                        >
                          <TableCell className="text-xs">
                            {safeFormat(s.sessionDate, "MMM d")}
                          </TableCell>
                          <TableCell className="text-xs">
                            {s.actualCoachName ?? s.scheduledCoachName ?? "—"}
                          </TableCell>
                          <TableCell className="text-xs">
                            {s.sessionType ?? "—"}
                          </TableCell>
              ...
              <CardContent>
                <Table>
                  <TableBody>
                    ...
                    <TableRow>
                      <TableCell className="text-xs font-medium">
                        Avg Purchase Price (Closed)
                      </TableCell>
                      <TableCell className="text-xs text-right">
                        {fmtMoney(prodStats?.avgPurchasePrice ?? 0)}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="text-xs font-medium">
                      ...
                      <TableCell className="text-xs text-right">
                        {fmtMoney(prodStats?.avgBuyerPrice ?? 0)}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="text-xs font-medium">
                      ...
                      <TableCell className="text-xs text-right">
                        {fmtMoney(prodStats?.avgSellerPrice ?? 0)}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="text-xs font-medium">
                    ...
                    <TableRow>
                      ...
                      <TableCell className="text-xs text-right">
                        {prodStats?.annualizedPace ?? "—"} units
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="text-xs font-medium">
                    ...
                    <TableRow>
                      ...
                      <TableCell className="text-xs text-right">
                        {fmtMoney(prodStats?.ytdVolume ?? 0)}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="text-xs font-medium">
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
                  <StatBox
                    label="Terminated"
                    value={prodStats?.terminatedCount ?? 0}
                    accent="text-red-600"
                  />
                  <StatBox
                    label="Term Rate"
                  ...
                  <StatBox
                    label="Lost Volume"
                    value={fmtMoney(prodStats?.terminatedVolume ?? 0)}
                    accent="text-red-600"
                  />
                </div>
                  ...
                  <div className="space-y-2">
                        ...
                        >
                          <div className="flex justify-between items-start gap-2">
                            <span className="font-medium text-red-700">
                              {t.terminationReason || "No reason provided"}
                            </span>
                            <span className="text-[10px] text-muted-foreground whitespace-nowrap">
                              {t.closingDate
                                ? safeFormat(t.closingDate, "MMM d, yyyy")
                                : "—"}
                            </span>
                          </div>
                          <div className="flex gap-3 mt-1 text-[10px] text-muted-foreground">
                            <span>
                              {fmtMoney(Number(t.purchasePrice ?? 0))}
                            </span>
                    ...
                    {terminations.length > 5 && (
                      <Button
                        ...
                        onClick={() =>
                          setShowAllTerminations(!showAllTerminations)
                        }
                      >
                        {showAllTerminations ? (
                          <>
                          ...
                          <>
                            ...
                            Show All {terminations.length} Terminations
                          </>
                        )}
                      </Button>
                    )}
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <StatBox
                    label="Agent Avg Rate"
                    value={`${prodStats?.avgCommissionRate ?? "—"}%`}
                  />
                  <StatBox
                    label="Savvy Avg"
                    value={`${prodStats?.companyAvgRate ?? "—"}%`}
                  />
                  <StatBox
                    label="Savvy Median"
                    value={`${prodStats?.companyMedianRate ?? "—"}%`}
                  />
                  <StatBox
                    label="Difference"
                    value={
                      prodStats?.avgCommissionRate && prodStats?.companyAvgRate
                        ? `${(prodStats.avgCommissionRate - prodStats.companyAvgRate).toFixed(2)}%`
                        : "—"
                    }
                    accent={
                      prodStats?.avgCommissionRate < prodStats?.companyAvgRate
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {goalsData?.annualGoal ? (
            <div className="grid gap-4 lg:grid-cols-2">
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Annual Goal</CardTitle>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableBody>
                      <TableRow>
                        ...
                        <TableCell className="text-xs text-right">
                          {goalsData.annualGoal.targetUnits ?? "—"}
                        </TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="text-xs font-medium">
                ...
                <Card>
                  ...
                  <CardContent className="p-0 overflow-x-auto">
                    <Table>
                      ...
                      <TableBody>
                        {goalsData.monthlyGoals.map((g: any, i: number) => (
                          <TableRow key={i}>
                            <TableCell className="text-xs">
                              {g.month ?? `Month ${i + 1}`}
                            </TableCell>
                            ...
                            <TableCell className="text-xs">
                              {(g.actualUnits ?? 0) >= (g.targetUnits ?? 0) ? (
            ...
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  This agent does not have goals configured in SavvyOS.
                </p>
                <Button
                  size="sm"
                  variant="outline"
        ...
        <TabsContent value="performance" className="mt-4 space-y-4">
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              ...
              <CardContent>
                {pipelineData?.pipelineByStatus &&
                Object.keys(pipelineData.pipelineByStatus).length > 0 ? (
                  <Table>
                    <TableHeader>
                      <TableRow>
                  ...
                  <div className="text-center py-6 text-muted-foreground">
                    ...
                    <p className="text-xs">No pipeline data</p>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">Tasks & Execution</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <StatBox
                    label="Open Tasks"
                    value={prodStats?.openTasks ?? 0}
                  />
                  <StatBox
                    label="Overdue Tasks"
                    value={overdueTasks}
                  ...
                  <StatBox
                    ...
                    value={prodStats?.tasksDueToday ?? 0}
                  />
                  <StatBox
                    label="Completed (30d)"
                    value={prodStats?.tasksCompleted30d ?? 0}
                  />
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm">Full Coaching History</CardTitle>
                <Button size="sm" onClick={() => setShowNewSession(true)}>
                  <Plus className="h-3.5 w-3.5 mr-1" />
                  New Session
                </Button>
            ...
            <CardContent className="p-0 overflow-x-auto">
              {(recentSessions ?? []).length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm">No coaching sessions recorded</p>
                </div>
              ) : (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {(recentSessions ?? []).map((s: any) => (
                        ...
                        >
                          <TableCell className="text-xs">
                            {safeFormat(s.sessionDate, "MMM d, yyyy")}
                          </TableCell>
                          <TableCell className="text-xs">
                            {s.scheduledCoachName ?? "—"}
                          </TableCell>
                          <TableCell className="text-xs">
                            {s.actualCoachName ?? "—"}
                          </TableCell>
        ...
        <TabsContent value="records" className="mt-4">
          <CoachingAssessmentsPanel agentId={agentId} />
        </TabsContent>

        {/* ─── PERFORMANCE RESET (Coaching) ─── */}
        <TabsContent value="coach" className="mt-4">
          <CoachingPerformanceResetPanel agentId={agentId} />
        </TabsContent>
            ...
            <CardContent>
              {(marketAssignments ?? []).length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-xs">No market assignments found</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {(marketAssignments ?? []).map((m: any, i: number) => (
                      <TableRow key={i}>
                        <TableCell className="text-xs font-medium">
                          {m.market?.name ?? "—"}
                        </TableCell>
          ...
          {profile?.marketProtectionStatus && (
            <Card>
              <CardContent className="p-4">
                <p className="text-xs">
                  <strong>Market Protection Status:</strong>{" "}
                  {profile.marketProtectionStatus}
                </p>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs">
                Assessment reports, coaching recordings, transcripts, and
                uploaded documents
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex min-h-32 flex-col items-center justify-center rounded-lg border border-dashed bg-muted/20 px-5 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Upload assessment reports, coaching recordings, or other
                  documents.
                </p>
              </div>
            </CardContent>
          ...
          <div className="space-y-3">
            <div>
              ...
              <Select
                ...
                onValueChange={v =>
                  setSessionForm({ ...sessionForm, sessionType: v })
                }
              >
                ...
                <SelectContent>
                  {[
                    ...
                    "New-Agent Launch Session",
                    "30-Day Launch Review",
                    "60-Day Launch Review",
                    "90-Day Launch Review",
                    "Performance Reset Session",
                    "Performance Reset Checkpoint",
                    "Productive-Agent Strategy Session",
                    "Stay and Retention Conversation",
                    "Specialist Intervention",
                    "Market-Coverage Conversation",
                    "Coach-Out Conversation",
                    "Tyler Strategy Session",
            ...
            <div>
              <label className="text-xs font-medium">Session Date & Time</label>
              <Input
                type="datetime-local"
            ...
            <div>
              <label className="text-xs font-medium">Session Coach</label>
              <div className="mt-1">
                <SearchableSelect
                  options={(coaches ?? [])
                    .slice()
                    ...
                    .map((coach: any) => ({
                      ...
                      label: coach.name ?? coach.email ?? `Coach ${coach.id}`,
                      description: coach.email ?? undefined,
                    }))}
                  value={sessionForm.scheduledCoachId}
                  onValueChange={v =>
                    setSessionForm({ ...sessionForm, scheduledCoachId: v })
                  }
                  placeholder="Search for a coach..."
                  searchPlaceholder="Type a coach name..."
                />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <Select
                value={sessionForm.schedulingSource}
                onValueChange={v =>
                  setSessionForm({
                    ...sessionForm,
                    schedulingSource: v as "SavvyOS" | "External",
                  })
                }
              >
                <SelectTrigger className="mt-1">
                ...
                <SelectContent>
                  <SelectItem value="SavvyOS">Create in SavvyOS</SelectItem>
                  <SelectItem value="External">
                    Record an existing external meeting
                  </SelectItem>
                </SelectContent>
              </Select>
              <p className="mt-1 text-[11px] text-muted-foreground">
                SavvyOS creates the Zoom link and calendar event when
                integrations are connected.
              </p>
            </div>
            {sessionForm.schedulingSource === "External" && (
              <div>
                <label className="text-xs font-medium">
                ...
                <Input
                  ...
                  placeholder="Paste a Calendly, Zoom, or other meeting link"
                  className="mt-1"
                />
              </div>
            )}
            <div>
              ...
              <Textarea
                value={sessionForm.reasonForSession}
                onChange={e =>
                  setSessionForm({
                    ...sessionForm,
                    reasonForSession: e.target.value,
                  })
                }
                placeholder="Why is this session needed?"
                className="mt-1"
                rows={2}
              />
            </div>
              ...
              onClick={() =>
                createSession.mutate({
                  ...
                  sessionDate: sessionForm.sessionDate
                    ? new Date(sessionForm.sessionDate).toISOString()
                    : new Date().toISOString(),
                  scheduledCoachId: sessionForm.scheduledCoachId
                    ? Number(sessionForm.scheduledCoachId)
                    : undefined,
                  durationMinutes: Number(sessionForm.durationMinutes) || 30,
                  schedulingSource: sessionForm.schedulingSource,
                  meetingLink: sessionForm.meetingLink || undefined,
                  reasonForSession: sessionForm.reasonForSession || undefined,
                })
              }
