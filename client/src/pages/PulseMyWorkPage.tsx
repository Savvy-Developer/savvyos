...
import { CalendarClock, ChevronDown, ChevronUp, ClipboardList, History } from "lucide-react";
...
export default function PulseMyWorkPage() {
  ...
  return <main className="pulse-page pulse-page-stack"><header className="border-b border-border pb-4"><div className="grid gap-3 xl:grid-cols-[minmax(0,1.12fr)_minmax(34rem,0.88fr)] xl:items-start"><div><p className="text-sm font-medium text-primary">Pulse</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">My EOS Dashboard</h1><nav className="mt-2.5 flex flex-wrap gap-1.5" aria-label="My EOS sections"><Button asChild variant="secondary" size="sm" className="h-8"><a href="#my-measurables"><ClipboardList className="mr-1.5 h-3.5 w-3.5" />My measurables</a></Button><Button asChild variant="secondary" size="sm" className="h-8"><a href="#weekly-preparation"><ClipboardList className="mr-1.5 h-3.5 w-3.5" />Weekly preparation</a></Button></nav></div><aside className="grid items-start gap-2 sm:grid-cols-2" aria-label="Frequently used Pulse panels"><PulseNotificationsInbox meetingId={workspaceId === "all" ? undefined : workspaceId} compact collapsible defaultExpanded={false} /><NextMeetingsPanel meetings={nextMeetings} limit={2} /></aside></div></header>
    <PulseMyMeasurables />
    <section className="space-y-3"><div><h2 className="text-xl font-semibold">My work</h2><p className="mt-1 text-sm text-muted-foreground">Current commitments and issues. Use the clear work form below when you need to add a new item.</p></div><PulseL10WorkCreator meetings={data.meetings} onCreated={changed} workspaceControls={<div><p className="mb-1 text-sm font-semibold">Show work from</p><div className="flex flex-wrap gap-1">{workspaces.map((workspace: any) => <button type="button" key={workspace.id} onClick={() => setWorkspaceId(workspace.id)} className={`h-8 rounded-md border px-2.5 text-left text-xs font-semibold transition-colors ${workspaceId === workspace.id ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background hover:bg-muted"}`}>{workspace.name}</button>)}</div><p className="mt-1.5 text-xs text-muted-foreground">Choose all L10s or one meeting. Every item keeps its meeting home.</p></div>} />
