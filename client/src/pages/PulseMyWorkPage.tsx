...
export default function PulseMyWorkPage() {
  ...
  return <main className="pulse-page pulse-page-stack"><header className="border-b border-border pb-4"><div className="grid gap-3 xl:grid-cols-[minmax(0,1.12fr)_minmax(34rem,0.88fr)] xl:items-start"><div><p className="text-sm font-medium text-primary">Pulse</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">My EOS Dashboard</h1><nav className="mt-2.5 flex" aria-label="My EOS sections"><Button asChild variant="secondary" size="sm" className="h-8"><a href="#weekly-preparation"><ClipboardList className="mr-1.5 h-3.5 w-3.5" />Weekly preparation</a></Button></nav><p className="mt-2 max-w-3xl text-base leading-6 text-muted-foreground">Your active work, meeting preparation, and notifications remain connected to their original Pulse forum.</p></div><aside className="grid gap-2 sm:grid-cols-2" aria-label="Frequently used Pulse panels"><PulseNotificationsInbox meetingId={workspaceId === "all" ? undefined : workspaceId} compact /><NextMeetingsPanel meetings={nextMeetings} limit={2} /></aside></div></header>
