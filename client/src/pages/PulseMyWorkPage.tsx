...
function UpcomingMeetingChip({ meeting }: { meeting?: any }) {
  if (!meeting) return null;
  return <div className="flex min-w-0 items-center gap-2 rounded-lg border border-primary/20 bg-primary/[0.018] px-2.5 py-2"><CalendarClock className="h-4 w-4 shrink-0 text-primary" /><div className="min-w-0"><p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Up next</p><a href={`/pulse/meetings/${meeting.id}`} className="block truncate text-sm font-semibold hover:text-primary hover:underline">{meeting.name}</a><p className="truncate text-xs text-muted-foreground">{formatMeetingDateTime(meeting.nextOccursAt, meeting.timezone)}</p></div>{meeting.canRun ? <Button asChild size="sm" className="ml-1 h-8 shrink-0 text-xs"><a href={`/pulse/meetings/${meeting.id}/run`}>Run</a></Button> : null}</div>;
}
...
export default function PulseMyWorkPage() {
  ...
  return <main className="pulse-page pulse-page-stack"><header className="border-b border-border pb-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-primary">Pulse</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">My EOS Dashboard</h1><nav className="mt-2 flex flex-wrap gap-1.5" aria-label="My EOS sections"><Button asChild variant="secondary" size="sm" className="h-8"><a href="#my-measurables"><ClipboardList className="mr-1.5 h-3.5 w-3.5" />My measurables</a></Button><Button asChild variant="secondary" size="sm" className="h-8"><a href="#weekly-preparation"><ClipboardList className="mr-1.5 h-3.5 w-3.5" />Weekly preparation</a></Button></nav></div><aside className="flex max-w-full flex-wrap items-start gap-2 xl:justify-end" aria-label="Frequently used Pulse panels"><UpcomingMeetingChip meeting={nextMeetings[0]} /><PulseNotificationsInbox meetingId={workspaceId === "all" ? undefined : workspaceId} compact collapsible defaultExpanded={false} /></aside></div></header>
    <PulseMyMeasurables />
