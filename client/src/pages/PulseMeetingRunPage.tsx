...
function TodosStep({ data, onChanged }: { data: any; onChanged: () => void }) {
  const status = trpc.pulse.l10.setTodoStatus.useMutation({ onSuccess: onChanged, onError: (error) => toast.error(error.message) });
  return <div className="space-y-2">{data.sections.todos.length ? data.sections.todos.map((todo: any) => <div key={todo.id} className="flex items-center gap-3 rounded-lg border border-border p-4"><button type="button" aria-label={`Mark ${todo.title} ${todo.status === "done" ? "open" : "done"}`} onClick={() => status.mutate({ meetingId: data.meeting.id, workItemId: todo.id, status: todo.status === "done" ? "open" : "done" })} className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border ${todo.status === "done" ? "border-emerald-500 bg-emerald-500 text-white" : "border-muted-foreground"}`}>{todo.status === "done" ? <Check className="h-4 w-4"/> : null}</button><div className="min-w-0 flex-1"><p className={todo.status === "done" ? "font-medium text-muted-foreground line-through" : "font-medium"}>{todo.title}</p><p className="mt-1 text-sm text-muted-foreground">{todo.assigneeName ?? "Unassigned"} · {todo.status === "done" ? "Done" : "Not done"}</p></div></div>) : <p className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">No To-Dos are open.</p>}</div>;
}

function IdsStep({ data, sessionId, onChanged }: { data: any; sessionId: string; onChanged: () => void }) {
  const [selectedIssue, setSelectedIssue] = useState<string | null>(null);
  const [solution, setSolution] = useState("");
  const [commitment, setCommitment] = useState("");
  const resolve = trpc.pulse.l10.resolveIssue.useMutation({ onSuccess: () => { setSolution(""); setCommitment(""); setSelectedIssue(null); onChanged(); }, onError: (error) => toast.error(error.message) });
  const issues = data.sections.issues.filter((issue: any) => issue.status !== "solved");
  const selected = issues.find((issue: any) => issue.id === selectedIssue);
  return <div className="space-y-4"><div className="grid gap-3 md:grid-cols-2">{issues.length ? issues.map((issue: any, index: number) => <button type="button" key={issue.id} onClick={() => setSelectedIssue(issue.id)} className={`rounded-xl border p-4 text-left transition-colors ${selectedIssue === issue.id ? "border-primary bg-primary/5" : "border-border hover:bg-muted/60"}`}><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Priority {index + 1}</p><p className="mt-1 font-semibold">{issue.title}</p><p className="mt-2 text-sm text-muted-foreground">{issue.assigneeName ?? "L10 participant"}</p></button>) : <p className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">No open Issues. Add any concern from the dashboard before moving on.</p>}</div>{selected ? <Card className="border-primary/25"><CardHeader><CardTitle className="text-lg">Solve: {selected.title}</CardTitle><CardDescription>Capture the real solution. Add a commitment only if it makes the solution real.</CardDescription></CardHeader><CardContent className="space-y-3"><Textarea className="min-h-24" value={solution} onChange={(event) => setSolution(event.target.value)} placeholder="What did the team decide?"/><Input value={commitment} onChange={(event) => setCommitment(event.target.value)} placeholder="Optional new To-Do commitment"/><Button disabled={!solution.trim() || resolve.isPending} onClick={() => resolve.mutate({ meetingId: data.meeting.id, sessionId, workItemId: selected.id, solvedNote: solution.trim(), commitment: commitment.trim() ? { title: commitment.trim(), assigneeId: selected.assigneeId ?? data.members[0]?.id } : undefined })}>{resolve.isPending ? "Saving…" : "Mark solved"}</Button></CardContent></Card> : null}</div>;
}
