...
function TodosStep({ data, sessionId, onCreate, onChanged }: { data: any; sessionId: string; onCreate: (type: "todo" | "issue") => void; onChanged: () => void }) {
  return <div className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Expand a commitment without leaving the running L10 to update status, details, documents, links, and comments.</p><Button type="button" onClick={() => onCreate("todo")}>Add To-Do</Button></div>{data.sections.todos.filter((todo: any) => !todo.parentWorkItemId).length ? data.sections.todos.filter((todo: any) => !todo.parentWorkItemId).map((todo: any) => <PulseInlineItemRow key={todo.id} item={todo} defaultDestinationId={data.meeting.id} sourceSessionId={sessionId} onChanged={onChanged} />) : <p className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">No To-Dos are open.</p>}</div>;
}
