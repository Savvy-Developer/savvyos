    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-slate-100 px-4">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-900 font-medium">Invalid reset link</p>
          <p className="text-slate-500 text-sm">This link is missing a reset token. Please request a new one.</p>
          <Link href="/forgot-password">
            <Button className="bg-cyan-500 hover:bg-cyan-600 text-white font-semibold">Request new link</Button>
          </Link>
        </div>
      </div>
    );
      ...
      <div className="relative w-full max-w-md">
        ...
        <Card className="border-slate-200 bg-white shadow-xl">
          <CardHeader className="space-y-1 pb-4">
            <CardTitle className="text-xl font-semibold text-slate-900 text-center">Set new password</CardTitle>
          ...
          <CardContent>
            {done ? (
              <div className="text-center space-y-4 py-2">
                ...
                <p className="text-slate-900 font-medium">Password updated!</p>
                <p className="text-slate-500 text-sm">Your password has been changed. Redirecting you to sign in…</p>
                <Link href="/login">
                  <Button className="bg-cyan-500 hover:bg-cyan-600 text-white font-semibold">Sign in now</Button>
                </Link>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                {resetMut.error && (
                  <Alert variant="destructive" className="border-red-200 bg-red-50">
                    <AlertDescription className="text-red-700 text-sm">{resetMut.error.message}</AlertDescription>
                  </Alert>
                )}

                <div className="space-y-1.5">
                  <Label htmlFor="password" className="text-slate-700 text-sm">New password</Label>
                  ...
                  {tooShort && <p className="text-xs text-red-600">Password must be at least 8 characters</p>}
                </div>

                <div className="space-y-1.5">
                  <Label htmlFor="confirm" className="text-slate-700 text-sm">Confirm password</Label>
                  <Input
                    id="confirm"
                    type={showPassword ? "text" : "password"}
                    autoComplete="new-password"
