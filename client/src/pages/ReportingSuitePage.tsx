...
function ColumnShare({ value, total }: { value: unknown; total: unknown }) {
  ...
  return <span className="text-xs font-medium text-muted-foreground">{share === null ? "—" : percentage(share)} of total</span>;
}
...
function AgentReport({ data }: { data: any }) {
  ...
  return <div className="space-y-7">
    ...
    <section className="grid gap-5 xl:grid-cols-[1.45fr_0.95fr]">
      <Card>
        <CardHeader><CardTitle className="text-base">Production momentum</CardTitle><CardDescription>Monthly closed volume and closings in the selected date range.</CardDescription></CardHeader>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Financial contribution</CardTitle><CardDescription>Gross commission and recorded Savvy net by month.</CardDescription></CardHeader>
        <CardContent>{monthly.length ? <ResponsiveContainer width="100%" height={280}><LineChart data={monthly.map((row: any) => ({ ...row, label: monthLabel(row.month) }))} margin={{ left: 0, right: 12, top: 6, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" tickLine={false} axisLine={false} fontSize={11} /><YAxis tickFormatter={(value) => money(value, true)} tickLine={false} axisLine={false} width={62} fontSize={11} /><Tooltip formatter={(value: number) => money(value)} /><Legend /><Line type="monotone" dataKey="grossCommission" name="Gross commission" stroke="#4338CA" strokeWidth={2.5} dot={false} /><Line type="monotone" dataKey="savvyNet" name="Savvy net" stroke="#1F6D5B" strokeWidth={2.5} dot={false} /></LineChart></ResponsiveContainer> : <ChartEmpty label="monthly financial data" />}</CardContent>
      </Card>
    </section>

    <section className="space-y-3">
      <SectionHeader title="Agent comparison" description="Ranked production and current follow-through signals for the selected scope. Each percentage is the agent’s share of the metric column total." />
      <Card>
        <CardContent className="p-0 overflow-x-auto">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1120px] text-sm">
              ...
              <tbody>{agents.map((agent: any) => {
                const flagCount = Number(agent.commissionFlags ?? 0) + Number(agent.pastExpectedCloseDate ?? 0) + Number(agent.noExpectedCloseDate ?? 0);
                return <tr key={agent.agentId} className="border-b last:border-0 hover:bg-muted/25">
