...
function TransactionReport({ data, update }: { data: any; update: (patch: QueryPatch) => void }) {
  ...
  const monthlyDescription = isTerminationView ? "Terminated transactions by month in the selected date range." : "Volume, units, gross commission, and Savvy net over the selected period.";
  const outcomeRecordsUrl = buildTransactionUrl(filters, { status: "closed" });
  ...
  return <div className="space-y-7"><section className="space-y-3"><SectionHeader title="Transaction performance" description={(filters.dateBasis === "contract" ? "Contract-date" : "Closing-date") + " performance for the selected period. All deltas compare with the immediately preceding period of equal length."} /><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Period units" value={number(summary.units)} description="Transactions in the selected period" delta={summary.change?.units} icon={BriefcaseBusiness} tone="text-sky-700" /><MetricCard label="Closed" value={number(summary.closedUnits)} description="Closed transactions in the selected period" delta={summary.change?.closings} icon={CheckCircle2} tone="text-emerald-700" /><MetricCard label="Volume" value={money(summary.volume, true)} description="Purchase volume in the selected period" delta={summary.change?.volume} icon={Landmark} tone="text-sky-700" /><MetricCard label="Gross commission" value={money(summary.grossCommission, true)} description="Recorded transaction GCI" delta={summary.change?.grossCommission} icon={CircleDollarSign} tone="text-indigo-700" /><MetricCard label="Savvy net" value={money(summary.savvyNet, true)} description="Recorded Savvy payout items" delta={summary.change?.savvyNet} icon={TrendingUp} tone="text-primary" /><MetricCard label="Avg. GCI" value={money(summary.averageGci, true)} description="Per selected-period transaction" delta={summary.change?.averageGci} icon={BarChart3} tone="text-violet-700" /><MetricCard label="Days to close" value={summary.averageDaysToClose === null ? "—" : String(Math.round(summary.averageDaysToClose))} description="Average contract to close" delta={summary.change?.averageDaysToClose} icon={CalendarClock} tone="text-amber-700" /><MetricCard label="Current under contract" value={number(pipeline?.units)} description="Live pipeline snapshot; not date-filtered" icon={BriefcaseBusiness} tone="text-sky-700" /></div></section><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><FlagCard label="Termination rate" value={summary.terminationRate ?? 0} displayValue={summary.terminationRate === null ? "—" : percentage(summary.terminationRate)} description={summary.terminationRate === null ? "Available when all statuses are selected" : "Terminated ÷ closed plus terminated"} icon={TrendingUp} href={buildTransactionUrl(filters, { status: "terminated" })} tone="rose" /><FlagCard label="Commission flags" value={flags.commissionFlags} description="Current review flags; not date-filtered" icon={Flag} href={buildTransactionUrl(filters, { flagPayoutIntegrity: "true" })} tone="violet" /><FlagCard label="Past expected close" value={flags.pastExpectedCloseDate} description="Current under-contract pipeline; not date-filtered" icon={CalendarClock} href={buildTransactionUrl(filters, { status: "under_contract", flagPastClosingDate: "true" })} tone="rose" /><FlagCard label="No expected close" value={flags.noExpectedCloseDate} description="Current under-contract pipeline; not date-filtered" icon={AlertTriangle} href={buildTransactionUrl(filters, { status: "under_contract", flagNoClosingDate: "true" })} tone="amber" /></section><section className="grid gap-5 xl:grid-cols-[1.45fr_0.95fr]"><Card><CardHeader><CardTitle className="text-base">{monthlyTitle}</CardTitle><CardDescription>{monthlyDescription}</CardDescription></CardHeader><CardContent>{monthly.length ? <ResponsiveContainer width="100%" height={300}><LineChart data={monthly.map((row: any) => ({ ...row, label: monthLabel(row.month) }))} margin={{ left: 0, right: 10, top: 6, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" tickLine={false} axisLine={false} fontSize={11} /><YAxis yAxisId="money" tickFormatter={(value) => money(value, true)} tickLine={false} axisLine={false} width={60} fontSize={11} /><YAxis yAxisId="units" orientation="right" tickLine={false} axisLine={false} width={26} fontSize={11} /><Tooltip formatter={(value: number, key: string) => key === "units" || key === "closings" ? number(value) : money(value)} /><Legend /><Line yAxisId="money" type="monotone" dataKey="grossCommission" name="Gross commission" stroke="#4338CA" strokeWidth={2.5} dot={false} /><Line yAxisId="money" type="monotone" dataKey="savvyNet" name="Savvy net" stroke="#1F6D5B" strokeWidth={2.5} dot={false} /><Line yAxisId="units" type="monotone" dataKey="units" name="Units" stroke="#D97706" strokeWidth={2.25} dot={{ r: 2 }} /></LineChart></ResponsiveContainer> : <ChartEmpty label="monthly transaction data" />}</CardContent></Card><Card><CardHeader><CardTitle className="text-base">Representation mix</CardTitle><CardDescription>Buyer, seller, and dual-agency units in the selected period.</CardDescription></CardHeader><CardContent>{transactionTypes.length ? <ResponsiveContainer width="100%" height={300}><PieChart><Pie data={transactionTypes.map((row: any) => ({ ...row, label: titleCase(row.transactionType) }))} dataKey="units" nameKey="label" cx="50%" cy="46%" outerRadius={86} innerRadius={52} paddingAngle={3}>{transactionTypes.map((_row: any, index: number) => <Cell key={index} fill={chartColors[index]} />)}</Pie><Tooltip formatter={(value: number) => number(value)} /><Legend verticalAlign="bottom" iconType="circle" /></PieChart></ResponsiveContainer> : <ChartEmpty label="representation mix" />}</CardContent></Card></section><section className="grid gap-5 xl:grid-cols-2"><Card><CardHeader><CardTitle className="text-base">Period outcomes</CardTitle><CardDescription>Closed and terminated activity is constrained to the selected date range. The separate pipeline figure is live.</CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground"><th className="px-4 py-3 text-left">Status</th><th className="px-3 py-3 text-right">Units</th><th className="px-3 py-3 text-right">Volume</th><th className="px-3 py-3 text-right">GCI</th><th className="px-4 py-3 text-right">Savvy net</th></tr></thead><tbody>{statuses.map((row: any) => <tr key={row.status} className="border-b last:border-0"><td className="px-4 py-3"><Badge variant="outline" className={statusColors[row.status] ?? ""}>{titleCase(row.status)}</Badge></td><td className="px-3 py-3 text-right tabular-nums">{number(row.units)}</td><td className="px-3 py-3 text-right tabular-nums">{money(row.volume, true)}</td><td className="px-3 py-3 text-right tabular-nums">{money(row.grossCommission, true)}</td><td className="px-4 py-3 text-right tabular-nums">{money(row.savvyNet, true)}</td></tr>)}</tbody></table></div><div className="border-t bg-sky-50/55 px-4 py-3"><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-semibold text-sky-900">Current under-contract pipeline</p><p className="text-xs text-sky-800">Live status inventory, intentionally independent of the selected date range.</p></div><div className="text-right"><p className="text-lg font-semibold text-sky-900">{number(pipeline?.units)} units</p><p className="text-xs text-sky-800">{money(pipeline?.grossCommission, true)} GCI</p></div></div></div></CardContent></Card><Card><CardHeader><CardTitle className="text-base">Representation contribution</CardTitle><CardDescription>Contribution by representation and outcome status, including GCI share.</CardDescription></CardHeader><CardContent className="p-0"><div className="max-h-[360px] overflow-auto"><table className="w-full text-sm"><thead className="sticky top-0 bg-muted/95"><tr className="border-b text-xs uppercase tracking-wide text-muted-foreground"><th className="px-4 py-3 text-left">Status</th><th className="px-3 py-3 text-left">Representation</th><th className="px-3 py-3 text-right">Units</th><th className="px-3 py-3 text-right">GCI</th><th className="px-4 py-3 text-right">GCI share</th></tr></thead><tbody>{representationByStatus.map((row: any, index: number) => <tr key={row.status + "-" + row.transactionType + "-" + index} className="border-b last:border-0"><td className="px-4 py-3"><Badge variant="outline" className={statusColors[row.status] ?? ""}>{titleCase(row.status)}</Badge></td><td className="px-3 py-3">{titleCase(row.transactionType)}</td><td className="px-3 py-3 text-right tabular-nums">{number(row.units)}</td><td className="px-3 py-3 text-right tabular-nums">{money(row.grossCommission, true)}</td><td className="px-4 py-3 text-right font-medium tabular-nums">{contributionTotal ? percentage((Number(row.grossCommission) / contributionTotal) * 100) : "—"}</td></tr>)}{!representationByStatus.length && <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-muted-foreground">No representation contribution is available for this scope.</td></tr>}</tbody></table></div></CardContent></Card></section><section className="space-y-3"><SectionHeader title={isTerminationView ? "Terminations by agent" : "Outcomes by agent"} description={isTerminationView ? "Date-scoped terminations ranked by agent. Select an agent to open the exact underlying records." : "Date-scoped closed and terminated outcomes by agent, with direct source-record access."} /><Card><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground"><th className="px-4 py-3 text-left">Agent</th><th className="px-3 py-3 text-right">Units</th><th className="px-3 py-3 text-right">Closed</th><th className="px-3 py-3 text-right">Terminated</th><th className="px-3 py-3 text-right">GCI</th><th className="px-4 py-3 text-right">Savvy net</th></tr></thead><tbody>{agentOutcomes.map((agent: any) => <tr key={String(agent.agentId) + agent.agentName} className="border-b last:border-0 hover:bg-muted/25"><td className="px-4 py-3 font-medium">{agent.agentId ? <a href={buildTransactionUrl(filters, { agentId: String(agent.agentId) })} className="text-primary hover:underline">{agent.agentName}</a> : agent.agentName}</td><td className="px-3 py-3 text-right tabular-nums">{number(agent.units)}</td><td className="px-3 py-3 text-right tabular-nums">{number(agent.closings)}</td><td className="px-3 py-3 text-right font-medium tabular-nums">{number(agent.terminations)}</td><td className="px-3 py-3 text-right tabular-nums">{money(agent.grossCommission, true)}</td><td className="px-4 py-3 text-right tabular-nums">{money(agent.savvyNet, true)}</td></tr>)}{!agentOutcomes.length && <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-muted-foreground">No agent outcomes are available for this scope.</td></tr>}</tbody></table></div></CardContent></Card></section><section className="space-y-3"><SectionHeader title="Focused transaction views" description="Open a deliberately narrowed report for terminations, representation, production, or review flags." /><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><a href={deepLink({ status: "terminated", page: null })}><Card className="h-full transition hover:-translate-y-0.5 hover:shadow-sm"><CardContent className="p-4"><p className="text-sm font-semibold">Termination analysis</p><p className="mt-1 text-xs text-muted-foreground">Monthly trend and terminated source records.</p></CardContent></Card></a><a href={deepLink({ status: "all", transactionType: "buyer", page: null })}><Card className="h-full transition hover:-translate-y-0.5 hover:shadow-sm"><CardContent className="p-4"><p className="text-sm font-semibold">Representation analysis</p><p className="mt-1 text-xs text-muted-foreground">Buyer, seller, and dual contribution views.</p></CardContent></Card></a><a href={deepLink({ status: "closed", page: null })}><Card className="h-full transition hover:-translate-y-0.5 hover:shadow-sm"><CardContent className="p-4"><p className="text-sm font-semibold">Monthly production</p><p className="mt-1 text-xs text-muted-foreground">Closed production, GCI, Savvy net, and per-agent evidence.</p></CardContent></Card></a><a href={deepLink({ status: "under_contract", page: null })}><Card className="h-full transition hover:-translate-y-0.5 hover:shadow-sm"><CardContent className="p-4"><p className="text-sm font-semibold">Commission & close flags</p><p className="mt-1 text-xs text-muted-foreground">Live pipeline review and payout integrity flags.</p></CardContent></Card></a></div></section><section className="space-y-3"><SectionHeader title="Transaction evidence" description="Page through the individual source records behind the selected period metrics and filters." action={<Badge variant="secondary">{number(pagination.total)} records</Badge>} /><Card><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-sm"><thead><tr className="border-b bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground"><th className="px-4 py-3 text-left">Client / transaction</th><th className="px-3 py-3 text-left">Agent</th><th className="px-3 py-3 text-left">Type</th><th className="px-3 py-3 text-left">Status</th><th className="px-3 py-3 text-left">Dates</th><th className="px-3 py-3 text-right">Volume</th><th className="px-3 py-3 text-right">GCI</th><th className="px-3 py-3 text-right">Savvy net</th><th className="px-4 py-3 text-left">Flags</th></tr></thead><tbody>{evidence.map((tx: any) => <tr key={tx.transactionId} className="border-b last:border-0 hover:bg-muted/25"><td className="px-4 py-3"><a href={"/transactions/" + tx.transactionId} className="font-semibold text-primary hover:underline">{tx.contactName}</a><p className="mt-0.5 max-w-[220px] truncate text-xs text-muted-foreground">{tx.propertyAddress ?? tx.transactionNumber ?? "No property address"}</p></td><td className="px-3 py-3">{tx.agentName}</td><td className="px-3 py-3"><Badge variant="outline" className={statusColors[tx.transactionType] ?? ""}>{titleCase(tx.transactionType)}</Badge></td><td className="px-3 py-3"><Badge variant="outline" className={statusColors[tx.status] ?? ""}>{titleCase(tx.status)}</Badge></td><td className="px-3 py-3 text-xs"><p>Contract: {day(tx.contractDate)}</p><p className="mt-1">Close: {day(tx.closingDate)}</p></td><td className="px-3 py-3 text-right tabular-nums">{money(tx.volume, true)}</td><td className="px-3 py-3 text-right font-medium tabular-nums">{money(tx.grossCommission, true)}</td><td className="px-3 py-3 text-right tabular-nums">{money(tx.savvyNet, true)}</td><td className="px-4 py-3"><div className="flex max-w-[200px] flex-wrap gap-1">{tx.commissionFlag && <Badge variant="outline" className="border-violet-200 bg-violet-50 text-violet-700">Commission</Badge>}{tx.pastExpectedCloseDate && <Badge variant="outline" className="border-rose-200 bg-rose-50 text-rose-700">Past close</Badge>}{tx.missingExpectedCloseDate && <Badge variant="outline" className="border-amber-200 bg-amber-50 text-amber-700">No close date</Badge>}{!tx.commissionFlag && !tx.pastExpectedCloseDate && !tx.missingExpectedCloseDate && <span className="text-xs text-muted-foreground">—</span>}</div></td></tr>)}</tbody></table></div>{!evidence.length && <p className="py-10 text-center text-sm text-muted-foreground">No transactions match this scope.</p>}<div className="flex items-center justify-between border-t px-4 py-3"><p className="text-xs text-muted-foreground">Page {pagination.page} of {pagination.totalPages}</p><div className="flex gap-2"><Button variant="outline" size="sm" disabled={pagination.page <= 1} onClick={() => update({ page: String(pagination.page - 1) })}><ChevronLeft className="mr-1 h-3.5 w-3.5" />Previous</Button><Button variant="outline" size="sm" disabled={pagination.page >= pagination.totalPages} onClick={() => update({ page: String(pagination.page + 1) })}>Next<ChevronRight className="ml-1 h-3.5 w-3.5" /></Button></div></div></CardContent></Card></section></div>;
}
