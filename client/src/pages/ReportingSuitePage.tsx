  ...
  return <div className="space-y-7">
    ...
    <section className="grid gap-5 xl:grid-cols-[1.45fr_0.95fr]">
      <Card>
        ...
        <CardContent>{monthly.length ? <ResponsiveContainer width="100%" height={300}><LineChart data={monthly.map((row: any) => ({ ...row, label: monthLabel(row.month) }))} margin={{ left: 0, right: 10, top: 6, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" tickLine={false} axisLine={false} fontSize={11} /><YAxis yAxisId="money" tickFormatter={(value) => money(value, true)} tickLine={false} axisLine={false} width={60} fontSize={11} /><YAxis yAxisId="units" orientation="right" tickLine={false} axisLine={false} width={26} fontSize={11} /><Tooltip formatter={(value: number, key: string) => key === "units" || key === "closings" || key === "underContract" ? number(value) : money(value)} /><Legend /><Line yAxisId="money" type="monotone" dataKey="grossCommission" name="Gross commission" stroke="#4338CA" strokeWidth={2.5} dot={false} /><Line yAxisId="money" type="monotone" dataKey="savvyNet" name="Savvy net" stroke="#1F6D5B" strokeWidth={2.5} dot={false} /><Line yAxisId="money" type="monotone" dataKey="futureGci" name="Under Contract GCI" stroke="#8B5CF6" strokeWidth={2.25} strokeDasharray="6 4" dot={false} /><Line yAxisId="money" type="monotone" dataKey="futureSavvyNet" name="Under Contract Savvy net" stroke="#0EA5A4" strokeWidth={2.25} strokeDasharray="6 4" dot={false} /><Line yAxisId="units" type="monotone" dataKey="units" name="Units" stroke="#D97706" strokeWidth={2.25} dot={{ r: 2 }} /><Line yAxisId="units" type="monotone" dataKey="underContract" name="Under Contract" stroke="#8B5CF6" strokeWidth={2.25} strokeDasharray="6 4" dot={{ r: 2 }} /></LineChart></ResponsiveContainer> : <ChartEmpty label="monthly transaction data" />}</CardContent>
      </Card>
      <Card>
