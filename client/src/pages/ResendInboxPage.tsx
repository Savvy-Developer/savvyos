...
export default function ResendInboxPage() {
  ...
  } = trpc.resendInbox.list.useQuery(
  ...
  const filteredThreads = useMemo(
    () =>
      (threads as InboxThread[]).filter(thread => {
        const haystack =
          `${thread.subject} ${thread.participantEmail} ${thread.receivedAddress}`.toLowerCase();
  ...
  const { data: conversation, isLoading: threadLoading } =
    trpc.resendInbox.getThread.useQuery(
  ...
  useEffect(() => {
    ...
    utils.resendInbox.list,
    utils.resendInbox.unreadCount,
  ]);
  ...
  const finishThread = trpc.resendInbox.finish.useMutation({
  ...
  async function syncAllResendHistory() {
    ...
    try {
      while (pages < maxPagesPerRun) {
        ...
        if (!result.hasMore) {
          toast.success(
            `Resend history synced: ${stored} added across ${pages} page${pages === 1 ? "" : "s"}`
          );
          break;
        }
        if (!result.nextCursor || seenCursors.has(result.nextCursor)) {
          throw new Error(
            "Resend returned an invalid pagination cursor. Please try Sync again."
          );
        }
      ...
      if (pages >= maxPagesPerRun) {
        toast.warning(
          `Imported ${pages * 100} messages or more. Click Sync Resend History again to continue the remaining history.`
        );
      }
      ...
      if (
        ...
        !/Resend returned an invalid pagination cursor/.test(error.message)
      ) {
  ...
  return (
    <div className="-m-4 flex h-[calc(100vh-56px)] min-w-0 flex-col bg-background md:-m-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Incoming replies and conversations received through Resend.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isSyncingHistory && syncProgress
              ? `Syncing ${syncProgress.scanned} emails…`
              : "Sync Resend History"}
          </Button>
          <Button
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        <aside
          className={`flex w-full shrink-0 flex-col border-r bg-card md:w-[360px] ${mobileDetail ? "hidden md:flex" : "flex"}`}
        >
            ...
            {listLoading ? (
              ...
              <div className="px-7 py-16 text-center text-muted-foreground">
                ...
                <p className="mt-1 text-xs">
                  {showArchived
                    ? "Archived email conversations appear here."
                    : "Use Sync Resend History to import every message already received."}
                </p>
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <span
                        className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${thread.isUnread ? "bg-red-500" : "bg-transparent"}`}
                      />
                      <div className="min-w-0 flex-1">
                        ...
                        <p
                          className={`mt-0.5 truncate text-sm ${thread.isUnread ? "font-semibold" : ""}`}
                        >
                          {thread.subject}
                        </p>
                        <p className="mt-0.5 truncate text-xs text-muted-foreground">
                          Received by {thread.receivedAddress}
                        </p>
                      </div>
                    </div>
                  </button>
        ...
        <main
          className={`min-w-0 flex-1 flex-col overflow-hidden ${mobileDetail ? "flex" : "hidden md:flex"}`}
        >
          {!selectedThreadId ? (
              ...
              <div className="flex flex-wrap items-center gap-2 border-b bg-card px-4 py-3 md:px-6">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    To {selectedThread.receivedAddress} ·{" "}
                    {selectedThread.participantEmail}
                  </p>
                  {selectedThreadPreview?.awaitingReply && (
                    <p className="mt-1 flex items-center gap-1 text-xs font-medium text-amber-700">
                      <AlertTriangle className="h-3.5 w-3.5" /> Read but{" "}
                      {awaitingReplyLabel(
                        selectedThreadPreview.awaitingReplySince
                      ).toLowerCase()}
                    </p>
                  )}
                </div>
                <Button
                  ...
                  onClick={() =>
                    setUnread.mutate({
                      ...
                      markedUnread: !selectedThread.isUnread,
                    })
                  }
                ...
                {!showArchived && !selectedThread.isFinished && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
                  ...
                  >
                    <CheckCheck className="mr-1.5 h-3.5 w-3.5" /> Finish
                  </Button>
                )}
                {selectedThread.contact && (
                  <Button asChild variant="outline" size="sm">
                    <Link href={`/contacts/${selectedThread.contact.id}`}>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto bg-muted/20 px-4 py-5 md:px-6">
                <div className="mx-auto max-w-4xl space-y-4">
                  {messages.map(message => (
                    <article
                      key={message.id}
                      className={`overflow-hidden rounded-xl border bg-card shadow-sm ${message.direction === "outbound" ? "border-cyan-200" : "border-border"}`}
                    >
                      <div
                        className={`flex flex-wrap items-center gap-x-3 gap-y-1 border-b px-4 py-3 ${message.direction === "outbound" ? "bg-cyan-50/70" : "bg-muted/30"}`}
                      >
                        <div className="flex min-w-0 flex-1 items-center gap-2">
                          <div
                            className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${message.direction === "outbound" ? "bg-cyan-600 text-white" : "bg-muted text-muted-foreground"}`}
                          >
                      ...
                      <div className="p-4">
                        <iframe
                          ...
                          className="min-h-[120px] w-full border-0"
                        />
                  ...
                  {!showArchived && (
                    <section className="rounded-xl border border-cyan-200 bg-card shadow-sm">
                      <div className="flex items-center justify-between border-b bg-cyan-50/70 px-4 py-3">
                        ...
                        <span className="text-xs text-muted-foreground">
                          From {selectedThread.receivedAddress}
                        </span>
                      </div>
                      {!showReply ? (
                        <div className="p-4">
                          <Button onClick={() => setShowReply(true)}>
                            <Reply className="mr-1.5 h-4 w-4" /> Compose reply
                          </Button>
                        </div>
                      ) : (
                        <div className="p-4">
                          ...
                          <div className="mt-3 flex items-center justify-between gap-3">
                            <p className="text-xs text-muted-foreground">
                              Your reply will stay in this email thread and be
                              sent from the address that received it.
                            </p>
                            <div className="flex shrink-0 gap-2">
                              ...
                              <Button
                                ...
                                disabled={!replyReady || sendReply.isPending}
                              >
                                {sendReply.isPending ? (
                                  <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" />
                                ) : (
