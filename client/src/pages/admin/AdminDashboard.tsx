  ...
  return (
    <TooltipProvider delayDuration={150}>
      <div className="space-y-7 pb-8">
        ...
        <Card className="sticky top-2 z-10 border-slate-200 bg-white/95 shadow-sm backdrop-blur">
        ...
        {isLoading ? <DashboardSkeleton /> : <>
          ...
          <section className="grid gap-5 2xl:grid-cols-[1.2fr_0.8fr]">
            <Card className="border-slate-200 shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-base">Revenue trend and scheduled production</CardTitle><CardDescription>Closed GCI by closing month; exact values are available in Transaction Reporting.</CardDescription></CardHeader><CardContent>{trend.length ? <ResponsiveContainer width="100%" height={260}><LineChart data={trend} margin={{ left: 8, right: 16, top: 10 }}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" /><XAxis dataKey="period" tick={{ fontSize: 11, fill: "#64748b" }} tickLine={false} axisLine={false} /><YAxis tickFormatter={(value) => currency(value, true)} tick={{ fontSize: 11, fill: "#64748b" }} tickLine={false} axisLine={false} width={68} /><Tooltip formatter={(value) => currency(value)} /><Line type="monotone" dataKey="gci" name="Closed GCI" stroke="#0f766e" strokeWidth={3} dot={{ r: 3, fill: "#0f766e" }} /></LineChart></ResponsiveContainer> : <EmptySection message="No closed production matches this view." />}</CardContent></Card>
