...
function contactName(thread: MarketingThread) {
  const name = `${thread.contactFirstName ?? ""} ${thread.contactLastName ?? ""}`.trim();
  return name || thread.contactPhone || thread.fromNumber || thread.toNumber || "Unmatched number";
}
...
export default function MarketingTextInboxPage() {
  ...
  const availableNumbers = trpc.marketingTextInbox.listAvailableNumbers.useQuery(undefined, {
  ...
  const selectNumber = trpc.marketingTextInbox.selectMarketingNumber.useMutation({
    onSuccess: (line) => {
      toast.success(`Marketing text line set to ${line.digits || line.name || "the selected Aircall number"}.`);
      setSelectedNumberId("");
  ...
  return (
    <div className="mx-auto flex h-full max-w-[1600px] flex-col gap-5 p-4 md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2"><Inbox className="h-6 w-6 text-primary" /><h1 className="text-2xl font-semibold tracking-tight">Marketing Text Inbox</h1></div>
          <p className="mt-1 text-sm text-muted-foreground">Replies and outbound messages for the dedicated Smart Plan marketing line.</p>
        </div>
        <Button variant="outline" onClick={() => { void configuration.refetch(); void threadsQuery.refetch(); }} disabled={configuration.isFetching || threadsQuery.isFetching}>
          <RefreshCw className={`mr-1.5 h-4 w-4 ${(configuration.isFetching || threadsQuery.isFetching) ? "animate-spin" : ""}`} /> Refresh
        </Button>
      </div>

      {!config?.sendReady && (
        <Card className="border-amber-200 bg-amber-50/50">
          <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base text-amber-950"><Settings2 className="h-5 w-5" /> Complete marketing text setup</CardTitle><CardDescription className="text-amber-900/80">Smart Plan text delivery stays blocked until the dedicated Aircall number is configured.</CardDescription></CardHeader>
          <CardContent className="space-y-4">
            {!config?.apiConfigured ? <div className="flex gap-3 rounded-md border border-amber-200 bg-white/60 p-3 text-sm text-amber-950"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><p><strong>Aircall credentials are not connected yet.</strong> Add the Aircall API ID and token to SavvyOS’s production configuration, then return here to select a dedicated SMS-enabled number.</p></div> : <>
              <div className="space-y-2"><Label htmlFor="marketing-number">Dedicated Aircall marketing number</Label><select id="marketing-number" value={selectedNumberId} onChange={(event) => setSelectedNumberId(event.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm"><option value="">Select an unassigned Aircall number…</option>{(availableNumbers.data ?? []).map((number) => <option key={number.id} value={String(number.id)}>{number.digits || number.name || `Aircall number #${number.id}`}{number.name ? ` — ${number.name}` : ""}</option>)}</select></div>
              <div className="flex flex-wrap items-center gap-3"><Button onClick={() => selectedNumberId && selectNumber.mutate({ numberId: Number(selectedNumberId) })} disabled={!selectedNumberId || selectNumber.isPending}><CheckCircle2 className="mr-1.5 h-4 w-4" /> {selectNumber.isPending ? "Saving…" : "Use as marketing line"}</Button><p className="text-xs text-muted-foreground">Only unassigned numbers appear here; ISA personal lines cannot be selected.</p></div>
            </>}
          </CardContent>
        </Card>
      )}

      {config?.sendReady && <div className="flex min-h-0 flex-1 overflow-hidden rounded-xl border bg-card shadow-sm">
        <aside className="flex w-full shrink-0 flex-col border-b md:w-[360px] md:border-b-0 md:border-r">
          <div className="border-b p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-medium">Conversations</p><p className="text-xs text-muted-foreground">{config.marketingNumber?.digits || config.marketingNumber?.name || "Dedicated marketing line"}</p></div><Badge variant="secondary"><Phone className="mr-1 h-3 w-3" /> Aircall</Badge></div><div className="relative mt-3"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="pl-9" placeholder="Search conversations" /></div></div>
          <div className="min-h-[240px] flex-1 overflow-y-auto">{threadsQuery.isLoading ? <p className="p-4 text-sm text-muted-foreground">Loading messages…</p> : ((threadsQuery.data ?? []) as MarketingThread[]).length === 0 ? <div className="p-8 text-center text-sm text-muted-foreground"><MessageSquare className="mx-auto mb-2 h-8 w-8 opacity-30" /><p>No messages on this line yet.</p></div> : ((threadsQuery.data ?? []) as MarketingThread[]).map((thread) => <button key={thread.id} type="button" onClick={() => { if (thread.contactId) setSelectedContactId(thread.contactId); }} className={`w-full border-b px-4 py-3 text-left transition-colors hover:bg-muted/60 ${selectedContactId === thread.contactId ? "bg-primary/5" : ""} ${!thread.contactId ? "cursor-not-allowed opacity-70" : ""}`} disabled={!thread.contactId}><div className="flex items-center justify-between gap-2"><p className="truncate text-sm font-medium">{contactName(thread)}</p><span className="shrink-0 text-[11px] text-muted-foreground">{formatTime(thread.receivedAt ?? thread.sentAt ?? thread.createdAt)}</span></div><div className="mt-1 flex items-center gap-2"><p className="truncate text-xs text-muted-foreground">{thread.direction === "inbound" ? "Reply: " : "Sent: "}{thread.body || "(No message body)"}</p>{thread.direction === "inbound" && <span className="h-2 w-2 shrink-0 rounded-full bg-primary" />}</div>{thread.smsMarketingOptedOutAt && <p className="mt-1 text-[11px] font-medium text-destructive">Marketing SMS opted out</p>}</button>)}</div>
        </aside>
