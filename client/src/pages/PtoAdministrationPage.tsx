...
import { CalendarDays, History, Loader2, PencilLine, Settings2, ShieldAlert, UsersRound } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 pb-10">
      ...
      <Tabs defaultValue="policy" className="space-y-6">
        ...
        <TabsContent value="balances"><Card><CardHeader><CardTitle>Everyone’s PTO balances</CardTitle><CardDescription>Balances combine accrued time, carryover, approved use, scheduled time, and named adjustments. The grid is restricted to PTO administrators.</CardDescription></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead className="border-b text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-2 py-3">Employee</th>{PTO_TYPES.map((type) => <th key={type} className="px-2 py-3">{typeLabel(type)}</th>)}</tr></thead><tbody>{(data?.employees ?? []).map((entry: any) => <tr key={entry.employee.id} className="border-b last:border-0"><td className="px-2 py-3"><p className="font-medium">{entry.employee.name ?? entry.employee.email}</p><p className="text-xs text-muted-foreground">{entry.employee.email}</p></td>{PTO_TYPES.map((type) => { const balance = entry.balances.find((item: any) => item.ptoType === type); return <td key={type} className="px-2 py-3"><p className="font-semibold tabular-nums">{balance?.remaining ?? 0} days</p><p className="text-xs text-muted-foreground">{balance?.accrued ?? 0} accrued · {balance?.used ?? 0} used</p></td>; })}</tr>)}</tbody></table></div></CardContent></Card></TabsContent>
