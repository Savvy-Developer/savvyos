...
async function ensureCompanyChannels(
  ...
  const sectionId =
    ...
    Number(
      (
        await db.insert(chatSections).values({
          name: "Company",
  ...
  for (const channel of [
    {
      name: "announcements",
      description: "Company announcements from Chat Admins.",
    },
    ...
    const [existingChannel] = await db
      ...
      .where(
        and(
          eq(chatChannels.name, channel.name),
          eq(chatChannels.isPermanent, true)
        )
      )
      .limit(1);
    if (!existingChannel) {
...
async function unreadSummary(
  ...
  if (!channelIds.length)
    return new Map<
      number,
      { unreadCount: number; unreadMentionCount: number }
    >();
  ...
  const readsByChannel = new Map(
    reads.map(row => [row.channelId, row.lastReadMessageId ?? 0])
  );
  ...
  const results = await Promise.all(
    channelIds.map(async channelId => {
      ...
      const [mentionCount] = await db
        ...
        .innerJoin(
          chatMessages,
          eq(chatMessages.id, chatMessageMentions.messageId)
        )
        .where(
          and(
            eq(chatMessages.channelId, channelId),
...
async function hydrateMessages(
  ...
  const parentIds = uniqueNumbers(
    rows
      .map(row => row.message.parentMessageId)
  ...
  for (const reaction of reactions) {
    ...
    const current = aggregate.get(reaction.emoji) ?? {
      count: 0,
      reactedByMe: false,
    };
    current.count += 1;
  ...
  const parentsById = new Map(
    parents.map(parent => [parent.message.id, parent])
  );
...
export const chatRouter = router({
  ...
  workspace: protectedProcedure.query(async ({ ctx }) => {
    ...
    const personalChannels = channels.filter(
      channel =>
        !channel.isPermanent &&
        state.memberChannelIds.has(channel.id) &&
    ...
    for (const group of visiblePermanentGroups) {
      const sectionKey =
        group.sectionId != null && activeSectionIds.has(group.sectionId)
          ? group.sectionId
          : null;
      const collection = groupsBySection.get(sectionKey) ?? [];
      collection.push({
        ...group,
        ...(summaries.get(group.id) ?? {
          unreadCount: 0,
    ...
    const personalChats = personalChannels
      .map(channel => {
        ...
        const person =
          channelType(channel.type) === "direct"
            ? (participants.find(
                participant => participant.id !== ctx.user.id
              ) ?? null)
            : null;
        ...
        return {
          ...
          participants: others,
          title: person
            ? (person.name ?? person.email ?? "Direct message")
            : channel.name,
          lastMessageAt:
            latestByChannel.get(channel.id)?.createdAt ?? channel.updatedAt,
      ...
      .sort(
        (left, right) =>
          Number(new Date(right.lastMessageAt)) -
          Number(new Date(left.lastMessageAt))
      );
    ...
    .query(async ({ input, ctx }) => {
      ...
      const directPersonByChannel = new Map(
        visibleDirectIds.map(channelId => [
          ...
          directParticipants.find(
            item =>
              item.channelId === channelId && item.person.id !== ctx.user.id
          ) ?? null,
        ])
      );
      ...
      const conversations = visibleChannels
        .map(channel => {
          ...
          const title = directPerson
            ? (directPerson.person.name ??
              directPerson.person.email ??
              "Direct message")
            : channel.name;
          const haystack =
            `${title} ${channel.description ?? ""}`.toLowerCase();
          return haystack.includes(input.query.toLowerCase())
            ? {
                channel,
                person: directPerson?.person ?? null,
                profilePhotoUrl: directPerson?.profilePhotoUrl ?? null,
              }
            : null;
        })
      ...
      const [messageRows, attachmentRows] = await Promise.all([
        state.db
          ...
          .where(
            and(
              inArray(chatMessages.channelId, visibleChannelIds),
              like(chatMessages.body, pattern)
            )
          )
          .orderBy(desc(chatMessages.createdAt), desc(chatMessages.id))
          .limit(30),
        state.db
          ...
          .innerJoin(
            chatMessages,
            eq(chatMessages.id, chatMessageAttachments.messageId)
          )
          .innerJoin(users, eq(users.id, chatMessages.senderId))
          .where(
            and(
              inArray(chatMessages.channelId, visibleChannelIds),
              like(chatMessageAttachments.fileName, pattern)
            )
          )
          .orderBy(desc(chatMessages.createdAt), desc(chatMessages.id))
          .limit(20),
      ...
      return {
        ...
        messages: messageRows.map(row => ({
          ...row,
          channelTitle: titleFor(row.message.channelId),
          channelType: channelsById.get(row.message.channelId)?.type ?? "group",
        })),
  ...
  messages: router({
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (input.parentMessageId) {
          ...
          if (!parentRows[0] || parentRows[0].channelId !== input.channelId) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "Choose a message from this conversation to reply to.",
            });
          }
        }
        ...
        const messageId = await state.db.transaction(async tx => {
          ...
          if (!state.channel.isPermanent) {
            await tx
              ...
              .where(
                and(
                  eq(chatChannelHides.channelId, input.channelId),
                  ne(chatChannelHides.userId, ctx.user.id)
                )
              );
          }
          if (attachmentIds.length) {
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (
          !canManageChatMessage({
            messageSenderId: state.message.senderId,
    ...
    markRead: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const [existingRead] = await state.db
          .select({ lastReadMessageId: chatChannelReads.lastReadMessageId })
          .from(chatChannelReads)
          .where(
            and(
              eq(chatChannelReads.channelId, input.channelId),
              eq(chatChannelReads.userId, ctx.user.id)
            )
          )
          .limit(1);
        const currentLastReadMessageId =
          existingRead?.lastReadMessageId ?? null;
        if (
          currentLastReadMessageId &&
          (!messageId || currentLastReadMessageId >= messageId)
        )
          return { success: true, messageId: currentLastReadMessageId };
        await state.db
    ...
    readState: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        const [row] = await state.db
          ...
          .where(
            and(
              eq(chatChannelReads.channelId, state.channel.id),
              eq(chatChannelReads.userId, ctx.user.id)
            )
          )
          .limit(1);
    ...
    markUnread: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const state = await requireMessageContext(ctx.user, input.messageId);
        const [previous] = await state.db
          .select({ id: chatMessages.id })
          .from(chatMessages)
          .where(
            and(
              eq(chatMessages.channelId, state.channel.id),
              lt(chatMessages.id, state.message.id)
            )
          )
          .orderBy(desc(chatMessages.id))
          .limit(1);
  ...
  reactions: router({
    toggle: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await state.db.insert(chatMessageReactions).values({
    ...
    open: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (input.userId === ctx.user.id) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Choose another person to start a direct message.",
          });
        }
        const targetRows = await state.db
          .select()
        ...
        if (existing[0]) {
          await state.db
            ...
            .where(
              and(
                eq(chatChannelHides.channelId, existing[0].id),
                eq(chatChannelHides.userId, ctx.user.id)
              )
            );
  ...
  conversations: router({
    ...
    create: protectedProcedure
      .input(
        z.object({
          userIds: z.array(z.number().int().positive()).min(1).max(29),
          name: z.string().trim().min(1).max(100).optional(),
        })
      )
      .mutation(async ({ input, ctx }) => {
        const state = await requireChatAccess(ctx.user);
        const inviteeIds = uniqueNumbers(input.userIds).filter(
          id => id !== ctx.user.id
        );
        if (!inviteeIds.length)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Choose at least one other person.",
          });
        const targets = await state.db
          .select()
          .from(users)
          .where(inArray(users.id, inviteeIds));
        if (
          targets.length !== inviteeIds.length ||
          targets.some(
            target => !target.isActive || target.personType !== "full_user"
          )
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Choose active SavvyOS users.",
          });
        }
        ...
        if (inviteeIds.length === 1) {
          ...
          const channelId = await state.db.transaction(async tx => {
            ...
            await tx.insert(chatChannelMembers).values([
              { channelId: id, userId: ctx.user.id, addedById: ctx.user.id },
              { channelId: id, userId: inviteeIds[0], addedById: ctx.user.id },
            ]);
            return id;
          });
          return { channelId, created: true, type: "direct" as const };
        }
        const generatedName = targets
          .map(target => target.name ?? target.email ?? "Teammate")
          .join(", ")
          .slice(0, 100);
        const channelId = await state.db.transaction(async tx => {
          const result = await tx.insert(chatChannels).values({
            type: "group",
            isPermanent: false,
            name: input.name?.trim() || generatedName || "Group chat",
            createdById: ctx.user.id,
          });
          const id = Number(result[0].insertId);
          await tx.insert(chatChannelMembers).values(
            [ctx.user.id, ...inviteeIds].map(userId => ({
              channelId: id,
              userId,
    ...
    archive: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (state.channel.isPermanent)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Permanent company groups cannot be archived.",
          });
        await state.db
          .insert(chatChannelHides)
          .values({ channelId: input.channelId, userId: ctx.user.id })
  ...
  groups: router({
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const state = await requireChatAdmin(ctx.user);
        const sections = await state.db
          .select({ id: chatSections.id, isArchived: chatSections.isArchived })
          .from(chatSections)
          .where(eq(chatSections.id, input.sectionId))
          .limit(1);
        if (!sections[0] || sections[0].isArchived)
          throw new TRPCError({
            code: "BAD_REQUEST",
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!group.isPermanent)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Personal chats cannot be managed as permanent groups.",
          });
        if (input.sectionId) {
          const sections = await state.db
            .select({
    ...
    archive: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!group.isPermanent)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Personal chats cannot be archived here.",
          });
        await state.db
          .update(chatChannels)
          .set({ isArchived: true })
    ...
    restore: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!group.isPermanent)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Personal chats cannot be restored here.",
          });
        await state.db
          .update(chatChannels)
          .set({ isArchived: false })
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!group.isPermanent)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Only permanent company groups can be deleted here.",
          });
        await state.db
          .delete(chatChannels)
          .where(eq(chatChannels.id, input.id));
  ...
  members: router({
    list: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (!group.isPermanent)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Personal chat participants are private.",
          });
        return state.db
          .select({
    ...
    add: protectedProcedure
      .input(channelIdSchema.extend({ userId: z.number().int().positive() }))
      .mutation(async ({ input, ctx }) => {
        const state = await requireChatAdmin(ctx.user);
        const group = await getChannelOrThrow(state.db, input.channelId);
        if (!group.isPermanent)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Personal chat participants cannot be managed here.",
          });
        const targetRows = await state.db
          .select({
            id: users.id,
    ...
    remove: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!group.isPermanent)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Personal chat participants cannot be removed.",
          });
        await state.db
    ...
    list: protectedProcedure.query(async ({ ctx }) => {
      ...
      const [sections, channels] = await Promise.all([
        ...
        state.db
          ...
          .where(
            and(
              eq(chatChannels.isArchived, true),
              eq(chatChannels.isPermanent, true)
            )
          )
          .orderBy(asc(chatChannels.name)),
