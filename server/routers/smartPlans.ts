...
import {
  renderSavvyCampaignEmail,
  sendSmartPlanEmail,
} from "../_core/smartPlanEmail";
...
const oneTimeSendInput = z
  ...
  .superRefine((input, ctx) => {
    ...
    const supportsDateAddedFilter =
      ...
      input.triggerType === "all_lead_sources";
    if (
      !supportsDateAddedFilter &&
      (input.dateAddedFrom || input.dateAddedTo)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "Date added filters are available only for Lead Source and All Lead Sources audiences.",
        path: ["dateAddedFrom"],
      });
    }
    if (
...
export function oneTimeExclusionReason(
  contact: typeof contacts.$inferSelect,
  channel: "email" | "sms"
): keyof OneTimeExclusionReasons | null {
  ...
  if (channel === "email") {
    ...
    return contactChannelAddresses(contact, channel).length
      ? null
      : "noEmailAddress";
  }
...
function recipientTargetsForContact(
  contact: typeof contacts.$inferSelect,
  channel: "email" | "sms"
): string[] {
  return oneTimeExclusionReason(contact, channel)
    ? []
    : contactChannelAddresses(contact, channel);
}

async function contactsForIds(
  db: any,
  contactIds: number[]
  ...
  for (
    ...
    start += CONTACT_QUERY_BATCH_SIZE
  ) {
...
async function oneTimeAudience(
  db: any,
  contactIds: number[],
  channel: "email" | "sms"
) {
  ...
  for (const contact of matchingContacts) {
    ...
    for (const recipientAddress of addresses) {
      ...
      if (channel === "email" && selectedEmailAddresses.has(normalizedAddress))
        continue;
...
async function selectedMarketingNumberId(
...
async function orderPlanStepsByTiming(db: any, planId: number): Promise<void> {
  ...
  const changed = orderedSteps.some(
    (step, index) => step.id !== currentIds[index]
  );
  if (!changed) return;
  ...
  for (let index = 0; index < orderedSteps.length; index++) {
    await db
      ...
      .set({ stepOrder: index })
      .where(eq(smartPlanSteps.id, orderedSteps[index].id));
  }
  for (const enrollment of liveEnrollments) {
    ...
    const nextIndex =
      pendingStepId === undefined
        ...
        : orderedIndexById.get(pendingStepId);
...
function validateDefaultSendWindow(input: z.infer<typeof planInput>): void {
  ...
  if (
    input.defaultSendDays?.length === 0 ||
    !isValidSmartPlanSendWindow(window)
  ) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message:
        "Choose at least one default delivery day and an end time after the start time.",
    });
  }
}
...
export const smartPlansRouter = router({
  ...
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    const result = await Promise.all(
      rows.map(async row => {
        ...
        const ids =
          (row.plan.triggerLeadSourceIds as number[] | null) ??
          (row.plan.triggerLeadSourceId ? [row.plan.triggerLeadSourceId] : []);
  ...
  get: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const rows = await db
        ...
        .leftJoin(
          leadSources,
          eq(smartPlans.triggerLeadSourceId, leadSources.id)
        )
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [result] = await db.insert(smartPlans).values({
        ...
        defaultSendDays:
          input.defaultSendDays ?? DEFAULT_SMART_PLAN_DELIVERY_WINDOW.days,
        defaultSendStartHour:
          input.defaultSendStartHour ??
          DEFAULT_SMART_PLAN_DELIVERY_WINDOW.startHour,
        defaultSendEndHour:
          input.defaultSendEndHour ??
          DEFAULT_SMART_PLAN_DELIVERY_WINDOW.endHour,
        defaultSendTimezone:
          input.defaultSendTimezone ??
          DEFAULT_SMART_PLAN_DELIVERY_WINDOW.timezone,
        status: input.status ?? "draft",
      });
      const newId = (result as any).insertId as number;
      await logActivity({
        userId: ctx.user.id,
        action: "smart_plan_created",
  ...
  createDraft: protectedProcedure
    .input(
      z.object({
        ...
        triggerType: smartPlanTriggerSchema.optional(),
        triggerScope: z
          .enum(["new_only", "existing_and_new", "manual"])
          .optional(),
      })
    )
    .mutation(async ({ input, ctx }) => {
      ...
      await logActivity({
        ...
        entityType: "smart_plan",
        entityId: draftId,
        details: { name: input.name, status: "draft" },
      });
      return { id: draftId };
    }),
  ...
  testSend: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.channel === "email" && !input.recipientEmail) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Enter an email address for the test send.",
        });
      }
      if (input.channel === "sms" && !input.recipientPhone) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Enter a phone number for the test send.",
        });
      }
      ...
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const marketingNumberId =
        input.channel === "sms" ? await selectedMarketingNumberId(db) : null;
      if (input.channel === "sms" && !marketingNumberId) {
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message:
            "Select a dedicated Aircall marketing number before sending a text test.",
        });
      }
      const result =
        input.channel === "email"
      ...
      if (!result.success) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: result.error || "Unable to send the test message.",
        });
      }
      await logActivity({
        userId: ctx.user.id,
  ...
  oneTimeSends: router({
    preview: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (
          input.triggerType === "lead_source" &&
          !input.triggerLeadSourceIds?.length
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (input.channel === "sms" && !(await selectedMarketingNumberId(db))) {
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "Select a dedicated Aircall marketing number before queueing a text campaign.",
          });
        }
        const now = new Date();
        ...
        if (scheduledAt.getTime() < now.getTime() - 60_000) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Choose a scheduled date and time that has not already passed.",
          });
        }
        // Resend's Marketing Broadcast service owns email pacing. Keep the
        // existing hourly throttle only for Aircall text messages.
        const staggerPerHour =
          input.channel === "sms" && input.staggerEnabled
        ...
        if (!audience.recipientTargets.length) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "No eligible recipients match this audience and channel.",
          });
        }
        ...
        const [result] = await db.insert(oneTimeSends).values({
          ...
          channel: input.channel,
          subject:
            input.channel === "email" ? (input.subject?.trim() ?? null) : null,
          body: input.body,
          ...
          emailDeliveryMethod:
            input.channel === "email" ? "resend_broadcast" : null,
          status: "queued",
        ...
        for (
          let start = 0;
          start < audience.recipientTargets.length;
          start += 500
        ) {
          await db.insert(oneTimeSendRecipients).values(
            audience.recipientTargets
    ...
    detail: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (!sendRow)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "One-time send not found.",
          });

        const conditions = [eq(oneTimeSendRecipients.sendId, input.sendId)];
        if (input.status)
          conditions.push(eq(oneTimeSendRecipients.status, input.status));
        if (input.activity === "delivered") {
          conditions.push(
            or(
              ...
              inArray(oneTimeSendRecipients.providerLastEvent, [
                "delivered",
                "opened",
                "clicked",
              ])
        ...
        if (input.activity === "clicked")
          conditions.push(
            or(
              isNotNull(oneTimeSendRecipients.clickedAt),
              eq(oneTimeSendRecipients.providerLastEvent, "clicked")
            )!
          );
        if (input.activity === "replied")
          conditions.push(isNotNull(oneTimeSendRecipients.repliedAt));
        if (input.activity === "bounced")
          conditions.push(
            or(
              isNotNull(oneTimeSendRecipients.bouncedAt),
              eq(oneTimeSendRecipients.providerLastEvent, "bounced")
            )!
          );
        if (input.activity === "complained")
          conditions.push(
            or(
              isNotNull(oneTimeSendRecipients.complainedAt),
              eq(oneTimeSendRecipients.providerLastEvent, "complained")
            )!
          );
        if (input.activity === "suppressed")
          conditions.push(
            or(
              isNotNull(oneTimeSendRecipients.suppressedAt),
              eq(oneTimeSendRecipients.providerLastEvent, "suppressed")
            )!
          );
        ...
        const [countRows, rows] = await Promise.all([
          ...
          db
            ...
            .innerJoin(
              contacts,
              eq(oneTimeSendRecipients.contactId, contacts.id)
            )
            .where(where)
            .orderBy(
              desc(oneTimeSendRecipients.sentAt),
              desc(oneTimeSendRecipients.id)
            )
            .limit(input.limit)
            .offset(offset),
        ]);
        ...
        const events =
          recipientIds.length === 0
            ...
            : await db
                ...
                .where(
                  inArray(oneTimeSendMessageEvents.recipientId, recipientIds)
                )
        ...
        return {
          ...sendRow,
          emailPreviewHtml:
            sendRow.send.channel === "email"
              ? renderSavvyCampaignEmail(
                  sendRow.send.subject || sendRow.send.name,
                  sendRow.send.body,
                  true
                )
              : null,
          recipients: rows.map(row => ({
            ...row,
            events: eventsByRecipient.get(row.recipient.id) ?? [],
          })),
    ...
    syncProviderStatus: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const outcomes = await mapWithConcurrency(
          ...
          5,
          async recipient => {
            const result = await getResendEmailStatus(
        ...
        return {
          refreshedRecipients: recipients.length,
          updated,
          failed,
          checkedAt,
        };
  ...
  publish: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (steps.length === 0)
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Plan must have at least one step before publishing.",
        });
      await db
        .update(smartPlans)
        .set({ status: "active" })
  ...
  update: protectedProcedure
    .input(
      z.object({
        ...
        data: planInput
          .partial()
          .extend({ includeExistingContacts: z.boolean().optional() }),
      })
    )
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
      ...
      if (windowChanged) {
        validateDefaultSendWindow({
          ...existingPlan,
          ...data,
          defaultSendWindowEnabled:
            data.defaultSendWindowEnabled ??
            existingPlan.defaultSendWindowEnabled,
          defaultSendDays: data.defaultSendDays ?? existingPlan.defaultSendDays,
          defaultSendStartHour:
            data.defaultSendStartHour ?? existingPlan.defaultSendStartHour,
          defaultSendEndHour:
            data.defaultSendEndHour ?? existingPlan.defaultSendEndHour,
          defaultSendTimezone:
            data.defaultSendTimezone ?? existingPlan.defaultSendTimezone,
        });
      }
      await db.update(smartPlans).set(data).where(eq(smartPlans.id, input.id));
      const enrollment = includeExistingContacts
        ? await bulkEnrollExistingContacts(input.id)
        : { enrolled: 0 };
      await logActivity({
        userId: ctx.user.id,
        ...
        details: {
          ...
          enrolledCurrentContacts: enrollment.enrolled,
        },
      });
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const e of enrollmentIds) {
        await db
          .delete(smartPlanExecutions)
          .where(eq(smartPlanExecutions.enrollmentId, e.id));
      }
      await db
        .delete(smartPlanEnrollments)
        .where(eq(smartPlanEnrollments.planId, input.id));
      await db
  ...
  steps: router({
          ...
          steps: z.array(
            z.object({
              ...
              sendDays: z
                ...
                .default(DEFAULT_SMART_PLAN_DELIVERY_WINDOW.days),
              sendStartHour: z
                .number()
                .int()
                .min(0)
                .max(23)
                .default(DEFAULT_SMART_PLAN_DELIVERY_WINDOW.startHour),
              sendEndHour: z
                .number()
                .int()
                .min(1)
                .max(24)
                .default(DEFAULT_SMART_PLAN_DELIVERY_WINDOW.endHour),
              timezone: z
                .string()
                .default(DEFAULT_SMART_PLAN_DELIVERY_WINDOW.timezone),
            })
          ),
        })
      )
    ...
    add: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const [insertedStep] = await db
          .select({ stepOrder: smartPlanSteps.stepOrder })
          .from(smartPlanSteps)
          .where(eq(smartPlanSteps.id, insertedId))
          .limit(1);
        return {
          id: insertedId,
          stepOrder: insertedStep?.stepOrder ?? nextOrder,
        };
      }),
    ...
    reorder: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db
          .update(smartPlanSteps)
          .set({ stepOrder: bOrder })
          .where(eq(smartPlanSteps.id, steps[idx].id));
        await db
          .update(smartPlanSteps)
  ...
  aiAnalysis: protectedProcedure.mutation(async ({ ctx }) => {
    if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
    const result = await analyzeSmartPlanPerformance();
    let emailDeliveredToTyler = false;
    try {
      const delivery = await sendTransactionalEmail(
        "smart_plan_ai_analysis",
        {
          ...
          smartPlanAnalysisRequestedBy: ctx.user.name ?? "An admin",
          smartPlanAnalysisHtml: renderSmartPlanAnalysisEmail(
            result,
            ctx.user.name
          ),
        },
        {
          idempotencyKey: `smart-plan-ai-analysis:${result.generatedAt.getTime()}:${ctx.user.id}`,
          allowTemplateOverride: false,
        }
      );
      emailDeliveredToTyler = delivery.sent;
      if (!delivery.sent) {
        console.warn(
          `[SmartPlans] Analysis email to Tyler was not delivered: ${delivery.reason ?? "unknown reason"}`
        );
      }
    } catch (error) {
      // Returning the on-page analysis is more important than allowing a
      // secondary notification failure to discard it.
      console.error("[SmartPlans] Analysis email to Tyler failed:", error);
    }
    await logActivity({
      userId: ctx.user.id,
      ...
      details: {
        ...
        totalEmailSends: result.evidence?.totalEmailSends ?? 0,
        usedAi: result.usedAi,
        emailDeliveredToTyler,
      },
    });
    await notifySavvyOSPromptRun({
      title: "Smart Plans AI analysis generated",
      summary:
        "Smart Plan delivery, engagement, reply, timing, and deliverability signals were reviewed from aggregate campaign data.",
      actionUrl: "/smart-plans",
    });
    return { ...result, emailDeliveredToTyler };
  }),

  /** Active lead sources without a usable active or paused lead-source Smart Plan. */
  leadSourcesWithoutPlans: protectedProcedure.query(async ({ ctx }) => {
    if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    ...
    const [sourceRows, planRows, volumeRows] = await Promise.all([
      db
        .select({
          id: leadSources.id,
          name: leadSources.name,
      ...
      db
        .select({
          status: smartPlans.status,
          triggerType: smartPlans.triggerType,
          triggerScope: smartPlans.triggerScope,
          triggerLeadSourceId: smartPlans.triggerLeadSourceId,
          triggerLeadSourceIds: smartPlans.triggerLeadSourceIds,
        })
        .from(smartPlans)
        .where(inArray(smartPlans.status, ["active", "paused"])),
      db
        .select({
          leadSourceId: contacts.leadSourceId,
          oneDay: sql<number>`COALESCE(SUM(CASE WHEN ${contacts.createdAt} >= ${oneDayAgo} THEN 1 ELSE 0 END), 0)`,
          sevenDays: sql<number>`COALESCE(SUM(CASE WHEN ${contacts.createdAt} >= ${sevenDaysAgo} THEN 1 ELSE 0 END), 0)`,
          fourteenDays: sql<number>`COALESCE(SUM(CASE WHEN ${contacts.createdAt} >= ${fourteenDaysAgo} THEN 1 ELSE 0 END), 0)`,
          thirtyDays: sql<number>`COALESCE(SUM(CASE WHEN ${contacts.createdAt} >= ${thirtyDaysAgo} THEN 1 ELSE 0 END), 0)`,
          ninetyDays: sql<number>`COALESCE(SUM(CASE WHEN ${contacts.createdAt} >= ${ninetyDaysAgo} THEN 1 ELSE 0 END), 0)`,
          ytd: sql<number>`COALESCE(SUM(CASE WHEN ${contacts.createdAt} >= ${startOfYear} THEN 1 ELSE 0 END), 0)`,
          allTime: sql<number>`COUNT(*)`,
        })
        .from(contacts)
        .where(isNotNull(contacts.leadSourceId))
        .groupBy(contacts.leadSourceId),
    ]);

    const sourceById = new Map(sourceRows.map(source => [source.id, source]));
    const volumesBySourceId = new Map(
      volumeRows.map(row => [
        row.leadSourceId!,
        {
          oneDay: Number(row.oneDay ?? 0),
          sevenDays: Number(row.sevenDays ?? 0),
          fourteenDays: Number(row.fourteenDays ?? 0),
          thirtyDays: Number(row.thirtyDays ?? 0),
          ninetyDays: Number(row.ninetyDays ?? 0),
          ytd: Number(row.ytd ?? 0),
          allTime: Number(row.allTime ?? 0),
        },
      ])
    );
    const coveredSourceIds = new Set<number>();
    const globalPlanCoversAllSources = planRows.some(
      plan =>
        plan.triggerType === "all_lead_sources" &&
        plan.triggerScope !== "manual"
    );
    for (const plan of planRows) {
      if (plan.triggerType !== "lead_source" || plan.triggerScope === "manual")
        continue;
      const sourceIds =
        (plan.triggerLeadSourceIds as number[] | null) ??
    ...
    return {
      ...
      sources: globalPlanCoversAllSources
        ? []
        : sourceRows
            ...
            .map(source => ({
              id: source.id,
              name: source.name,
              parentName: source.parentId
                ? (sourceById.get(source.parentId)?.name ?? null)
                : null,
              volume: volumesBySourceId.get(source.id) ?? {
                oneDay: 0,
                sevenDays: 0,
                fourteenDays: 0,
                thirtyDays: 0,
    ...
    list: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        const rows = await db
          ...
          .orderBy(
            desc(smartPlanExecutions.sentAt),
            desc(smartPlanExecutions.id)
          )
          .limit(input.limit);
        return rows;
      }),
  ...
  enrollments: router({
    list: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        const conditions = [];
        if (input.planId)
          conditions.push(eq(smartPlanEnrollments.planId, input.planId));
        if (input.contactId)
          conditions.push(eq(smartPlanEnrollments.contactId, input.contactId));
        ...
        const planIds = Array.from(
          new Set(rows.map(row => row.enrollment.planId))
        );
        ...
        const [allSteps, allExecutions] = await Promise.all([
          planIds.length
            ? db
                .select()
                .from(smartPlanSteps)
                .where(inArray(smartPlanSteps.planId, planIds))
                .orderBy(
                  asc(smartPlanSteps.planId),
                  asc(smartPlanSteps.stepOrder)
                )
            : Promise.resolve([]),
          enrollmentIds.length
            ? db
                .select()
                .from(smartPlanExecutions)
                .where(inArray(smartPlanExecutions.enrollmentId, enrollmentIds))
                .orderBy(
                  desc(smartPlanExecutions.sentAt),
                  desc(smartPlanExecutions.id)
                )
        ...
        for (const execution of allExecutions) {
          const enrollmentExecutions =
            executionsByEnrollment.get(execution.enrollmentId) ?? [];
          enrollmentExecutions.push(execution);
          executionsByEnrollment.set(
            execution.enrollmentId,
            enrollmentExecutions
          );
        }
        const enriched = rows.map(row => {
          const steps = stepsByPlan.get(row.enrollment.planId) ?? [];
          const executions =
            executionsByEnrollment.get(row.enrollment.id) ?? [];
          return {
            ...row,
            currentStep: steps[row.enrollment.currentStepIndex] ?? null,
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (input.contactId) {
          ...
          const rows = await db
            ...
            .innerJoin(
              smartPlans,
              eq(smartPlanEnrollments.planId, smartPlans.id)
            )
            .where(
              enrollmentIds.length === 1
                ...
                : and(
                    ...enrollmentIds.map(id =>
                      eq(smartPlanExecutions.enrollmentId, id)
                    )
                  )
            )
