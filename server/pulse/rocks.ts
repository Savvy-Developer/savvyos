...
export const pulseRocksRouter = router({
  ...
  addRock: pulseProcedure.input(z.object({ meetingId: z.string().uuid(), workItemId: z.string().uuid() })).mutation(async ({ ctx, input }) => {
    const db = await dbOrThrow(); await requireManager(db, ctx.user.id, input.meetingId);
  ...
  removeRock: pulseProcedure.input(z.object({ meetingId: z.string().uuid(), mappingId: z.string().uuid() })).mutation(async ({ ctx, input }) => {
    const db = await dbOrThrow(); await requireManager(db, ctx.user.id, input.meetingId);
  ...
  globalQuarterly: pulseProcedure.query(async ({ ctx }) => {
