...
function emptyCohort(): CohortMetrics {
  return { leads: 0, appointments: 0, transactionsStarted: 0, transactionsClosed: 0, closedVolume: 0, closedGci: 0 };
}
...
export async function buildWeeklyLeadReport(asOf = new Date()): Promise<WeeklyLeadReport> {
  ...
  for (const result of r90CohortClosedTransactions) {
    ...
    row.r90Cohort.closedVolume += numberValue(result.volume);
  }
  ...
  for (const result of cohortClosedTransactions) {
    ...
    row.cohort.closedVolume += numberValue(result.volume);
  }
...
function weeklyLeadTable(rows: WeeklyLeadReportSourceRow[]): string {
  ...
  const tableRows = visibleRows.map((row, index) => `<tr style="background:${index % 2 ? "#F9FAFB" : "#FFFFFF"};"><td width="150" style="width:150px;max-width:150px;padding:8px 6px;border-bottom:1px solid #E5E7EB;font-size:10px;font-weight:600;color:#111827;line-height:1.3;white-space:normal;overflow-wrap:anywhere;">${escapeHtml(row.sourceName)}</td><td style="background:#EFF6FF;padding:8px 4px;border-bottom:1px solid #BFDBFE;border-left:2px solid #60A5FA;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.newLeads)}</td><td style="background:#EFF6FF;padding:8px 4px;border-bottom:1px solid #BFDBFE;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.appointmentsSet)}</td><td style="background:#EFF6FF;padding:8px 4px;border-bottom:1px solid #BFDBFE;text-align:center;font-size:11px;font-weight:600;color:#111827;">${formatPercent(row.appointmentsSet, row.newLeads)}</td><td style="background:#F0FDFA;padding:8px 4px;border-bottom:1px solid #99F6E4;border-left:2px solid #2DD4BF;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.r30Leads)}</td><td style="background:#F0FDFA;padding:8px 4px;border-bottom:1px solid #99F6E4;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.r30Appointments)}</td><td style="background:#F0FDFA;padding:8px 4px;border-bottom:1px solid #99F6E4;text-align:center;font-size:11px;font-weight:600;color:#111827;">${formatPercent(row.r30Appointments, row.r30Leads)}</td><td style="background:#FAF5FF;padding:8px 4px;border-bottom:1px solid #E9D5FF;border-left:2px solid #C084FC;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.r90Leads)}</td><td style="background:#FAF5FF;padding:8px 4px;border-bottom:1px solid #E9D5FF;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.r90Appointments)}</td><td style="background:#FAF5FF;padding:8px 4px;border-bottom:1px solid #E9D5FF;text-align:center;font-size:11px;font-weight:600;color:#111827;">${formatPercent(row.r90Appointments, row.r90Leads)}</td></tr>`).join("");
  return `<div style="overflow-x:auto;margin:0 -12px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="min-width:850px;table-layout:fixed;border:1px solid #D1D5DB;border-collapse:collapse;"><tr><th rowspan="2" width="150" style="background:#111827;width:150px;max-width:150px;padding:9px 6px;text-align:left;color:#FFFFFF;font-size:10px;letter-spacing:.3px;border-bottom:1px solid #374151;">LEAD SOURCE</th><th colspan="3" style="background:#075985;padding:9px 6px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;border-bottom:1px solid #0C4A6E;">THIS WEEK</th><th colspan="3" style="background:#0F766E;padding:9px 6px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;border-bottom:1px solid #115E59;">R30</th><th colspan="3" style="background:#6D28D9;padding:9px 6px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;border-bottom:1px solid #581C87;">R90</th></tr><tr><th style="background:#0C4A6E;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">LEADS</th><th style="background:#0C4A6E;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">APPTS</th><th style="background:#0C4A6E;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">RATE</th><th style="background:#115E59;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">LEADS</th><th style="background:#115E59;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">APPTS</th><th style="background:#115E59;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">RATE</th><th style="background:#581C87;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">LEADS</th><th style="background:#581C87;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">APPTS</th><th style="background:#581C87;padding:8px 6px;text-align:center;color:#FFFFFF;font-size:9px;">RATE</th></tr>${tableRows || `<tr><td colspan="10" style="padding:18px;text-align:center;color:#6B7280;font-size:12px;">No new leads were recorded this week.</td></tr>`}</table></div>`;
}
...
function matureYieldTable(rows: WeeklyLeadReportSourceRow[]): string {
  const visibleRows = rows
    ...
    .sort((a, b) => (b.r90Cohort.closedVolume + b.cohort.closedVolume) - (a.r90Cohort.closedVolume + a.cohort.closedVolume) || (b.r90Cohort.transactionsClosed + b.cohort.transactionsClosed) - (a.r90Cohort.transactionsClosed + a.cohort.transactionsClosed) || b.r90Cohort.leads - a.r90Cohort.leads || a.sourceName.localeCompare(b.sourceName));
  const tableRows = visibleRows.map((row, index) => `<tr style="background:${index % 2 ? "#F9FAFB" : "#FFFFFF"};"><td width="150" style="width:150px;max-width:150px;padding:8px 6px;border-bottom:1px solid #E5E7EB;font-size:10px;font-weight:600;color:#111827;line-height:1.3;white-space:normal;overflow-wrap:anywhere;">${escapeHtml(row.sourceName)}</td><td style="background:#EFF6FF;padding:8px 4px;border-bottom:1px solid #BFDBFE;border-left:2px solid #60A5FA;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.r90Cohort.leads)}</td><td style="background:#EFF6FF;padding:8px 4px;border-bottom:1px solid #BFDBFE;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.r90Cohort.transactionsClosed)}</td><td style="background:#EFF6FF;padding:8px 4px;border-bottom:1px solid #BFDBFE;text-align:center;font-size:11px;font-weight:600;color:#111827;">${formatPercent(row.r90Cohort.transactionsClosed, row.r90Cohort.leads)}</td><td style="background:#EFF6FF;padding:8px 4px;border-bottom:1px solid #BFDBFE;text-align:right;font-size:11px;font-weight:600;color:#111827;white-space:nowrap;">${formatCurrency(row.r90Cohort.closedVolume)}</td><td style="background:#F0FDFA;padding:8px 4px;border-bottom:1px solid #99F6E4;border-left:2px solid #2DD4BF;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.cohort.leads)}</td><td style="background:#F0FDFA;padding:8px 4px;border-bottom:1px solid #99F6E4;text-align:center;font-size:11px;color:#374151;">${formatInteger(row.cohort.transactionsClosed)}</td><td style="background:#F0FDFA;padding:8px 4px;border-bottom:1px solid #99F6E4;text-align:center;font-size:11px;font-weight:600;color:#111827;">${formatPercent(row.cohort.transactionsClosed, row.cohort.leads)}</td><td style="background:#F0FDFA;padding:8px 4px;border-bottom:1px solid #99F6E4;text-align:right;font-size:11px;font-weight:600;color:#111827;white-space:nowrap;">${formatCurrency(row.cohort.closedVolume)}</td></tr>`).join("");
  return `<div style="overflow-x:auto;margin:0 -12px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="min-width:810px;table-layout:fixed;border:1px solid #D1D5DB;border-collapse:collapse;"><tr><th rowspan="2" width="150" style="background:#111827;width:150px;max-width:150px;padding:9px 6px;text-align:left;color:#FFFFFF;font-size:10px;letter-spacing:.3px;border-bottom:1px solid #374151;">LEAD SOURCE</th><th colspan="4" style="background:#075985;padding:9px 4px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;border-bottom:1px solid #0C4A6E;">R90</th><th colspan="4" style="background:#0F766E;padding:9px 4px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;border-bottom:1px solid #115E59;">R91–R179</th></tr><tr><th style="background:#0C4A6E;padding:8px 4px;text-align:center;color:#FFFFFF;font-size:9px;">LEADS</th><th style="background:#0C4A6E;padding:8px 4px;text-align:center;color:#FFFFFF;font-size:9px;">CLOSED</th><th style="background:#0C4A6E;padding:8px 4px;text-align:center;color:#FFFFFF;font-size:9px;">RATE</th><th style="background:#0C4A6E;padding:8px 4px;text-align:right;color:#FFFFFF;font-size:9px;">VOLUME</th><th style="background:#115E59;padding:8px 4px;text-align:center;color:#FFFFFF;font-size:9px;">LEADS</th><th style="background:#115E59;padding:8px 4px;text-align:center;color:#FFFFFF;font-size:9px;">CLOSED</th><th style="background:#115E59;padding:8px 4px;text-align:center;color:#FFFFFF;font-size:9px;">RATE</th><th style="background:#115E59;padding:8px 4px;text-align:right;color:#FFFFFF;font-size:9px;">VOLUME</th></tr>${tableRows || `<tr><td colspan="9" style="padding:18px;text-align:center;color:#6B7280;font-size:12px;">No source cohort activity was recorded.</td></tr>`}</table></div>`;
}
...
function signals(report: WeeklyLeadReport): string {
  ...
  const highestMatureYield = [...report.rows].filter((row) => row.r90Cohort.closedVolume + row.cohort.closedVolume > 0).sort((a, b) => (b.r90Cohort.closedVolume + b.cohort.closedVolume) - (a.r90Cohort.closedVolume + a.cohort.closedVolume))[0];
  ...
  if (highestMatureYield) output.push(signalCard("Source yield:", `${highestMatureYield.sourceName} generated ${formatCurrency(highestMatureYield.r90Cohort.closedVolume + highestMatureYield.cohort.closedVolume)} in closed volume across its R90 and R91–R179 cohorts.`, "#059669"));
...
export function renderWeeklyLeadReport(report: WeeklyLeadReport): string {
  ...
  return `<div style="font-size:20px;font-weight:700;line-height:1.3;color:#111827;">Weekly Lead Report</div>
    ...
    ${sectionHeading("2. Source Yield — R90 vs R91–R179", "R90 is the most recent 90-day lead cohort. R91–R179 is the preceding mature cohort. Both compare lead-to-close conversion and closed volume by source.")}
...
export async function sendWeeklyLeadReportTest(asOf = new Date()): Promise<{ sent: boolean; skipped: boolean; reason?: string; report: WeeklyLeadReport }> {
  ...
  const delivery = await sendTransactionalEmail(
    ...
    },
    {
      ...
      idempotencyKey: `weekly-lead-report:test:cohort-color-volume-v7:${report.reportDateKey}:${TEST_RECIPIENT_EMAIL}`,
    },
  );
