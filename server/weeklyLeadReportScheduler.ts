...
function sourceTable(rows: WeeklyLeadReportSourceRow[]): string {
  const tableRows = rows.map((row, index) => `<tr style="background:${index % 2 ? "#F9FAFB" : "#FFFFFF"};"><td style="padding:10px 8px;border-bottom:1px solid #E5E7EB;font-size:12px;font-weight:600;color:#111827;">${escapeHtml(row.sourceName)}</td><td style="padding:10px 7px;border-bottom:1px solid #E5E7EB;text-align:center;font-size:12px;color:#374151;">${formatInteger(row.newLeads)}</td><td style="padding:10px 7px;border-bottom:1px solid #E5E7EB;text-align:center;font-size:12px;color:#374151;">${formatInteger(row.transactionsCreated)}</td><td style="padding:10px 7px;border-bottom:1px solid #E5E7EB;text-align:center;font-size:12px;color:#374151;">${formatInteger(row.newUnderContract)}</td><td style="padding:10px 7px;border-bottom:1px solid #E5E7EB;text-align:center;font-size:12px;color:#374151;">${formatInteger(row.closedTransactions)}</td><td style="padding:10px 7px;border-bottom:1px solid #E5E7EB;text-align:center;font-size:12px;color:#374151;">${formatInteger(row.appointmentsSet)}</td><td style="padding:10px 7px;border-bottom:1px solid #E5E7EB;text-align:right;font-size:12px;font-weight:600;color:#111827;white-space:nowrap;">${formatCurrency(row.closedGci)}</td><td style="padding:10px 7px;border-bottom:1px solid #E5E7EB;text-align:center;font-size:12px;color:#374151;">${formatPercent(row.cohort.transactionsClosed, row.cohort.leads)}</td></tr>`).join("");
  return `<div style="overflow-x:auto;margin:0 -12px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="min-width:760px;border:1px solid #D1D5DB;border-collapse:collapse;"><tr style="background:#0A0A0A;"><th style="padding:9px 8px;text-align:left;color:#FFFFFF;font-size:10px;letter-spacing:.3px;">LEAD SOURCE</th><th style="padding:9px 7px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;">NEW LEADS</th><th style="padding:9px 7px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;">NEW TXNS</th><th style="padding:9px 7px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;">UNDER CONTRACT</th><th style="padding:9px 7px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;">CLOSED</th><th style="padding:9px 7px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;">APPTS</th><th style="padding:9px 7px;text-align:right;color:#FFFFFF;font-size:10px;letter-spacing:.3px;">CLOSED GCI</th><th style="padding:9px 7px;text-align:center;color:#FFFFFF;font-size:10px;letter-spacing:.3px;">90D CLOSE %</th></tr>${tableRows || `<tr><td colspan="8" style="padding:18px;text-align:center;color:#6B7280;font-size:12px;">No lead-source activity was recorded this week.</td></tr>`}</table></div>`;
}
...
function signals(report: WeeklyLeadReport): string {
  ...
  const highVolumeNoAppointment = [...report.rows].filter((row) => row.newLeads >= 3 && row.appointmentsSet === 0).sort((a, b) => b.newLeads - a.newLeads)[0];
  if (topCloser) output.push(signalCard("Top closed-GCI source:", `${topCloser.sourceName} produced ${formatCurrency(topCloser.closedGci)} from ${topCloser.closedTransactions} closed transaction${topCloser.closedTransactions === 1 ? "" : "s"} this week.`, "#059669"));
  if (highVolumeNoAppointment) output.push(signalCard("Appointment opportunity:", `${highVolumeNoAppointment.sourceName} generated ${highVolumeNoAppointment.newLeads} new lead${highVolumeNoAppointment.newLeads === 1 ? "" : "s"} with no appointment recorded this week. Review assignment and first-contact follow-up.`, "#D97706"));
  if (report.quality.unassignedNewLeads > 0) output.push(signalCard("Assignment gap:", `${report.quality.unassignedNewLeads} new lead${report.quality.unassignedNewLeads === 1 ? " is" : "s are"} not assigned to an ISA.`, "#DC2626"));
  if (report.quality.unattributedNewLeads > 0 || report.quality.unattributedTransactions > 0) output.push(signalCard("Attribution cleanup:", `${report.quality.unattributedNewLeads} new lead${report.quality.unattributedNewLeads === 1 ? " is" : "s are"} and ${report.quality.unattributedTransactions} transaction event${report.quality.unattributedTransactions === 1 ? " is" : "s are"} missing a lead-source attribution.`, "#DC2626"));
  if (report.quality.agingUnworkedLeads > 0) output.push(signalCard("Aging follow-up:", `${formatInteger(report.quality.agingUnworkedLeads)} lead${report.quality.agingUnworkedLeads === 1 ? " is" : "s are"} created in the prior 30 days and still marked New or Attempted Contact.`, "#D97706"));
  if (!output.length) output.push(signalCard("Clean weekly snapshot:", "No material source-attribution, assignment, or appointment-coverage exceptions were detected in the current report window.", "#0891B2"));
  return output.join("");
}
...
export function renderWeeklyLeadReport(report: WeeklyLeadReport): string {
  ...
  const funnelCells = [
    ["New", pipeline.new_lead],
    ["Attempted Contact", pipeline.attempted_contact],
    ["Nurture", pipeline.nurture],
    ["Active Client", pipeline.active_client],
    ["Under Contract", pipeline.under_contract],
    ["Closed", pipeline.closed],
    ["Dead / Do Not Contact", pipeline.dead + pipeline.do_not_contact],
  ].map(([label, value]) => `<td style="width:25%;padding:5px;vertical-align:top;"><div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:7px;padding:10px;"><div style="font-size:16px;font-weight:700;color:#111827;">${formatInteger(Number(value))}</div><div style="font-size:9px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.35px;margin-top:4px;line-height:1.3;">${escapeHtml(String(label))}</div></div></td>`);
  ...
  return `<div style="font-size:20px;font-weight:700;line-height:1.3;color:#111827;">Weekly Lead Report</div>
    ...
    <div style="font-size:14px;color:#374151;line-height:1.6;">This report ties lead acquisition to appointment activity, transaction outcomes, and attribution quality. “New Transactions” reflects records created this week; Under Contract and Closed use their respective event dates.</div>
    <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="margin:17px -5px 0;"><tr>${metricCard(formatInteger(totals.newLeads), "New leads", "#0891B2")}${metricCard(formatInteger(totals.transactionsCreated), "New transactions")}${metricCard(formatInteger(totals.newUnderContract), "New under contract", "#0F766E")}</tr><tr>${metricCard(formatInteger(totals.closedTransactions), "Closed transactions", "#059669")}${metricCard(formatCurrency(totals.closedGci), "Closed GCI", "#059669")}${metricCard(formatInteger(totals.appointmentsSet), "Appointments set", "#7C3AED")}</tr></table>
    ${sectionHeading("Lead Source Scorecard", "Sources with activity during the reporting week are shown. The 90-day close rate measures closed transactions against leads created during the rolling 90-day cohort.")}
    ${sourceTable(report.rows)}
    ${sectionHeading("Current Pipeline Health", "Active, unarchived contacts across SavvyOS—not only leads received this week.")}
    <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="margin:0 -5px;"><tr>${funnelCells.slice(0, 4).join("")}</tr><tr>${funnelCells.slice(4).join("")}</tr></table>
    ${sectionHeading("Leadership Focus")}
    ${signals(report)}
    ${sectionHeading("Data Quality & Follow-Up")}
    <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="border:1px solid #E5E7EB;border-collapse:collapse;"><tr><td style="padding:10px;border-bottom:1px solid #E5E7EB;font-size:12px;color:#374151;">New leads without a lead source</td><td style="padding:10px;border-bottom:1px solid #E5E7EB;text-align:right;font-size:12px;font-weight:700;color:${report.quality.unattributedNewLeads ? "#DC2626" : "#111827"};">${formatInteger(report.quality.unattributedNewLeads)}</td></tr><tr><td style="padding:10px;border-bottom:1px solid #E5E7EB;font-size:12px;color:#374151;">Weekly transaction events without source attribution</td><td style="padding:10px;border-bottom:1px solid #E5E7EB;text-align:right;font-size:12px;font-weight:700;color:${report.quality.unattributedTransactions ? "#DC2626" : "#111827"};">${formatInteger(report.quality.unattributedTransactions)}</td></tr><tr><td style="padding:10px;border-bottom:1px solid #E5E7EB;font-size:12px;color:#374151;">New leads not assigned to an ISA</td><td style="padding:10px;border-bottom:1px solid #E5E7EB;text-align:right;font-size:12px;font-weight:700;color:${report.quality.unassignedNewLeads ? "#DC2626" : "#111827"};">${formatInteger(report.quality.unassignedNewLeads)}</td></tr><tr><td style="padding:10px;border-bottom:1px solid #E5E7EB;font-size:12px;color:#374151;">30-day leads still new or attempted-contact</td><td style="padding:10px;border-bottom:1px solid #E5E7EB;text-align:right;font-size:12px;font-weight:700;color:${report.quality.agingUnworkedLeads ? "#D97706" : "#111827"};">${formatInteger(report.quality.agingUnworkedLeads)}</td></tr><tr><td style="padding:10px;border-bottom:1px solid #E5E7EB;font-size:12px;color:#374151;">Overdue open follow-up tasks</td><td style="padding:10px;border-bottom:1px solid #E5E7EB;text-align:right;font-size:12px;font-weight:700;color:${report.quality.overdueOpenTasks ? "#D97706" : "#111827"};">${formatInteger(report.quality.overdueOpenTasks)}</td></tr><tr><td style="padding:10px;font-size:12px;color:#374151;">Inactive sources receiving new leads</td><td style="padding:10px;text-align:right;font-size:12px;font-weight:700;color:${report.quality.inactiveSourcesReceivingNewLeads ? "#DC2626" : "#111827"};">${formatInteger(report.quality.inactiveSourcesReceivingNewLeads)}</td></tr></table>
    <table cellpadding="0" cellspacing="0" border="0" role="presentation" style="margin:24px 0 0;"><tr><td style="background:#0fc0df;border-radius:7px;"><a href="${APP_URL}/analytics/lead-cohorts" style="display:inline-block;padding:12px 22px;font-size:13px;font-weight:700;color:#0A0A0A;text-decoration:none;">Open Lead Cohort Analytics</a></td></tr></table>
    <div style="margin-top:16px;font-size:10px;line-height:1.45;color:#6B7280;">Attribution is based on the lead source stored on the primary transaction contact. Source hierarchy is shown as Parent · Sub-source. The report excludes archived contacts from lead and cohort calculations.</div>`;
}
...
export async function sendWeeklyLeadReportTest(asOf = new Date()): Promise<{ sent: boolean; skipped: boolean; reason?: string; report: WeeklyLeadReport }> {
  ...
  const delivery = await sendTransactionalEmail(
    ...
    },
    {
      ...
      idempotencyKey: `weekly-lead-report:test:${report.reportDateKey}:${TEST_RECIPIENT_EMAIL}`,
    },
  );
