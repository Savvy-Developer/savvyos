...
const TEST_MARKET_PROFILE: MarketProfile = {
  executiveSummary:
    "This is a SavvyOS workflow test that demonstrates the complete evidence-backed Agent Market profile format and the private local-feedback loop.",
  bestFitInvestors: [
    "Investors who value property-specific STR diligence",
    "Buyers seeking evidence-led market context",
  ],
  notIdealFor: [
    "Buyers seeking guaranteed revenue, financing, or regulatory outcomes",
  ],
  buyBox: {
    purchasePriceGuidance:
      "Validate current purchase-price guidance against live market evidence.",
    propertyTypes: ["STR-suitable homes"],
    bedroomGuidance:
      "Confirm guest demand and operating assumptions before purchase.",
  ...
  marketDynamics: [
    "Demand, supply, and regulation require current local validation.",
    "Operating economics should be assessed property by property.",
  ],
  agentGuidance: [
    "Use the profile as decision support rather than a guarantee.",
    "Submit local intelligence through this feedback workflow.",
  ],
  watchouts: [
    "Verify regulations, revenue, insurance, operating costs, and property-level feasibility.",
  ],
  evidenceNotes: [
    "This is a complete test snapshot for delivery validation.",
    "Live profiles use approved current market evidence.",
  ],
  researchGaps: [
    "Add current local research before using a test snapshot for client guidance.",
  ],
  confidence: "limited",
};
...
function listHtml(items: string[]): string {
  if (!items.length)
    return `<p style="margin:5px 0 14px;font-size:13px;line-height:1.55;color:#6B7280;">No current evidence was returned for this section.</p>`;
...
export function renderMarketProfileSnapshot(profile: MarketProfile): string {
  ...
  const buyBoxRows = [
    ...
    [
      ...
      asList(buyBox.propertyCharacteristics).join(" · "),
    ],
...
export function summarizeMarketProfileChanges(
  previous: MarketProfile | null | undefined,
  next: MarketProfile
): string[] {
  if (!previous)
    return [
      "A new living profile was generated with the current market read, investor fit, buy-box guidance, market dynamics, agent guidance, watchouts, evidence notes, research gaps, and confidence.",
    ];
  ...
  const changed = fields
    .filter(
      ([, oldValue, newValue]) => canonical(oldValue) !== canonical(newValue)
    )
    .map(([label]) => `${label} was updated.`);
  return changed.length
    ? changed
    : [
        "The profile was regenerated from updated evidence; its structured guidance did not materially change.",
      ];
}
  ...
  const [recentDelivery] = await db
    ...
    .where(
      and(
        ...
        eq(
          emailNotificationDeliveries.recipientEmail,
...
export async function notifyAssignedAgentsOfMarketProfileUpdate(params: {
  ...
  const changes = summarizeMarketProfileChanges(
    params.previousProfile,
    params.profile
  );
  ...
  for (const agent of agents) {
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(
            marketProfileFeedbackRequests.marketProfileId,
            params.marketProfileId
          ),
    ...
    if (await hasRecentMarketProfileUpdateEmail(db, agent.email)) {
      skipped += 1;
      console.info(
        `[AgentMarkets] Skipping market profile update email for ${agent.email}: a market AI update was sent within the last 48 hours.`
      );
      continue;
    }
    ...
    const feedbackUrl = await generateMagicLinkUrl(
      agent.email,
      `/agent-market-feedback/${requestId}`
    );
    const delivery = await sendTransactionalEmail(
      ...
      },
      {
        ...
        idempotencyKey: `market-profile-update:${requestId}`,
      }
    );
    if (delivery.sent) {
      notified += 1;
      ...
      console.warn(
        `[AgentMarkets] Could not deliver profile update email to ${agent.email}: ${delivery.reason ?? "unknown error"}`
      );
    }
  }
...
export async function sendMarketProfileUpdateTestEmail(params: {
  ...
  if (await hasRecentMarketProfileUpdateEmail(db, params.recipient.email)) {
    console.info(
      `[AgentMarkets] Skipping market profile update test email for ${params.recipient.email}: a market AI update was sent within the last 48 hours.`
    );
    return { skipped: true };
  }
  const [intelligence] = await db
    ...
    .where(
      eq(marketIntelligenceProfiles.marketProfileId, params.marketProfileId)
    )
    .limit(1);
  const profile =
    (intelligence?.profileJson as MarketProfile | null) ?? TEST_MARKET_PROFILE;
  ...
  try {
    ...
    const delivery = await sendTransactionalEmail(
      ...
      {
        injectMagicLinks: false,
        idempotencyKey: `market-profile-update-test:${requestId}`,
      }
    );
    if (!delivery.sent)
      throw new Error(
        delivery.reason ?? "Provider did not accept the test email."
      );
    return { requestId, skipped: false };
  } catch (error) {
    await db
      .delete(marketProfileFeedbackRequests)
